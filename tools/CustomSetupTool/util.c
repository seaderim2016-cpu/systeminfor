...
CONST PH_STRINGREF UninstallKeyNames[] = {
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall"
                      L"\\SystemInformer"),
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall"
                      L"\\SystemInformer-Preview"),
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall"
                      L"\\SystemInformer-Canary"),
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall"
                      L"\\SystemInformer-Developer"),
};
CONST PH_STRINGREF AppPathsKeyName =
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows\\CurrentVersion\\App "
                      L"Paths\\SystemInformer.exe");
CONST PH_STRINGREF TaskmgrIfeoKeyName =
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows NT\\CurrentVersion\\Image "
                      L"File Execution Options\\taskmgr.exe");
CONST PH_STRINGREF SystemInformerIfeoKeyName =
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows NT\\CurrentVersion\\Image "
                      L"File Execution Options\\SystemInformer.exe");
CONST PH_STRINGREF AppCompatFlagsLayersKeyName = PH_STRINGREF_INIT(
    L"Software\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\Layers");
CONST PH_STRINGREF CurrentUserRunKeyName =
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows\\CurrentVersion\\Run");
CONST PH_STRINGREF LocalDumpsKeyName =
    PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows\\Windows Error "
                      L"Reporting\\LocalDumps\\SystemInformer.exe");

/**
 * Deletes the uninstall keys for System Informer.
 */
VOID SetupDeleteUninstallKey(VOID) {
  for (ULONG i = 0; i < RTL_NUMBER_OF(UninstallKeyNames); i++) {
    HANDLE keyHandle;

    if (NT_SUCCESS(PhOpenKey(&keyHandle, DELETE | KEY_WOW64_64KEY,
                             PH_KEY_LOCAL_MACHINE, &UninstallKeyNames[i], 0))) {
 ...
 */
NTSTATUS SetupCreateUninstallKey(_In_ PPH_SETUP_CONTEXT Context) {
  NTSTATUS status;
  HANDLE keyHandle;
  PH_STRINGREF value;
  ...
  status = PhCreateKey(
      &keyHandle, KEY_ALL_ACCESS | KEY_WOW64_64KEY, PH_KEY_LOCAL_MACHINE,
      &UninstallKeyNames[PH_RELEASE_CHANNEL_ID], OBJ_OPENIF, 0, NULL);
  ...
  if (NT_SUCCESS(status)) {
    ...
    string = SetupCreateFullPath(Context->SetupInstallPath,
                                 L"\\systeminformer.exe,0");
    PhSetValueKeyZ(keyHandle, L"DisplayIcon", REG_SZ, string->Buffer,
                   (ULONG)string->Length + sizeof(UNICODE_NULL));

    PhInitializeStringRef(&value, L"System Informer");
    PhSetValueKeyZ(keyHandle, L"DisplayName", REG_SZ, value.Buffer,
    ...
    PhSetValueKeyZ(keyHandle, L"DisplayVersion", REG_SZ, string->Buffer,
                   (ULONG)string->Length + sizeof(UNICODE_NULL));

    PhInitializeStringRef(
        &value, L"https://system-informer.com/"); // package manager (winget)
                                                  // consistency (jxy-s)
    PhSetValueKeyZ(keyHandle, L"HelpLink", REG_SZ, value.Buffer,
                   (ULONG)value.Length + sizeof(UNICODE_NULL));

    string = SetupCreateFullPath(Context->SetupInstallPath, L"");
    PhSetValueKeyZ(keyHandle, L"InstallLocation", REG_SZ, string->Buffer,
                   (ULONG)string->Length + sizeof(UNICODE_NULL));

    PhInitializeStringRef(&value, L"Winsider Seminars & Solutions, Inc.");
    PhSetValueKeyZ(keyHandle, L"Publisher", REG_SZ, value.Buffer,
                   (ULONG)value.Length + sizeof(UNICODE_NULL));

    string = SetupCreateFullPath(Context->SetupInstallPath,
                                 L"\\systeminformer-setup.exe");
    PhMoveReference(&string,
                    PhFormatString(L"\"%s\" -uninstall", PhGetString(string)));
    PhSetValueKeyZ(keyHandle, L"UninstallString", REG_SZ, string->Buffer,
                   (ULONG)string->Length + sizeof(UNICODE_NULL));
 ...
 */
PPH_STRING SetupFindInstallDirectory(VOID) {
  PPH_STRING setupInstallPath = GetApplicationInstallPath();

  if (PhIsNullOrEmptyString(setupInstallPath)) {
    static CONST PH_STRINGREF programW6432 =
        PH_STRINGREF_INIT(L"%ProgramW6432%\\SystemInformer\\");
    static CONST PH_STRINGREF programFiles =
        PH_STRINGREF_INIT(L"%ProgramFiles%\\SystemInformer\\");
    SYSTEM_INFO info;
  ...
  if (!PhIsNullOrEmptyString(setupInstallPath)) {
    if (!PhEndsWithStringRef(&setupInstallPath->sr, &PhNtPathSeparatorString,
                             TRUE)) {
      PhSwapReference(
          &setupInstallPath,
          PhConcatStringRef2(&setupInstallPath->sr, &PhNtPathSeparatorString));
    }
  }
 ...
 * \param Context The setup context.
 */
VOID SetupDeleteAppdataDirectory(_In_ PPH_SETUP_CONTEXT Context) {
  PPH_STRING appdataDirectory;

  if (appdataDirectory = PhGetKnownFolderPathZ(&FOLDERID_RoamingAppData,
                                               L"\\SystemInformer\\")) {
...
VOID SetupDeleteSystemInformerIfeo(VOID) {
  HANDLE keyHandle;

  if (NT_SUCCESS(PhOpenKey(&keyHandle, DELETE, PH_KEY_LOCAL_MACHINE,
                           &SystemInformerIfeoKeyName, 0))) {
    NtDeleteKey(keyHandle);
    NtClose(keyHandle);
  }
}
...
_Function_class_(PH_ENUM_KEY_CALLBACK) BOOLEAN NTAPI
    SetupDeleteAppCompatFlagsLayersCallback(
        _In_ HANDLE RootDirectory, _In_ PKEY_VALUE_FULL_INFORMATION Information,
        _In_opt_ PVOID Context) {
  if (Information->Type == REG_SZ) {
    static CONST PH_STRINGREF fileNameSuffix =
        PH_STRINGREF_INIT(L"SystemInformer.exe");
...
VOID SetupDeleteAppCompatFlagsLayersEntry(VOID) {
  HANDLE keyHandle;

  if (NT_SUCCESS(PhOpenKey(&keyHandle, KEY_READ | KEY_WRITE | DELETE,
                           PH_KEY_CURRENT_USER, &AppCompatFlagsLayersKeyName,
                           0))) {
    PhEnumerateValueKey(keyHandle, KeyValueFullInformation,
                        SetupDeleteAppCompatFlagsLayersCallback, NULL);
    NtClose(keyHandle);
  }

  if (NT_SUCCESS(PhOpenKey(&keyHandle, KEY_READ | KEY_WRITE | DELETE,
                           PH_KEY_LOCAL_MACHINE, &AppCompatFlagsLayersKeyName,
                           0))) {
    PhEnumerateValueKey(keyHandle, KeyValueFullInformation,
                        SetupDeleteAppCompatFlagsLayersCallback, NULL);
    NtClose(keyHandle);
 ...
 * \return Successful or errant status.
 */
NTSTATUS SetupCreateUninstallFile(_In_ PPH_SETUP_CONTEXT Context) {
  NTSTATUS status;
  ...
  if (PhStartsWithStringRef2(&currentFilePath->sr,
                             PhGetString(Context->SetupInstallPath), TRUE)) {
    return STATUS_SUCCESS;
  }

  // Move the outdated setup.exe into the trash (temp) folder.

  uninstallFilePath = SetupCreateFullPath(Context->SetupInstallPath,
                                          L"\\systeminformer-setup.exe");
  ...
  if (PhDoesFileExistWin32(PhGetString(uninstallFilePath))) {
    ...
    if (!NT_SUCCESS(status =
                        PhMoveFileWin32(PhGetString(uninstallFilePath),
                                        PhGetString(tempFileName), FALSE))) {
  ...
  // 2. The correct ACL is applied to the file. Moving the file would retain the
  // existing ACL.

  if (!NT_SUCCESS(status =
                      PhCopyFileWin32(PhGetString(currentFilePath),
                                      PhGetString(uninstallFilePath), FALSE))) {
    return status;
  }
 ...
 * \param Context The setup context.
 */
VOID SetupDeleteUninstallFile(_In_ PPH_SETUP_CONTEXT Context) {
  PPH_STRING uninstallFilePath;
  ...
  if (PhDoesFileExistWin32(PhGetString(uninstallFilePath))) {
    ...
    PhMoveFileWin32(PhGetString(uninstallFilePath), PhGetString(tempFileName),
                    FALSE);
 ...
 * \return Successful or errant status.
 */
NTSTATUS SetupUninstallDriver(_In_ PPH_SETUP_CONTEXT Context) {
  NTSTATUS status = STATUS_SUCCESS;
  SC_HANDLE serviceHandle;

  if (NT_SUCCESS(PhOpenService(&serviceHandle,
                               SERVICE_QUERY_STATUS | SERVICE_STOP,
                               PhIsNullOrEmptyString(Context->SetupServiceName)
                                   ? L"KSystemInformer"
                                   : PhGetString(Context->SetupServiceName)))) {
  ...
  if (NT_SUCCESS(PhOpenService(&serviceHandle, DELETE,
                               PhIsNullOrEmptyString(Context->SetupServiceName)
                                   ? L"KSystemInformer"
                                   : PhGetString(Context->SetupServiceName)))) {
 ...
 * \param Context The setup context.
 */
VOID SetupCreateWindowsOptions(_In_ PPH_SETUP_CONTEXT Context) {
  // Create the app paths keys
  {
    ...
    status = PhCreateKey(&keyHandle, KEY_ALL_ACCESS | KEY_WOW64_64KEY,
                         PH_KEY_LOCAL_MACHINE, &AppPathsKeyName, OBJ_OPENIF, 0,
                         NULL);

    if (NT_SUCCESS(status)) {
      // string = PhFormatString(L"\"%s\\SystemInformer.exe\"",
      // PhGetString(Context->SetupInstallPath));

      if (string = SetupCreateFullPath(Context->SetupInstallPath,
                                       L"\\SystemInformer.exe")) {
        PhSetValueKey(keyHandle, NULL, REG_SZ, string->Buffer,
                      (ULONG)string->Length + sizeof(UNICODE_NULL));
  ...
  //{
  //    static PH_STRINGREF settingsPath =
  //    PH_STRINGREF_INIT(L"%APPDATA%\\SystemInformer\\settings.xml");
  ...
  //
  //        value = PhConcatStrings(3, L"\"", PhGetString(clientPathString),
  //        L"\""); status = NtSetValueKey(
  //            taskmgrKeyHandle,
  //            &valueName,
  //            0,
  //            REG_SZ,
  ...
  //    if (!NT_SUCCESS(status))
  //    {
  //        PhShowStatus(NULL, L"Unable to set the Windows default Task
  //        Manager.", status, 0);
  //    }
  //}
  ...
  //            L"\"");
  //
  //        NtSetValueKey(runKeyHandle, &valueName, 0, REG_SZ, value->Buffer,
  //        (ULONG)value->Length + 2); NtClose(runKeyHandle);
  //    }
  //    else
  //    {
  //        PhShowStatus(NULL, L"Unable to create the startup entry.", status,
  //        0);
  //    }
  //}
}
 ...
 */
_Function_class_(PH_ENUM_KEY_CALLBACK) BOOLEAN NTAPI
    SetupDeleteAutoRunKeyCallback(_In_ HANDLE RootDirectory,
                                  _In_ PKEY_VALUE_FULL_INFORMATION Information,
                                  _In_opt_ PVOID Context) {
  if (Information->Type == REG_SZ) {
    static CONST PH_STRINGREF fileName =
        PH_STRINGREF_INIT(L"\\SystemInformer.exe");
    PH_STRINGREF value;
 ...
 * \param Context The setup context.
 */
VOID SetupDeleteWindowsOptions(_In_ PPH_SETUP_CONTEXT Context) {
  HANDLE keyHandle;
  ...
  if (NT_SUCCESS(PhOpenKey(&keyHandle, KEY_READ | KEY_WRITE,
                           PH_KEY_CURRENT_USER, &CurrentUserRunKeyName, 0))) {
    PhEnumerateValueKey(keyHandle, KeyValueFullInformation,
                        SetupDeleteAutoRunKeyCallback, NULL);
    NtClose(keyHandle);
  }
}

/**
 * Creates the Windows Error Reporting LocalDumps registry key for
 * SystemInformer.exe.
 *
 * \return Successful or errant status.
 */
NTSTATUS SetupCreateLocalDumpsKey(VOID) {
  NTSTATUS status;
  HANDLE keyHandle;
  ...
  status = PhCreateKey(&keyHandle, KEY_ALL_ACCESS | KEY_WOW64_64KEY,
                       PH_KEY_LOCAL_MACHINE, &LocalDumpsKeyName, OBJ_OPENIF, 0,
                       NULL);

  if (NT_SUCCESS(status)) {
    // Set DumpCount (REG_DWORD) = 10
    dumpCountValue = 10;
    PhSetValueKeyZ(keyHandle, L"DumpCount", REG_DWORD, &dumpCountValue,
                   sizeof(ULONG));

    // Set DumpFolder (REG_SZ) = %APPDATA%\SystemInformer\CrashDumps
    PhInitializeStringRef(&dumpFolderValue,
                          L"%APPDATA%\\SystemInformer\\CrashDumps");
    PhSetValueKeyZ(keyHandle, L"DumpFolder", REG_SZ, dumpFolderValue.Buffer,
                   (ULONG)dumpFolderValue.Length + sizeof(UNICODE_NULL));

    // Set DumpType (REG_DWORD) = 1 (mini dump)
    dumpTypeValue = 1;
    PhSetValueKeyZ(keyHandle, L"DumpType", REG_DWORD, &dumpTypeValue,
                   sizeof(ULONG));
...
/**
 * Deletes the Windows Error Reporting LocalDumps registry key for
 * SystemInformer.exe.
 */
VOID SetupDeleteLocalDumpsKey(VOID) {
  HANDLE keyHandle;

  if (NT_SUCCESS(PhOpenKey(&keyHandle, DELETE | KEY_WOW64_64KEY,
                           PH_KEY_LOCAL_MACHINE, &LocalDumpsKeyName, 0))) {
    NtDeleteKey(keyHandle);
    NtClose(keyHandle);
  }
}
 ...
 * \param Context The setup context.
 */
VOID SetupCreateShortcuts(_In_ PPH_SETUP_CONTEXT Context) {
  PPH_STRING string;
  PPH_STRING clientPathString;
  HRESULT status;

  if (string = PhGetKnownFolderPathZ(
          &FOLDERID_ProgramData,
          L"\\Microsoft\\Windows\\Start Menu\\Programs\\System Informer.lnk")) {
    if (clientPathString = SetupCreateFullPath(Context->SetupInstallPath,
                                               L"\\SystemInformer.exe")) {
      status = SetupCreateLink(
          PhGetString(string), PhGetString(clientPathString),
          PhGetString(Context->SetupInstallPath), L"SystemInformer");
  ...
  if (string = PhGetKnownFolderPathZ(
          &FOLDERID_ProgramData,
          L"\\Microsoft\\Windows\\Start Menu\\Programs\\PE Viewer.lnk")) {
    if (clientPathString =
            SetupCreateFullPath(Context->SetupInstallPath, L"\\peview.exe")) {
      status = SetupCreateLink(
          PhGetString(string), PhGetString(clientPathString),
          PhGetString(Context->SetupInstallPath), L"SystemInformer_PEViewer");
  ...
  if (string = PhGetKnownFolderPathZ(&FOLDERID_PublicDesktop,
                                     L"\\System Informer.lnk")) {
    if (clientPathString = SetupCreateFullPath(Context->SetupInstallPath,
                                               L"\\SystemInformer.exe")) {
      status = SetupCreateLink(
          PhGetString(string), PhGetString(clientPathString),
 ...
 * \param Context The setup context.
 */
VOID SetupDeleteShortcuts(_In_ PPH_SETUP_CONTEXT Context) {
  PPH_STRING string;
 ...
 * \return Successful or errant status.
 */
NTSTATUS SetupExecuteApplication(_In_ PPH_SETUP_CONTEXT Context) {
  NTSTATUS status;
  PPH_STRING string;
  ...
  string =
      SetupCreateFullPath(Context->SetupInstallPath, L"\\SystemInformer.exe");
  parameters = PhCreateString(SETUP_APP_PARAMETERS);

  if (Context->Hide) {
    PhMoveReference(&parameters,
                    PhConcatStringRefZ(&parameters->sr, L" -hide"));
  }

  if (PhDoesFileExistWin32(PhGetString(string))) {
    status = PhShellExecuteEx(Context->DialogHandle, PhGetString(string),
                              PhGetString(parameters), NULL, SW_SHOW,
                              PH_SHELL_EXECUTE_DEFAULT, 0, NULL);
  } else {
...
/**
 * Upgrades the settings file from a legacy format (PH) to the new format
 * (System Informer).
 *
 * \return Successful or errant status.
 */
NTSTATUS SetupUpgradeSettingsFile(VOID) {
  NTSTATUS status = STATUS_SUCCESS;
  ...
  settingsFilePath = PhGetKnownFolderPathZ(&FOLDERID_RoamingAppData,
                                           L"\\SystemInformer\\settings.xml");
  ...
  legacyNightlyFileName = PhGetKnownFolderPathZ(
      &FOLDERID_RoamingAppData, L"\\Process Hacker\\settings.xml");

  // Construct legacy stable build settings.xml path
  legacySettingsFileName = PhGetKnownFolderPathZ(
      &FOLDERID_RoamingAppData, L"\\Process Hacker 2\\settings.xml");

  // Attempt to migrate nightly build settings if they exist
  if (legacyNightlyFileName &&
      PhDoesFileExistWin32(PhGetString(legacyNightlyFileName))) {
    if (NT_SUCCESS(PhCopyFileWin32(PhGetString(legacyNightlyFileName),
                                   PhGetString(settingsFilePath), TRUE))) {
  ...
  if (!migratedNightly && legacySettingsFileName &&
      PhDoesFileExistWin32(PhGetString(legacySettingsFileName))) {
    if (NT_SUCCESS(PhCopyFileWin32(PhGetString(legacySettingsFileName),
                                   PhGetString(settingsFilePath), TRUE))) {
      status = STATUS_SUCCESS;
      goto CleanupExit;
    } else {
...
CleanupExit:
  if (legacySettingsFileName)
    PhDereferenceObject(legacySettingsFileName);
  if (legacyNightlyFileName)
    PhDereferenceObject(legacyNightlyFileName);
  if (settingsFilePath)
    PhDereferenceObject(settingsFilePath);
  return status;
}
 ...
 * \return Successful or errant status.
 */
NTSTATUS SetupConvertSettingsFile(VOID) {
  ...
  //
  convertFilePath = PhGetKnownFolderPathZ(&FOLDERID_RoamingAppData,
                                          L"\\SystemInformer\\settings.json");

  if (PhIsNullOrEmptyString(convertFilePath))
    goto CleanupExit;

  //
  // Check if the settings.json file actually exists before attempting
  // conversion
  //
  ...
  //
  status =
      PhConvertSettingsXmlToJson(&settingsFilePath->sr, &convertFilePath->sr);

CleanupExit:
  if (settingsFilePath)
    PhDereferenceObject(settingsFilePath);
  if (convertFilePath)
    PhDereferenceObject(convertFilePath);
  return status;
}
 ...
 * \return Successful or errant status.
 */
NTSTATUS ExtractResourceToFile(_In_ PVOID DllBase, _In_ PCWSTR Name,
  ...
  status = PhLoadResource(DllBase, Name, RT_RCDATA, &resourceLength,
                          &resourceBuffer);
  ...
  status = PhCreateFileWin32Ex(
      &fileHandle, FileName, FILE_GENERIC_READ | FILE_GENERIC_WRITE | DELETE,
  ...
  status = NtWriteFile(fileHandle, NULL, NULL, NULL, &isb, resourceBuffer,
                       resourceLength, NULL, NULL);
...
BOOLEAN ConnectionAvailable(VOID) {
  ...
  if (SUCCEEDED(CoCreateInstance(&CLSID_NetworkListManager, NULL, CLSCTX_ALL,
    ...
    INetworkListManager_get_IsConnectedToInternet(networkListManager,
                                                  &isConnectedInternet);

    // Cleanup the INetworkListManger COM objects.
    INetworkListManager_Release(networkListManager);

    // Check if Windows is connected to a network and it's connected to the
    // internet.
 ...
 */
HRESULT SetupCreateLink(_In_ PCWSTR LinkFilePath, _In_ PCWSTR FilePath,
                        _In_ PCWSTR FileParentDir, _In_ PCWSTR AppId) {
  HRESULT status = S_OK;
  ...
  status = CoCreateInstance(&CLSID_ShellLink, NULL, CLSCTX_INPROC_SERVER,
                            &IID_IShellLinkW, &shellLinkPtr);
  if (FAILED(status))
    goto CleanupExit;

  status = IShellLinkW_QueryInterface(shellLinkPtr, &IID_IPersistFile,
                                      &persistFilePtr);
  if (FAILED(status))
    goto CleanupExit;

  status = IShellLinkW_QueryInterface(shellLinkPtr, &IID_IPropertyStore,
                                      &propertyStorePtr);
  if (SUCCEEDED(status)) // Only proceed if QueryInterface for IPropertyStore
                         // succeeded
  {
    ...
    memset(V_UNION(&propValue, pwszVal), 0,
           propValueLength + sizeof(UNICODE_NULL));
    memcpy(V_UNION(&propValue, pwszVal), AppId, propValueLength);

    status = IPropertyStore_SetValue(propertyStorePtr, &PKEY_AppUserModel_ID,
                                     &propValue);
    // If SetValue fails, we might still want to try to save the shortcut
    // (without AppId) or just let the CleanupExit handle it. For now, we
    // continue and check overall save status.

    IPropertyStore_Commit(propertyStorePtr);
    IPropertyStore_Release(
        propertyStorePtr);   // Release immediately if successful
    propertyStorePtr = NULL; // Clear pointer after release

    PropVariantClear(&propValue);
  }
  // If QueryInterface for IPropertyStore failed, status remains the error from
  // that, and we proceed without setting AppId.

  // IShellLinkW_SetDescription(shellLinkPtr, FileComment);
  // IShellLinkW_SetHotkey(shellLinkPtr, MAKEWORD(VK_END, HOTKEYF_CONTROL |
  // HOTKEYF_ALT));
...
CleanupExit:
  ...
  if (propertyStorePtr) // Ensure propertyStorePtr is released if it was
 ...
 */
BOOLEAN CheckApplicationInstalled(VOID) {
 ...
 */
PPH_STRING CreateLegacyApplicationName(VOID) {
  PH_FORMAT format[18];
 ...
 */
BOOLEAN CheckApplicationInstallPathLegacy(_In_ PPH_STRING Directory) {
  BOOLEAN installed = FALSE;
  PPH_STRING fileName;
  PPH_STRING exePath;
 ...
 */
PPH_STRING GetApplicationInstallPath(VOID) {
  HANDLE keyHandle;
  PPH_STRING installPath = NULL;

  for (ULONG i = 0; i < RTL_NUMBER_OF(UninstallKeyNames); i++) {
    if (NT_SUCCESS(PhOpenKey(&keyHandle, KEY_READ | KEY_WOW64_64KEY,
                             PH_KEY_LOCAL_MACHINE, &UninstallKeyNames[i], 0))) {
  ...
  PhMoveReference(&installPath,
                  PhGetApplicationDirectoryFileName(&debugpath, FALSE));
#endif
 ...
 */
NTSTATUS SetupLegacySetupInstalled(VOID) {
  ...
  keyName = PhConcatStrings(
      7, L"Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\",
      PhGetString(processString), L"_", PhGetString(hackerString), L"2", L"_",
      L"is1");
  if (!keyName)
    goto CleanupExit;
...
CleanupExit:
  if (processString)
    PhDereferenceObject(processString);
  if (hackerString)
    PhDereferenceObject(hackerString);
  if (keyName)
...
_Function_class_(PH_ENUM_DIRECTORY_OBJECTS) static NTSTATUS NTAPI
  ...
  if (NT_SUCCESS(PhGetMutantOwnerInformation(objectHandle, &objectInfo))) {
    ...
    // Do not terminate the setup process if it's the parent of this process.
    // This scenario happens when the setup process restarts itself for
    // elevation. The parent process will return the same exit code as this
    // setup process instance, terminating breaks that.
    if (setupMutant &&
        NT_SUCCESS(
            PhGetProcessBasicInformation(NtCurrentProcess(), &processInfo)) &&
        processInfo.InheritedFromUniqueProcessId ==
            objectInfo.ClientId.UniqueProcess)
      goto CleanupExit;

    PhOpenProcessClientId(&processHandle,
                          PROCESS_QUERY_LIMITED_INFORMATION |
                              PROCESS_TERMINATE | SYNCHRONIZE,
                          &objectInfo.ClientId);

    hwnd = PhGetProcessMainWindowEx(objectInfo.ClientId.UniqueProcess,
                                    processHandle, FALSE);
 ...
 * \return TRUE to continue enumeration, FALSE to stop.
 */
_Function_class_(PH_ENUM_DIRECTORY_FILE) static BOOLEAN CALLBACK
    SetupCheckDirectoryCallback(_In_ HANDLE RootDirectory,
                                _In_ PVOID Information, _In_ PVOID Context) {
  ...
  if (PhEqualStringRef2(&baseName, L".", TRUE) ||
      PhEqualStringRef2(&baseName, L"..", TRUE))
    return TRUE;

  status = PhOpenFile(
      &fileHandle, &baseName, FILE_READ_ATTRIBUTES | DELETE | SYNCHRONIZE,
      RootDirectory, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
      FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL);

  if (NT_SUCCESS(status)) {
    PFILE_PROCESS_IDS_USING_FILE_INFORMATION processIds;

    status = PhGetProcessIdsUsingFile(fileHandle, &processIds);
      ...
      for (ULONG i = 0; i < processIds->NumberOfProcessIdsInList; i++) {
        ...
        if (NT_SUCCESS(status)) {
          ...
          if (baseDirectory = PhGetBaseDirectory(fileName)) {
            if (PhStartsWithString(context->SetupInstallPath, baseDirectory,
                                   TRUE)) {
              ...
              if (NT_SUCCESS(PhOpenProcess(&processHandle,
                ...
                if (windowHandle = PhGetProcessMainWindowEx(
                        processId, processHandle, FALSE)) {
                  PhEndWindowSession(windowHandle);
                  // SendMessageTimeout(windowHandle, WM_QUIT, 0, 0, SMTO_BLOCK,
                  // 5000, NULL);
                }
 ...
 * \return Successful or errant status.
 */
NTSTATUS SetupShutdownApplication(_In_ PPH_SETUP_CONTEXT Context) {
  NTSTATUS status;
  ...
  PhEnumDirectoryObjects(PhGetNamespaceHandle(), PhpPreviousInstancesCallback,
                         NULL);

  status = PhCreateFileWin32(
      &directoryHandle, PhGetString(Context->SetupInstallPath),
      FILE_LIST_DIRECTORY | SYNCHRONIZE, FILE_ATTRIBUTE_DIRECTORY,
      FILE_SHARE_READ, FILE_OPEN,
      FILE_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT);

  if (NT_SUCCESS(status)) {
    status = PhEnumDirectoryFile(directoryHandle, NULL,
                                 SetupCheckDirectoryCallback, Context);
 ...
 */
PPH_STRING SetupCreateFullPath(_In_ PPH_STRING Path, _In_ PCWSTR FileName) {
 ...
 * \return Successful or errant status.
 */
NTSTATUS SetupOverwriteFile(_In_ PPH_STRING FileName, _In_ PVOID Buffer,
  ...
  status = PhCreateFileWin32Ex(
      &fileHandle, PhGetString(FileName), FILE_GENERIC_WRITE | DELETE,
      &allocationSize, FILE_ATTRIBUTE_NORMAL,
      FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE, FILE_OVERWRITE_IF,
      FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL);
  ...
  status = NtWriteFile(fileHandle, NULL, NULL, NULL, &isb, Buffer, BufferLength,
                       NULL, NULL);
 ...
 * \return Successful or errant status.
 */
NTSTATUS SetupHashFile(_In_ PPH_STRING FileName,
  ...
  status = PhCreateFileWin32(
      &fileHandle, PhGetString(FileName), FILE_GENERIC_READ,
