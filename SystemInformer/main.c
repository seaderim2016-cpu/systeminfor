...
INT WINAPI wWinMain(_In_ HINSTANCE Instance, _In_opt_ HINSTANCE PrevInstance,
  ...
  if (!NT_SUCCESS(PhInitializePhLib(L"sPicyaDham")))
    return 1;
  ...
  if (PhStartupParameters.RunAsServiceMode) {
    PhExitApplication(
        PhRunAsServiceStart(PhStartupParameters.RunAsServiceMode));
  }
  ...
  // N.B. Must be called after loading plugins since we set Microsoft signed
  // only.
  ...
  if (PhIsExecutingInWow64()) {
    PhShowWarning2(
        NULL, L"Warning.", L"%s",
        L"You are attempting to run the 32-bit version of System Informer on "
        L"64-bit Windows. "
  ...
  // Set the default timer resolution.
  {
    if (WindowsVersion >= WINDOWS_11_24H2) {
      PhSetProcessPowerThrottlingState(
          NtCurrentProcess(), POWER_THROTTLING_PROCESS_IGNORE_TIMER_RESOLUTION,
 ...
 *
 * Processes Windows messages for the main window, dialogs, and registered
 * message loop filters. Handles accelerator keys, dialog messages, and
 * dispatches messages to the appropriate handlers. Drains the auto pool after
 * each message to manage memory efficiently.
 * \return The exit code of the application, typically the wParam of the WM_QUIT
 * message.
 */
LONG PhMainMessageLoop(VOID) {
  ...
  acceleratorTable = LoadAccelerators(NtCurrentImageBase(),
  ...
  while (result = GetMessage(&message, NULL, 0, 0)) {
    ...
    if (!processed) {
      if (message.hwnd == PhMainWndHandle ||
 ...
 *
 * This function adds the specified dialog window handle to the internal list of
 * dialogs managed by the application. This allows the main message loop to
 * recognize and process messages for the dialog, enabling features such as
 * keyboard navigation and message routing.
 *
 * \param DialogWindowHandle The handle to the dialog window to register.
 * \remarks Registered dialogs are processed in the main message loop for
 ...
 *
 * This function removes the specified dialog window handle from the internal
 * list of dialogs managed by the application. This ensures that the main
 * message loop no longer processes messages for the dialog, disabling features
 * such as keyboard navigation and message routing for the unregistered dialog.
 *
 * \param DialogWindowHandle The handle to the dialog window to unregister.
 * \remarks Unregistered dialogs are no longer processed in the main message
 * loop for dialog-specific messages.
 */
 ...
 *
 * This function adds a custom filter to the internal list of message loop
 * filters. Each filter is a callback that can intercept and process Windows
 * messages before they are handled by the main window or dialogs. Filters are
 * useful for implementing global hotkeys, custom message handling, or
 ...
 * \return A pointer to the filter entry, which can be used to unregister the
 * filter later.
 * \remarks The filter will be called for each message in the main message loop.
 */
PPH_MESSAGE_LOOP_FILTER_ENTRY
PhRegisterMessageLoopFilter(_In_ PPH_MESSAGE_LOOP_FILTER Filter,
 ...
 * expected window class name. If found, attempts to activate and bring the
 ...
 * \return FALSE if the previous instance window was found and activated; TRUE
 * to continue enumeration.
 */
_Function_class_(PH_WINDOW_ENUM_CALLBACK) static BOOLEAN CALLBACK
    PhPreviousInstanceWindowEnumProc(_In_ HWND WindowHandle,
                                     _In_ PVOID Context) {
  PPHP_PREVIOUS_MAIN_WINDOW_CONTEXT context =
      (PPHP_PREVIOUS_MAIN_WINDOW_CONTEXT)Context;
  CLIENT_ID clientId;
  ...
  if (clientId.UniqueProcess == context->ProcessId) {
    ...
    if (GetClassName(WindowHandle, className, RTL_NUMBER_OF(className))) {
      if (PhEqualStringZ(className, PhGetString(context->WindowName), FALSE)) {
        ...
        PhTrace("Found previous instance window: %ls (%p)", className,
                WindowHandle);

        SendMessageTimeout(WindowHandle, WM_PH_ACTIVATE,
                           PhStartupParameters.SelectPid, 0,
 ...
 *
 * This function attempts to locate the main window of a previous instance of
 * the application (by process ID and window class name), and brings it to the
 * foreground. It performs several checks to ensure the process is in the same
 * session and owned by the same user. The search is retried multiple times to
 * handle race conditions.
 * \param ProcessId The process ID of the previous instance.
 */
static VOID PhForegroundPreviousInstance(_In_ HANDLE ProcessId) {
  ...
  PhTraceFuncEnter("Foreground previous instance: %lu",
  ...
  if (PhIsNullOrEmptyString(context.WindowName))
    goto CleanupExit;
  if (!NT_SUCCESS(PhOpenProcess(&processHandle,
                                PROCESS_QUERY_LIMITED_INFORMATION, ProcessId)))
    goto CleanupExit;
  ...
  // Try to locate the window a few times because some users reported that it
  // might not yet have been created. (dmex)
  do {
...
CleanupExit:
  ...
  PhTraceFuncExit("Foreground previous instance: %lu (%lu attempts)",
                  HandleToUlong(ProcessId), attempts);
}

/**
 * Initializes previous instance detection and activation logic.
 *
 * This function enforces single-instance behavior and handles elevation
 * scenarios. If only one instance is allowed, it attempts to activate a
 * previous instance. If elevation is required, it runs the application as
 * administrator and activates the previous instance if necessary.
 */
VOID PhInitializePreviousInstance(VOID) {
  if (PhGetIntegerSetting(SETTING_ALLOW_ONLY_ONE_INSTANCE) &&
      !PhStartupParameters.NewInstance && !PhStartupParameters.ShowOptions &&
      !PhStartupParameters.PhSvc && !PhStartupParameters.KphStartupHigh &&
 ...
 *
 * This function is called for each object in the namespace directory. It looks
 * for mutant objects and, if found, checks if they belong to a different
 * process. If so, it attempts to bring the previous instance to the foreground.
 *
...
_Function_class_(PH_ENUM_DIRECTORY_OBJECTS) NTSTATUS NTAPI
    PhpPreviousInstancesCallback(_In_ HANDLE RootDirectory,
  ...
  if (NT_SUCCESS(PhOpenMutant(&objectHandle, MUTANT_QUERY_STATE, RootDirectory,
                              Name))) {
 ...
 *
 * This function enumerates namespace objects to find a mutant object
 * representing a previous instance. If found, it brings the previous instance's
 * window to the foreground and exits the current process.
 */
VOID PhActivatePreviousInstance(VOID) {
  ...
  status = PhEnumDirectoryObjects(PhGetNamespaceHandle(),
                                  PhpPreviousInstancesCallback, NULL);
  ...
  //
  //         {
  //             for (ULONG i = 0; i < processIds->NumberOfProcessIdsInList;
  //             i++)
  //             {
  ...
  //                 PhGetProcessImageFileNameByProcessId(processId,
  //                 &fileName)))
  //                 {
 ...
 *
 * This function checks if the process's current directory matches the
 * application's directory. If not, it sets the current directory to the
 * application's directory.
 */
NTSTATUS PhInitializeDirectoryPolicy(VOID) {
  ...
  if (!PhStringRefToUnicodeString(&applicationDirectory->sr,
                                  &applicationDirectoryUs)) {
    PhDereferenceObject(applicationDirectory);
    return STATUS_UNSUCCESSFUL;
  }
...
VOID PhpCreateUnhandledExceptionCrashDump(
    _In_ PEXCEPTION_POINTERS ExceptionInfo, _In_ PH_TRIAGE_DUMP_TYPE DumpType) {
  HANDLE fileHandle;
  ...
  fileName = PhConcatStrings(5, PhGetString(directory), L"SystemInformer",
                             L"_DumpFile_", alphastring, L".dmp");
  PhCreateDirectoryFullPath(&fileName->sr);

  if (NT_SUCCESS(PhCreateFile(
          &fileHandle, &fileName->sr, FILE_GENERIC_WRITE, FILE_ATTRIBUTE_NORMAL,
    ...
    case PhTriageDumpTypeMinimal:
      dumpType = MiniDumpWithDataSegs | MiniDumpWithUnloadedModules |
                 MiniDumpWithProcessThreadData | MiniDumpWithThreadInfo |
                 MiniDumpIgnoreInaccessibleMemory;
      break;
    case PhTriageDumpTypeNormal:
      dumpType = MiniDumpWithDataSegs | MiniDumpWithHandleData |
    ...
    case PhTriageDumpTypeFull:
      dumpType = MiniDumpWithDataSegs | MiniDumpWithFullMemory |
                 ...
                 MiniDumpWithProcessThreadData | MiniDumpWithFullMemoryInfo |
                 MiniDumpWithThreadInfo | MiniDumpIgnoreInaccessibleMemory |
                 MiniDumpWithTokenInformation | MiniDumpWithAvxXStateContext;
      break;
    }

    PhWriteMiniDumpProcess(NtCurrentProcess(), NtCurrentProcessId(), fileHandle,
                           dumpType, &exceptionInfo, NULL, NULL);
 ...
 *
 * This function is called when an unhandled exception occurs. It presents the
 * user with options to create a crash dump, restart, ignore, or exit. It also
 * generates a crash dump file on the desktop if requested.
 * \param ExceptionInfo Pointer to the exception information.
 * \return An exception disposition value.
 */
LONG CALLBACK
PhpUnhandledExceptionCallback(_In_ PEXCEPTION_POINTERS ExceptionInfo) {
  ...
  if (NT_SUCCESS(PhIsInteractiveUserSession())) {
    ...
    TASKDIALOG_BUTTON buttons[6] = {
        {101, L"Full\nA complete dump of the process, rarely needed most of "
              L"the time."},
        {102, L"Normal\nFor most purposes, this dump file is the most useful."},
        {103, L"Minimal\nA very limited dump with limited data."},
        {104, L"Restart\nRestart the application."}, // and hope it doesn't
                                                     // crash again.";
    ...
    if (NT_NTWIN32(ExceptionInfo->ExceptionRecord->ExceptionCode))
      errorMessage = PhGetStatusMessage(
          0,
          PhNtStatusToDosError(ExceptionInfo->ExceptionRecord->ExceptionCode));
    else
      errorMessage =
          PhGetStatusMessage(ExceptionInfo->ExceptionRecord->ExceptionCode, 0);

    message = PhFormatString(L"0x%08X (%s)",
    ...
    if (PhShowTaskDialog(&config, &result, NULL, NULL)) {
      ...
      case 104: {
        PhShellProcessHacker(
            NULL, NULL, SW_SHOW, PH_SHELL_EXECUTE_DEFAULT,
      ...
      if (PhShowMessage(NULL, MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2,
    ...
    if (WinStationSendMessageW(
            ...
            title->Buffer, (ULONG)title->Length, message->Buffer,
            (ULONG)message->Length, MB_OKCANCEL | MB_ICONERROR, 30, &response,
            FALSE)) {
#ifdef DEBUG
      if (response == IDOK) {
        PhpCreateUnhandledExceptionCrashDump(ExceptionInfo,
 ...
 *
 * This function configures the process error mode to suppress critical error
 * dialogs and disables the Windows error reporting dialog for unhandled
 * exceptions. It also installs a custom unhandled exception filter to handle
 * application crashes, generate crash dumps, and present user-friendly dialogs
 * or messages.
 */
NTSTATUS PhInitializeExceptionPolicy(VOID) {
#if PHNT_MINIMAL_ERRORMODE
  ULONG errorMode;

  if (NT_SUCCESS(PhGetProcessErrorMode(NtCurrentProcess(), &errorMode))) {
    ClearFlag(errorMode, SEM_FAILCRITICALERRORS | SEM_NOGPFAULTERRORBOX |
...
#endif
  PhpPreviousUnhandledExceptionFilter =
 ...
 *
 * This function creates a named mutant object in the process's namespace to
 * enforce single-instance behavior and activate previous instances of System
 * Informer.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhInitializeNamespacePolicy(VOID) {
  HANDLE mutantHandle;
  SIZE_T returnLength;
  ...
  InitializeObjectAttributes(&objectAttributes, &objectName,
                             OBJ_CASE_INSENSITIVE, PhGetNamespaceHandle(),
                             NULL);

  NtCreateMutant(&mutantHandle, MUTANT_QUERY_STATE, &objectAttributes, TRUE);
 ...
 *
 * If PH_COM_SVC is defined, it configures global COM options and sets a custom
 * security descriptor that allows access to authenticated users, local system,
 * and administrators. Otherwise, it simply initializes COM with apartment
 * threading and disables OLE1 DDE.
 */
NTSTATUS PhInitializeComPolicy(VOID) {
#ifdef PH_COM_SVC
#include <cguid.h>
...
#define HKLM_ONLY_CLASSIC_COM_CATALOG COMGLB_RESERVED5
    ...
    IGlobalOptions_Set(globalOptions, COMGLB_EXCEPTION_HANDLING,
                       COMGLB_EXCEPTION_DONOT_HANDLE_ANY);
    IGlobalOptions_Set(globalOptions, COMGLB_RO_SETTINGS,
                       COMGLB_FAST_RUNDOWN | COMGLB_ENABLE_AGILE_OOP_PROXIES |
                           HKLM_ONLY_CLASSIC_COM_CATALOG);
    IGlobalOptions_Release(globalOptions);
  }

  securityDescriptorAllocationLength =
      SECURITY_DESCRIPTOR_MIN_LENGTH + (ULONG)sizeof(ACL) +
      (ULONG)sizeof(ACCESS_ALLOWED_ACE) +
      PhLengthSid(&PhSeAuthenticatedUserSid) +
      (ULONG)sizeof(ACCESS_ALLOWED_ACE) + PhLengthSid(&PhSeLocalSystemSid) +
      (ULONG)sizeof(ACCESS_ALLOWED_ACE) + PhLengthSid(&administratorsSid);
  ...
  PhCreateAcl(
      dacl, securityDescriptorAllocationLength - SECURITY_DESCRIPTOR_MIN_LENGTH,
      ACL_REVISION);
  PhAddAccessAllowedAce(dacl, ACL_REVISION, FILE_READ_DATA | FILE_WRITE_DATA,
                        &PhSeAuthenticatedUserSid);
  PhAddAccessAllowedAce(dacl, ACL_REVISION, FILE_READ_DATA | FILE_WRITE_DATA,
                        &PhSeLocalSystemSid);
  PhAddAccessAllowedAce(dacl, ACL_REVISION, FILE_READ_DATA | FILE_WRITE_DATA,
                        administratorsSid);
  ...
  if (!SUCCEEDED(CoInitializeSecurity(securityDescriptor, UINT_MAX, NULL, NULL,
  ...
  assert(RtlLengthSecurityDescriptor(securityDescriptor) <
 ...
 *
 * This function checks if the Shift key is held down during startup. If so, it
 * attempts to launch Task Manager (`taskmgr.exe`) instead of System Informer.
 * This provides a quick way for users to access Task Manager if needed, for
 * example, if System Informer is set as the default Task Manager replacement
 * and the user wants to access the original Task Manager without changing
 * settings.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhInitializeExecutionPolicy(VOID) {
  // Note: GetAsyncKeyState queries the global keyboard bitmask without kernel
  // transitions, blocking, handles, events or messages etc... The bitmask is
  // also independent of any message loop or window. We can call it extremely
  // early but only the high bit (0x8000) of the key state is valid without a
  // message loop.

  if (GetAsyncKeyState(VK_SHIFT) & 0x8000) {
    if (NT_SUCCESS(PhShellExecuteEx(NULL, L"taskmgr.exe", NULL, NULL, SW_SHOW,
                                    0, 0, NULL))) {
      PhExitApplication(STATUS_SUCCESS);
    }
  }
 ...
 *
 * This function configures process-level mitigations to prevent crashes by
 * third party software. * The function uses the Native API to set the
 * mitigation policy. If the operating system does not support the required
 * mitigation policies, the function performs no action and returns success.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhInitializeMitigationPolicy(VOID) {
  ...
  if (WindowsVersion >= WINDOWS_10) {
    ...
    // NtSetInformationProcess(NtCurrentProcess(), ProcessMitigationPolicy,
    // &policyInfo, sizeof(PROCESS_MITIGATION_POLICY_INFORMATION));
 ...
 *
 * This function creates a new random desktop, switches the current thread and
 * input to that desktop, and launches a new instance of System Informer on it.
 * After the child process exits, it restores the original desktop and cleans up
 * resources.
 *
 * \remarks The function is typically used for scenarios where process isolation
 * or UI testing is required on a separate desktop, such as security testing or
 * debugging.
 */
VOID PhInitializeDesktopPolicy(VOID) {
  NTSTATUS status;
  HDESK desktopHandle;
  WCHAR alphastring[16] = L"";
  ...
  if (desktopHandle = CreateDesktop(alphastring, NULL, NULL, 0,
 ...
 *
 * This function configures the process to trigger a system critical break if it
 * is terminated, based on the specified flag. When enabled, the process is
 * protected from termination by non-privileged users, and the system will
 * generate a critical error if the process is killed. This is typically used to
 * prevent accidental or unauthorized termination of critical processes. The
 * function only applies the policy if the current process is running with
 * elevated privileges and the "Enable Break On Termination" setting is enabled.
 * \param Enabled TRUE to enable the break-on-termination policy; FALSE to
 * disable it.
 */
VOID PhEnableTerminationPolicy(_In_ BOOLEAN Enabled) {
  if (PhGetOwnTokenAttributes().Elevated &&
      PhGetIntegerSetting(SETTING_ENABLE_BREAK_ON_TERMINATION)) {
    NTSTATUS status;

    status = PhSetProcessBreakOnTermination(NtCurrentProcess(), Enabled);
 ...
 * throughout the System Informer application for timing operations and
 * measurements.
 */
NTSTATUS PhInitializeTimerPolicy(VOID) {
  static BOOL timerSuppression = FALSE;

  SetUserObjectInformation(NtCurrentProcess(),
                           UOI_TIMERPROC_EXCEPTION_SUPPRESSION,
                           &timerSuppression, sizeof(BOOL));
 ...
 */
NTSTATUS PhInitializeAppSystem(VOID) {
 ...
 */
VOID PhInitializeAppSettings(VOID) {
  ...
  if (!PhStartupParameters.NoSettings) {
    ...
    // 2. A file named SystemInformer.exe.settings.json in the program
    ...
    if (PhStartupParameters.SettingsFileName) {
      if (PhDetermineDosPathNameType(
              &PhStartupParameters.SettingsFileName->sr) == RtlPathTypeRooted) {
        PhSetReference(&PhSettingsFileName,
        ...
        if (NT_SUCCESS(
                PhGetFullPath(PhGetString(PhStartupParameters.SettingsFileName),
                              &settingsFileName, NULL))) {
          PhMoveReference(&settingsFileName,
                          PhDosPathNameToNtPathName(&settingsFileName->sr));
    ...
    if (PhIsNullOrEmptyString(PhSettingsFileName) || !NT_SUCCESS(status)) {
      status = PhLoadSettingsAutoDetect(NULL, L"settings", &settingsPath, NULL,
                                        &PhPortableEnabled);
    ...
    if (status == STATUS_FILE_CORRUPT_ERROR) {
      if (PhShowMessage2(NULL, TD_YES_BUTTON | TD_NO_BUTTON, TD_WARNING_ICON,
                         L"System Informer's settings file is corrupt. Do you "
                         L"want to reset it?",
                         L"If you select No, the settings system will not "
                         L"function properly.") == IDYES) {
        if (PhSettingsFileName)
  ...
  //    {
  //        if
  //        (PhDetermineDosPathNameType(&PhStartupParameters.SettingsFileName->sr)
  //        == RtlPathTypeRooted)
  //        {
  //            PhSetReference(&PhSettingsFileName,
  //            PhStartupParameters.SettingsFileName);
  ...
  //            // Get an absolute path now.
  //            if
  //            (NT_SUCCESS(PhGetFullPath(PhGetString(PhStartupParameters.SettingsFileName),
  //            &settingsFileName, NULL)))
  //            {
  //                PhMoveReference(&settingsFileName,
  //                PhDosPathNameToNtPathName(&settingsFileName->sr));
  ...
  //                if (settingsFileName =
  //                PhGetApplicationFileNameZ(L".settings.xml"))
  //                {
  //                    if (PhDoesFileExist(&settingsFileName->sr))
  //                    {
  //                        PPH_STRING jsonFileName =
  //                        PhGetBaseNameChangeExtensionZ(&settingsFileName->sr,
  //                        L".json");
  ...
  //                        {
  //                            PhMoveReference(&PhSettingsFileName,
  //                            jsonFileName); PhPortableEnabled = TRUE;
  //                        }
  //                        else
  //                        {
  //                            PhShowStatus(NULL, L"Unable to convert settings
  //                            to json format.", convertStatus, 0);
  ...
  //    if (PhIsNullOrEmptyString(PhSettingsFileName))
  //    {
  //        PhSettingsFileName = PhGetKnownLocationZ(PH_FOLDERID_RoamingAppData,
  //        L"\\SystemInformer\\settings.json", TRUE);
  //    }
  ...
  //        {
  //            PPH_STRING xmlFileName =
  //            PhGetBaseNameChangeExtensionZ(&PhSettingsFileName->sr, L".xml");
  ...
  //                    else
  //                    {
  //                        PhShowStatus(NULL, L"Unable to convert settings to
  //                        json format.", convertStatus, 0);
  //                    }
  //                }
  ...
  //                TD_WARNING_ICON,
  //                L"System Informer's settings file is corrupt. Do you want to
  //                reset it?", L"If you select No, the settings system will not
  //                function properly." ) == IDYES)
  //            {
  //                PhResetSettingsFile(&PhSettingsFileName->sr);
  //            }
  //            else
  //            {
  //                // Pretend we don't have a settings file, the user can
  //                resolve the issue
  //                // in this case without resetting the user(s) settings or
  //                customization to defaults.
  //                PhDereferenceObject(PhSettingsFileName);
  //                PhSettingsFileName = NULL;
  //            }
  //        }
  ...
  //        {
  //            PhShowStatus(NULL, L"Unable to load the settings file.", status,
  //            0);
  //        }
  //    }
  //}
  ...
  PhThemeWindowForegroundColor =
      PhGetIntegerSetting(SETTING_THEME_WINDOW_FOREGROUND_COLOR);
  PhThemeWindowBackgroundColor =
      PhGetIntegerSetting(SETTING_THEME_WINDOW_BACKGROUND_COLOR);
  PhThemeWindowBackground2Color =
      PhGetIntegerSetting(SETTING_THEME_WINDOW_BACKGROUND2_COLOR);
  PhThemeWindowHighlightColor =
      PhGetIntegerSetting(SETTING_THEME_WINDOW_HIGHLIGHT_COLOR);
  PhThemeWindowHighlight2Color =
      PhGetIntegerSetting(SETTING_THEME_WINDOW_HIGHLIGHT2_COLOR);
  PhThemeWindowTextColor = PhGetIntegerSetting(SETTING_THEME_WINDOW_TEXT_COLOR);
  PhEnableThemeAcrylicSupport =
      WindowsVersion >= WINDOWS_11 &&
      !!PhGetIntegerSetting(SETTING_ENABLE_THEME_ACRYLIC_SUPPORT);
  PhEnableThemeAcrylicWindowSupport =
      WindowsVersion >= WINDOWS_11 &&
      !!PhGetIntegerSetting(SETTING_ENABLE_THEME_ACRYLIC_WINDOW_SUPPORT);
  PhEnableThemeNativeButtons =
      !!PhGetIntegerSetting(SETTING_ENABLE_THEME_NATIVE_BUTTONS);
  PhEnableThemeListviewBorder =
      !!PhGetIntegerSetting(SETTING_TREE_LIST_BORDER_ENABLE);
  PhEnableDeferredLayout =
      !!PhGetIntegerSetting(SETTING_ENABLE_DEFERRED_LAYOUT);
  PhEnableServiceNonPoll =
      !!PhGetIntegerSetting(SETTING_ENABLE_SERVICE_NON_POLL);
  PhEnableServiceNonPollNotify =
      !!PhGetIntegerSetting(SETTING_ENABLE_SERVICE_NON_POLL_NOTIFY);
  PhServiceNonPollFlushInterval =
      PhGetIntegerSetting(SETTING_NON_POLL_FLUSH_INTERVAL);
  PhEnableKsiSupport = !!PhGetIntegerSetting(SETTING_KSI_ENABLE) &&
                       !PhStartupParameters.NoKph && !PhIsExecutingInWow64();
  PhEnableKsiWarnings = !!PhGetIntegerSetting(SETTING_KSI_ENABLE_WARNINGS);
  PhFontQuality =
      PhGetFontQualitySetting(PhGetIntegerSetting(SETTING_FONT_QUALITY));
  ...
  if (PhStartupParameters.UpdateChannel != PhInvalidChannel) {
    PhSetIntegerSetting(SETTING_RELEASE_CHANNEL,
                        PhStartupParameters.UpdateChannel);
  }

  if (PhStartupParameters.ShowHidden && !PhNfIconsEnabled()) {
    // HACK(jxy-s) The default used to be that system tray icons where enabled,
    // this keeps the old behavior for automation workflows. If the user
    // specified "-hide" then they want to start the program hidden to the
    // system tray and not show any main window. If there are no system tray
    // icons enabled then we need to enable them so the behavior is consistent.
 ...
 *
 * This function is invoked for each parsed command line option during
 * application startup. It updates the global PhStartupParameters structure and
 * related state based on the provided option and value. Supported options
 * include settings file path, visibility, debug mode, privilege elevation,
 * plugin parameters, process priority, and more.
 *
 * \param Option Pointer to the command line option structure, or NULL for
 * non-option arguments.
 * \param Value Optional value associated with the option, or NULL if not
 * applicable.
 * \param Context Optional context pointer (unused).
 * \return TRUE to continue processing further options, or FALSE to stop.
 */
_Function_class_(PH_COMMAND_LINE_CALLBACK) BOOLEAN NTAPI
    PhpCommandLineOptionCallback(_In_opt_ PCPH_COMMAND_LINE_OPTION Option,
  ...
  if (Option) {
    ...
    case PH_ARG_POINT: {
      ...
      if (Value &&
          PhSplitStringRefAtChar(&Value->sr, L',', &xString, &yString)) {
        LONG64 x;
        LONG64 y;

        if (PhStringToInteger64(&xString, 10, &x) &&
            PhStringToInteger64(&yString, 10, &y)) {
    ...
    case PH_ARG_PLUGIN:
      ...
      if (Value)
        PhAddItemList(PhStartupParameters.PluginParameters,
    ...
    case PH_ARG_SELECTTAB:
      ...
      break;
    case PH_ARG_SYSINFO:
      PhSwapReference(&PhStartupParameters.SysInfo,
                      Value ? Value : PhReferenceEmptyString());
      break;
...
/**
 * Parses and processes the application's startup parameters from the command
 * line.
 *
 * This function defines and processes all supported command line options for
 * System Informer. It updates the global PhStartupParameters structure based on
 * the parsed arguments, enabling or disabling features, setting file paths, and
...
VOID PhpProcessStartupParameters(VOID) {
  ...
  if (!PhParseCommandLine(&commandLine, options, RTL_NUMBER_OF(options),
                          PH_COMMAND_LINE_IGNORE_UNKNOWN_OPTIONS |
                              PH_COMMAND_LINE_IGNORE_FIRST_PART,
                          PhpCommandLineOptionCallback, NULL) ||
      PhStartupParameters.Help) {
    PhShowInformation2(NULL, L"Command line options:", L"%s",
                       L"-debug\n"
                       L"-elevate\n"
                       L"-help\n"
                       L"-hide\n"
  ...
  if (PhStartupParameters.Elevate && !PhGetOwnTokenAttributes().Elevated) {
    PhShellProcessHacker(NULL, NULL, SW_SHOW, PH_SHELL_EXECUTE_ADMIN,
                         PH_SHELL_APP_PROPAGATE_PARAMETERS, 0, NULL);
    PhExitApplication(STATUS_SUCCESS);
  }
}
...
VOID PhpEnablePrivileges(VOID) {
  ...
  if (NT_SUCCESS(PhOpenProcessToken(NtCurrentProcess(), TOKEN_ADJUST_PRIVILEGES,
                                    &tokenHandle))) {
    const LUID_AND_ATTRIBUTES privileges[] = {
        {RtlConvertUlongToLuid(SE_DEBUG_PRIVILEGE), SE_PRIVILEGE_ENABLED},
        {RtlConvertUlongToLuid(SE_INC_BASE_PRIORITY_PRIVILEGE),
         SE_PRIVILEGE_ENABLED},
        {RtlConvertUlongToLuid(SE_INC_WORKING_SET_PRIVILEGE),
         SE_PRIVILEGE_ENABLED},
        {RtlConvertUlongToLuid(SE_LOAD_DRIVER_PRIVILEGE), SE_PRIVILEGE_ENABLED},
        {RtlConvertUlongToLuid(SE_PROF_SINGLE_PROCESS_PRIVILEGE),
         SE_PRIVILEGE_ENABLED},
        ...
        {RtlConvertUlongToLuid(SE_TAKE_OWNERSHIP_PRIVILEGE),
         SE_PRIVILEGE_ENABLED},
        {RtlConvertUlongToLuid(SE_SECURITY_PRIVILEGE), SE_PRIVILEGE_ENABLED},
    };
    UCHAR privilegesBuffer[FIELD_OFFSET(TOKEN_PRIVILEGES, Privileges) +
                           sizeof(privileges)];
    ...
    NtAdjustPrivilegesToken(tokenHandle, FALSE, tokenPrivileges, 0, NULL, NULL);
