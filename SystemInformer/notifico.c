...
VOID PhNfSaveSettings(VOID) {
  ...
  for (ULONG i = 0; i < PhTrayIconItemList->Count; i++) {
    ...
    if (BooleanFlagOn(icon->Flags, PH_NF_ICON_ENABLED)) {
      ...
      if (PhFormatToBuffer(format, RTL_NUMBER_OF(format), buffer,
                           sizeof(buffer), &returnLength)) {
        PhAppendStringBuilderEx(&iconListBuilder, buffer,
                                returnLength - sizeof(UNICODE_NULL));
      } else {
        PhAppendFormatStringBuilder(
            &iconListBuilder, L"%lu|%lu|%s|", icon->SubId, TRUE,
  ...
  if (PhIsNullOrEmptyString(settingsString)) {
    ...
    for (i = 0; i < RTL_NUMBER_OF(PhNfpTrayIconItemGuids); i++) {
      ...
      if (iconGuid = PhFormatGuid(&PhNfpTrayIconItemGuids[i])) {
        PhAppendFormatStringBuilder(&iconListBuilder, L"%s|", iconGuid->Buffer);
    ...
    PhMoveReference(&settingsString,
                    PhFinalStringBuilderString(&iconListBuilder));
...
VOID PhNfCreateIconThreadDelayed(VOID) {
  ...
  if (iconCount && PhBeginInitOnce(&initOnce)) {
    if (NT_SUCCESS(PhCreateEvent(
            &PhpTrayIconEventHandle, EVENT_ALL_ACCESS, SynchronizationEvent,
...
PPH_NF_ICON
PhNfRegisterIcon(_In_opt_ PPH_PLUGIN Plugin, _In_ ULONG Id, _In_ GUID Guid,
                 _In_opt_ PVOID Context, _In_ PCWSTR Text, _In_ ULONG Flags,
...
PPH_NF_ICON
PhNfPluginRegisterIcon(_In_ PPH_PLUGIN Plugin, _In_ ULONG SubId, _In_ GUID Guid,
                       _In_opt_ PVOID Context, _In_ PCWSTR Text,
                       _In_ ULONG Flags,
                       _In_ PPH_NF_ICON_REGISTRATION_DATA RegistrationData) {
  return PhNfRegisterIcon(Plugin, SubId, Guid, Context, Text, Flags,
...
VOID PhNfLoadStage2(VOID) {
  ...
  PhNfTransparencyEnabled =
      !!PhGetIntegerSetting(SETTING_ICON_TRANSPARENCY_ENABLED);
  PhNfPersistTrayIconPositionEnabled =
      !!PhGetIntegerSetting(SETTING_ICON_TRAY_PERSIST_GUID_ENABLED);

  if (PhNfPersistTrayIconPositionEnabled) {
    PhNfLoadGuids();
  }

  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_CPU_USAGE,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_CPU_USAGE], NULL,
                   L"CPU &usage", 0, PhNfpCpuUsageIconUpdateCallback, NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_CPU_HISTORY,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_CPU_HISTORY], NULL,
                   L"CPU &history", 0, PhNfpCpuHistoryIconUpdateCallback, NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_IO_HISTORY,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_IO_HISTORY], NULL,
                   L"&I/O history", 0, PhNfpIoHistoryIconUpdateCallback, NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_COMMIT_HISTORY,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_COMMIT_HISTORY],
                   NULL, L"&Commit charge history", 0,
                   PhNfpCommitHistoryIconUpdateCallback, NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_PHYSICAL_HISTORY,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_PHYSICAL_HISTORY],
                   NULL, L"&Physical memory history", 0,
                   PhNfpPhysicalHistoryIconUpdateCallback, NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_CPU_TEXT,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_CPU_TEXT], NULL,
                   L"CPU usage (text)", 0, PhNfpCpuUsageTextIconUpdateCallback,
                   NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_IO_TEXT,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_IO_TEXT], NULL,
                   L"IO usage (text)", 0, PhNfpIoUsageTextIconUpdateCallback,
                   NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_COMMIT_TEXT,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_COMMIT_TEXT], NULL,
                   L"Commit usage (text)", 0, PhNfpCommitTextIconUpdateCallback,
                   NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_PHYSICAL_TEXT,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_PHYSICAL_TEXT],
                   NULL, L"Physical usage (text)", 0,
                   PhNfpPhysicalUsageTextIconUpdateCallback, NULL);
  PhNfRegisterIcon(NULL, PH_TRAY_ICON_ID_PLAIN_ICON,
                   PhNfpTrayIconItemGuids[PH_TRAY_ICON_GUID_PLAIN_ICON], NULL,
                   L"System Informer icon (static)", 0,
                   PhNfpPlainIconUpdateCallback, NULL);
  ...
  if (PhPluginsEnabled) {
    ...
    PhInvokeCallback(PhGetGeneralCallback(GeneralCallbackTrayIconsInitializing),
                     &pointers);
  }
  ...
  PhRegisterCallback(
      ...
      PhNfpProcessesUpdatedHandler, NULL, &PhNfpProcessesUpdatedRegistration);
}
...
VOID PhNfUninitialization(VOID) {
  ...
  for (ULONG i = 0; i < PhTrayIconItemList->Count; i++) {
    ...
    if (RtlInterlockedClearBits(&icon->Flags, PH_NF_ICON_ENABLED) ==
        PH_NF_ICON_ENABLED) {
  ...
  //     {
  //         NtWaitForSingleObject(PhpTrayIconThreadHandle, FALSE,
  //         PhTimeoutFromMilliseconds(&timeout, 1000));
  //     }
  // #endif
}

VOID PhNfForwardMessage(_In_ HWND WindowHandle, _In_ ULONG_PTR WParam,
  ...
  if (registeredIcon->MessageCallback) {
    if (registeredIcon->MessageCallback(registeredIcon, WParam, LParam,
  ...
  case WM_LBUTTONUP: {
    if (!PhGetIntegerSetting(SETTING_ICON_SINGLE_CLICK) &&
        PhNfMiniInfoEnabled && IconClickUpDueToDown) {
      PH_NF_MSG_SHOWMINIINFOSECTION_DATA showMiniInfoSectionData;

      if (PhNfpGetShowMiniInfoSectionData(iconIndex, registeredIcon,
                                          &showMiniInfoSectionData)) {
        ...
        if (showMiniInfoSectionData.SectionName) {
          IconClickShowMiniInfoSectionData.SectionName =
              PhDuplicateStringZ(showMiniInfoSectionData.SectionName);
        }

        PhSetTimer(WindowHandle, TIMER_ICON_CLICK_ACTIVATE,
                   GetDoubleClickTime() + NFP_ICON_CLICK_ACTIVATE_DELAY,
  ...
  case WM_LBUTTONDBLCLK: {
    if (!PhGetIntegerSetting(SETTING_ICON_SINGLE_CLICK)) {
      if (PhNfMiniInfoEnabled) {
        // We will get another WM_LBUTTONUP message corresponding to the
        // double-click, and we need to make sure that it doesn't start the
        // activation timer again.
        PhKillTimer(WindowHandle, TIMER_ICON_CLICK_ACTIVATE);
  ...
  case NIN_KEYSELECT: {
    // HACK: explorer seems to send two NIN_KEYSELECT messages when the user
    // selects the icon and presses ENTER.
  ...
  case NIN_POPUPOPEN: {
    if (WindowsVersion >= WINDOWS_11_22H2) {
      // NIN_POPUPOPEN is sent when the user hovers the cursor over an icon BUT
      // Windows 11 either blocks the notification or ignores the hover time and
      // displays the popup instantly. We try and workaround the missing hover
      // time by using a timer to delay the popup for 1 second. If we get a
      // NIN_POPUPCLOSE then cancel the timer and the popup. Note: We only
      // workaround the missing hover time not the blocked/missing NIN_POPUPOPEN
      // notifications. If we want to workaround the broken NIN_POPUPOPEN
      // notifications on Win11 the tray icons also send WM_MOUSEMOSE and before
      // NIN_POPUPOPEN existed XP applications would compare the cursor position
      // in a timer callback to show or hide the popup. (dmex)

      PopupIconIndex = iconIndex;
      PopupRegisteredIcon = registeredIcon;

      PhSetTimer(PhMainWndHandle, TIMER_ICON_POPUPOPEN,
                 NFP_ICON_RESTORE_HOVER_DELAY,
                 PhNfpIconShowPopupHoverTimerProc);
    } else {
      PH_NF_MSG_SHOWMINIINFOSECTION_DATA showMiniInfoSectionData;
      POINT location;

      if (PhNfMiniInfoEnabled && !IconDisableHover &&
          PhNfpGetShowMiniInfoSectionData(iconIndex, registeredIcon,
                                          &showMiniInfoSectionData)) {
        GetCursorPos(&location);
        PhPinMiniInformation(MiniInfoIconPinType, 1, 0,
                             PH_MINIINFO_DONT_CHANGE_SECTION_IF_PINNED,
...
VOID PhNfSetVisibleIcon(_In_ PPH_NF_ICON Icon, _In_ BOOLEAN Visible) {
  if (Visible) {
    RtlInterlockedSetBits(&Icon->Flags, PH_NF_ICON_ENABLED);
    ...
    RtlInterlockedPushEntrySList(&PhpTrayIconWorkQueueListHead,
                                 &data->ListEntry);
#endif
  } else {
...
VOID NTAPI PhpToastCallback(_In_ HRESULT Result, _In_ PH_TOAST_REASON Reason,
                            _In_ PVOID Context) {
  if (Reason == PhToastReasonActivated)
    PhShowDetailsForIconNotification();
}

HRESULT PhpShowToastNotification(_In_ PPH_STRING Title, _In_ PPH_STRING Text,
                                 _In_ ULONG Timeout,
  ...
  if (PhBeginInitOnce(&initOnce)) {
    iconFileName =
        PhGetApplicationDirectoryFileNameZ(L"Resources\\icon.png", FALSE);
  ...
  if (HR_SUCCESS(
          PhAppResolverGetAppIdForProcess(NtCurrentProcessId(), &processAppId)))
    iconAppId = processAppId->sr;
  else {
    if (HR_SUCCESS(
            PhAppResolverGetAppIdForWindow(PhMainWndHandle, &processAppId)))
      iconAppId = processAppId->sr;
    else {
  ...
  PhInitFormatS(&format[0],
                L"<toast><visual><binding "
                L"template=\"ToastImageAndText02\"><image id=\"1\" src=\"");
  ...
  result = PhShowToastStringRef(&iconAppId, &toastXml->sr, Timeout * 1000,
                                ToastCallback, Context);
...
BOOLEAN PhNfpShowBalloonTip(_In_ PCWSTR Title, _In_ PCWSTR Text,
                            _In_ ULONG Timeout) {
  ...
  wcsncpy_s(notifyIcon.szInfoTitle, RTL_NUMBER_OF(notifyIcon.szInfoTitle),
            Title, _TRUNCATE);
  wcsncpy_s(notifyIcon.szInfo, RTL_NUMBER_OF(notifyIcon.szInfo), Text,
            _TRUNCATE);
  notifyIcon.uTimeout = Timeout;

  if (PhGetIntegerSetting(SETTING_ICON_BALLOON_SHOW_ICON) ||
...
BOOLEAN PhNfShowBalloonTip(_In_ PCWSTR Title, _In_ PCWSTR Text,
                           _In_ ULONG Timeout) {
  if (!PhNfIconsEnabled()) {
    return FALSE;
  }
...
HRESULT PhNfShowBalloonTipEx(_In_ PCWSTR Title, _In_ PCWSTR Text,
                             _In_ ULONG Timeout,
                             _In_opt_ PPH_TOAST_CALLBACK ToastCallback,
                             _In_opt_ PVOID Context) {
  ...
  return PhpShowToastNotification(BalloonTitle, BalloonText, Timeout,
                                  ToastCallback, Context, TRUE);
}
...
HICON PhNfBitmapToIcon(_In_ HBITMAP Bitmap) {
  ...
  // Create a monochrome mask bitmap for the icon.
  {
    ...
    if (!(mask = CreateBitmap(bitmapInfo.bmWidth, bitmapInfo.bmHeight, 1, 1,
                              NULL)))
      return NULL;
  }
...
PPH_NF_ICON PhNfGetIconById(_In_ ULONG SubId) {
  for (ULONG i = 0; i < PhTrayIconItemList->Count; i++) {
    PPH_NF_ICON icon = PhTrayIconItemList->Items[i];
...
VOID PhNfNotifyMiniInfoPinned(_In_ BOOLEAN Pinned) {
  if (PhNfMiniInfoPinned != Pinned) {
    ...
    // Go through every icon and set/clear the NIF_SHOWTIP flag depending on
    // whether the mini info window is pinned. If it's pinned then we want to
    // show normal tooltips, because the section doesn't change automatically
    // when the cursor hovers over an icon.
...
HICON PhNfGetApplicationIcon(_In_opt_ LONG DpiValue) {
...
HICON PhNfpGetBlackIcon(VOID) {
  if (!PhNfpBlackIcon) {
    ...
    PhNfpBeginBitmap2(&PhNfpBlackBitmapContext, &width, &height,
                      &PhNfpBlackBitmap, &bits, &hdc, &oldBitmap);
    memset(bits, PhNfTransparencyEnabled ? 1 : 0,
...
VOID PhDrawTrayIconText(_In_ HDC hdc, _In_ PVOID Bits,
  ...
  if (DrawInfo->BackColor == 0) {
    ...
    PhFillMemoryUlong(bits, COLORREF_TO_BITS(DrawInfo->BackColor),
                      numberOfPixels);
  }
  ...
  DrawInfo->Text = *Text;
  GetTextExtentPoint32(hdc, Text->Buffer, (ULONG)Text->Length / sizeof(WCHAR),
                       &textSize);
  ...
  boxRectangle.Left =
      (DrawInfo->Width - boxRectangle.Width) / (LONG)sizeof(WCHAR);
  boxRectangle.Top =
      (DrawInfo->Height - boxRectangle.Height) / (LONG)sizeof(WCHAR);
  ...
  DrawText(hdc, DrawInfo->Text.Buffer,
           (ULONG)DrawInfo->Text.Length / sizeof(WCHAR), &DrawInfo->TextRect,
...
HFONT PhNfGetTrayIconFont(_In_opt_ LONG DpiValue) {
  ...
  if (!PhNfTrayIconFont) {
    ...
    PhNfTrayIconFont =
        CreateFont(PhGetDpi(-11, DpiValue), 0, 0, 0, FW_NORMAL, FALSE, FALSE,
...
BOOLEAN PhNfpAddNotifyIcon(_In_ PPH_NF_ICON Icon) {
  NOTIFYICONDATA notifyIcon;
  ...
  wcsncpy_s(notifyIcon.szTip, RTL_NUMBER_OF(notifyIcon.szTip),
  ...
  if (!PhNfMiniInfoEnabled || PhNfMiniInfoPinned ||
      FlagOn(Icon->Flags, PH_NF_ICON_NOSHOW_MINIINFO))
...
BOOLEAN PhNfpRemoveNotifyIcon(_In_ PPH_NF_ICON Icon) {
  NOTIFYICONDATA notifyIcon;
...
BOOLEAN PhNfpModifyNotifyIcon(_In_ PPH_NF_ICON Icon, _In_ ULONG Flags,
  ...
  if (FlagOn(Flags, NIF_TIP)) {
    ...
    wcsncpy_s(notifyIcon.szTip, RTL_NUMBER_OF(notifyIcon.szTip),
              PhGetStringOrDefault(Text, PhApplicationName), _TRUNCATE);
  }

  if (!PhShellNotifyIcon(NIM_MODIFY, &notifyIcon)) {
    // Explorer probably died and we lost our icon. Try to add the icon, and try
    // again.
    PhNfpAddNotifyIcon(Icon);
    PhShellNotifyIcon(NIM_MODIFY, &notifyIcon);
  }
...
//     if (PhMainWndExiting)
//         return FALSE;
//     if ((icon = PhNfGetIconById(Id)) && (icon->Flags &
//     PH_NF_ICON_UNAVAILABLE))
//         return FALSE;
//
...
#ifdef PH_NF_ENABLE_WORKQUEUE
VOID PhNfTrayIconFlushWorkQueueData(VOID) {
  PSLIST_ENTRY entry;
  PPH_NF_WORKQUEUE_DATA data;
  ...
  while (entry) {
    ...
    if (data->ShowBalloon) {
      if (!HR_SUCCESS(PhpShowToastNotification(
              data->BalloonTitle, data->BalloonText, data->BalloonTimeout,
              PhpToastCallback, NULL, FALSE))) {
        PhNfpShowBalloonTip(PhGetString(data->BalloonTitle),
...
_Function_class_(USER_THREAD_START_ROUTINE) NTSTATUS
    PhNfpTrayIconUpdateThread(_In_opt_ PVOID Context) {
  ...
  while (TRUE) {
    ...
    if (NT_SUCCESS(
            NtWaitForSingleObject(PhpTrayIconEventHandle, FALSE, NULL))) {
#ifdef PH_NF_ENABLE_WORKQUEUE
...
_Function_class_(PH_CALLBACK_FUNCTION) VOID
    PhNfpProcessesUpdatedHandler(_In_opt_ PVOID Parameter,
                                 _In_opt_ PVOID Context) {
...
VOID PhNfpUpdateRegisteredIcon(_In_ PPH_NF_ICON Icon) {
  ...
  Icon->UpdateCallback(Icon, &newIconOrBitmap, &updateFlags, &newText,
                       Icon->Context);
...
_Function_class_(PH_NF_BEGIN_BITMAP) VOID
    PhNfpBeginBitmap(_Out_ PULONG Width, _Out_ PULONG Height,
                     _Out_ HBITMAP *Bitmap, _Out_opt_ PVOID *Bits,
                     _Out_ HDC *Hdc, _Out_ HBITMAP *OldBitmap) {
  PhNfpBeginBitmap2(&PhNfpDefaultBitmapContext, Width, Height, Bitmap, Bits,
                    Hdc, OldBitmap);
}

_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpBeginBitmap2(_Inout_ PPH_NF_BITMAP Context, _Out_ PULONG Width,
                      _Out_ PULONG Height, _Out_ HBITMAP *Bitmap,
                      _Out_opt_ PVOID *Bits, _Out_ HDC *Hdc,
  ...
  if (Context->TaskbarDpi == 0 || Context->TaskbarDpi != dpiValue) {
    ...
    // Re-initialize fonts with updated DPI (only when there's an existing
    // handle). (dmex)
    // PhNfGetTrayIconFont(dpiValue);
    // PhNfGetApplicationIcon(dpiValue);
  }

  // Cleanup existing resources when the DPI changes so we're able to
  // re-initialize with updated DPI. (dmex)
  ...
  if (!Context->Initialized) {
    ...
    Context->Bitmap = CreateDIBSection(Context->Hdc, &bitmapInfo,
                                       DIB_RGB_COLORS, &Context->Bits, NULL, 0);
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpCpuHistoryIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                      _Out_ PVOID *NewIconOrBitmap,
                                      _Out_ PULONG Flags,
                                      _Out_ PPH_STRING *NewText,
                                      _In_opt_ PVOID Context) {
  static PH_GRAPH_DRAW_INFO drawInfo = {16,
                                        16,
                                        PH_GRAPH_USE_LINE_2,
                                        2,
                                        RGB(0x00, 0x00, 0x00),
  ...
  Icon->Pointers->BeginBitmap(&drawInfo.Width, &drawInfo.Height, &bitmap, &bits,
                              &hdc, &oldBitmap);
  ...
  *Flags = PH_NF_UPDATE_IS_BITMAP;
  ...
  if (PhMaxCpuHistory.Count != 0)
    maxCpuProcessId =
        UlongToHandle(PhGetItemCircularBuffer_ULONG(&PhMaxCpuHistory, 0));
  else
  ...
  PhInitFormatF(&format[1], (PhCpuKernelUsage + PhCpuUserUsage) * 100,
                PhMaxPrecisionUnit);
  ...
  if (maxCpuProcessItem) {
    ...
    PhInitFormatF(&format[6], maxCpuProcessItem->CpuUsage * 100,
                  PhMaxPrecisionUnit);
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpIoHistoryIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                     _Out_ PVOID *NewIconOrBitmap,
                                     _Out_ PULONG Flags,
                                     _Out_ PPH_STRING *NewText,
                                     _In_opt_ PVOID Context) {
  ...
  for (i = 0; i < lineDataCount; i++) {
    lineData1[i] = (FLOAT)PhGetItemCircularBuffer_ULONG64(&PhIoReadHistory, i) +
                   (FLOAT)PhGetItemCircularBuffer_ULONG64(&PhIoOtherHistory, i);
    lineData2[i] = (FLOAT)PhGetItemCircularBuffer_ULONG64(&PhIoWriteHistory, i);
  ...
  *Flags = PH_NF_UPDATE_IS_BITMAP;
  ...
  if (PhMaxIoHistory.Count != 0)
    maxIoProcessId =
        UlongToHandle(PhGetItemCircularBuffer_ULONG(&PhMaxIoHistory, 0));
  else
    maxIoProcessId = NULL;
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpCommitHistoryIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                         _Out_ PVOID *NewIconOrBitmap,
                                         _Out_ PULONG Flags,
                                         _Out_ PPH_STRING *NewText,
                                         _In_opt_ PVOID Context) {
  static PH_GRAPH_DRAW_INFO drawInfo = {
      16, 16, 0, 2, RGB(0x00, 0x00, 0x00), 16, NULL, NULL, 0, 0, 0, 0};
  ULONG maxDataCount;
  ULONG lineDataCount;
  PFLOAT lineData1;
  ULONG i;
  ...
  PhDivideSinglesBySingle(lineData1, (FLOAT)PhPerfInformation.CommitLimit,
                          lineDataCount);
  ...
  *Flags = PH_NF_UPDATE_IS_BITMAP;
  ...
  commitFraction = (FLOAT)PhPerfInformation.CommittedPages /
                   (FLOAT)PhPerfInformation.CommitLimit;

  PhInitFormatS(&format[0], L"Commit: ");
  PhInitFormatSize(&format[1],
                   UInt32x32To64(PhPerfInformation.CommittedPages, PAGE_SIZE));
  PhInitFormatS(&format[2], L" (");
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpPhysicalHistoryIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                           _Out_ PVOID *NewIconOrBitmap,
                                           _Out_ PULONG Flags,
                                           _Out_ PPH_STRING *NewText,
                                           _In_opt_ PVOID Context) {
  ...
  PhDivideSinglesBySingle(lineData1,
  ...
  *Flags = PH_NF_UPDATE_IS_BITMAP;
  ...
  physicalUsage = PhSystemBasicInformation.NumberOfPhysicalPages -
                  PhPerfInformation.AvailablePages;
  physicalFraction = (FLOAT)physicalUsage /
                     (FLOAT)PhSystemBasicInformation.NumberOfPhysicalPages;
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpCpuUsageIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                    _Out_ PVOID *NewIconOrBitmap,
                                    _Out_ PULONG Flags,
                                    _Out_ PPH_STRING *NewText,
                                    _In_opt_ PVOID Context) {
  ...
  Icon->Pointers->BeginBitmap(&width, &height, &bitmap, &bits, &hdc,
                              &oldBitmap);
  ...
  *Flags = PH_NF_UPDATE_IS_BITMAP;
  ...
  if (maxCpuProcessId) {
    if (maxCpuProcessItem = PhReferenceProcessItem(maxCpuProcessId)) {
      ...
      PhInitFormatS(&format[2], L": ");
      PhInitFormatF(&format[3], maxCpuProcessItem->CpuUsage * 100.f,
                    PhMaxPrecisionUnit);
  ...
  if (maxCpuText)
    PhInitFormatSR(&format[3], maxCpuText->sr);
  else
    PhInitFormatC(&format[3], L' ');

  *NewText = PhFormat(format, 4, 0);
  //*NewText = PhFormatString(L"CPU usage: %.2f%%%s", (PhCpuKernelUsage +
  //PhCpuUserUsage) * 100, PhGetStringOrEmpty(maxCpuText));
  if (maxCpuText)
    PhDereferenceObject(maxCpuText);
}

// Text icons

_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpCpuUsageTextIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                        _Out_ PVOID *NewIconOrBitmap,
                                        _Out_ PULONG Flags,
                                        _Out_ PPH_STRING *NewText,
                                        _In_opt_ PVOID Context) {
  ...
  *Flags = PH_NF_UPDATE_IS_BITMAP;
  ...
  PhInitFormatS(&format[0], L"CPU usage: ");
  PhInitFormatF(&format[1], (PhCpuKernelUsage + PhCpuUserUsage) * 100.f,
                PhMaxPrecisionUnit);
  PhInitFormatC(&format[2], L'%');
  if (maxCpuText)
    PhInitFormatSR(&format[3], maxCpuText->sr);
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpIoUsageTextIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                       _Out_ PVOID *NewIconOrBitmap,
                                       _Out_ PULONG Flags,
                                       _Out_ PPH_STRING *NewText,
                                       _In_opt_ PVOID Context) {
  static PH_GRAPH_DRAW_INFO drawInfo = {
      16, 16, 0, 0, RGB(0x00, 0x00, 0x00), 0, NULL, NULL, 0, 0, 0, 0};
  HBITMAP bitmap;
  PVOID bits;
  HDC hdc;
  HBITMAP oldBitmap;
  ...
  SIZE_T returnLength;
  WCHAR buffer[PH_INT64_STR_LEN_1];
  static ULONG64 maxValue =
      UInt32x32To64(100000, 1024); // minimum scaling of 100 MB.
  ...
  if (maxValue <
      (PhIoReadDelta.Delta + PhIoWriteDelta.Delta + PhIoOtherDelta.Delta))
    maxValue =
        (PhIoReadDelta.Delta + PhIoWriteDelta.Delta + PhIoOtherDelta.Delta);
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpCommitTextIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                      _Out_ PVOID *NewIconOrBitmap,
                                      _Out_ PULONG Flags,
                                      _Out_ PPH_STRING *NewText,
                                      _In_opt_ PVOID Context) {
  ...
  commitFraction = (FLOAT)PhPerfInformation.CommittedPages /
                   PhPerfInformation.CommitLimit * 100.f;
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpPhysicalUsageTextIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                             _Out_ PVOID *NewIconOrBitmap,
                                             _Out_ PULONG Flags,
                                             _Out_ PPH_STRING *NewText,
                                             _In_opt_ PVOID Context) {
  ...
  physicalUsage = PhSystemBasicInformation.NumberOfPhysicalPages -
                  PhPerfInformation.AvailablePages;
  physicalFraction =
      (FLOAT)physicalUsage / PhSystemBasicInformation.NumberOfPhysicalPages;
...
_Function_class_(PH_NF_ICON_UPDATE_CALLBACK) VOID
    PhNfpPlainIconUpdateCallback(_In_ PPH_NF_ICON Icon,
                                 _Out_ PVOID *NewIconOrBitmap,
                                 _Out_ PULONG Flags, _Out_ PPH_STRING *NewText,
                                 _In_opt_ PVOID Context) {
  static PPH_STRING string = NULL;

  if (!string) {
    PH_STRINGREF text = PH_STRINGREF_INIT(L"sPicyaDham");
...
_Success_(return) BOOLEAN PhNfpGetShowMiniInfoSectionData(
    _In_ ULONG IconIndex, _In_ PPH_NF_ICON RegisteredIcon,
...
VOID PhNfpIconClickActivateTimerProc(_In_ HWND hwnd, _In_ UINT uMsg,
                                     _In_ UINT_PTR idEvent, _In_ ULONG dwTime) {
  PhPinMiniInformation(
      MiniInfoActivePinType, 1, 0,
...
VOID PhNfpDisableHover(VOID) {
  IconDisableHover = TRUE;
  PhSetTimer(PhMainWndHandle, TIMER_ICON_RESTORE_HOVER,
             NFP_ICON_RESTORE_HOVER_DELAY, PhNfpIconRestoreHoverTimerProc);
}

VOID PhNfpIconRestoreHoverTimerProc(_In_ HWND hwnd, _In_ UINT uMsg,
                                    _In_ UINT_PTR idEvent, _In_ ULONG dwTime) {
  IconDisableHover = FALSE;
  PhKillTimer(PhMainWndHandle, TIMER_ICON_RESTORE_HOVER);
}

VOID PhNfpIconDisablePopupHoverWin11Workaround(VOID) {
  if (WindowsVersion >= WINDOWS_11_22H2) {
...
VOID PhNfpIconShowPopupHoverTimerProc(_In_ HWND hwnd, _In_ UINT uMsg,
                                      _In_ UINT_PTR idEvent,
                                      _In_ ULONG dwTime) {
  PH_NF_MSG_SHOWMINIINFOSECTION_DATA showMiniInfoSectionData;
  POINT location;

  if (PhNfMiniInfoEnabled && !IconDisableHover && PopupIconIndex != ULONG_MAX &&
      PopupRegisteredIcon &&
      PhNfpGetShowMiniInfoSectionData(PopupIconIndex, PopupRegisteredIcon,
                                      &showMiniInfoSectionData)) {
    GetCursorPos(&location);
    PhPinMiniInformation(MiniInfoIconPinType, 1, 0,
                         PH_MINIINFO_DONT_CHANGE_SECTION_IF_PINNED,
