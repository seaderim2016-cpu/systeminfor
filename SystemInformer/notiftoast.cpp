...
#define RETURN_IF_FAILED(_HR_)                                                 \
  do {                                                                         \
    const auto __hr = _HR_;                                                    \
    if (HR_FAILED(__hr)) {                                                     \
      return __hr;                                                             \
    }                                                                          \
  } while (false)
...
*/
template <typename T>
bool GetModuleProcAddress(_Out_ T *FunctionPointer, _In_ const wchar_t *Module,
...
/*!
    @brief Wrapper around RoGetActivationFactory to make it a little less clunky
   to use.

    @details PhActivateToastRuntime must be called prior to this else the
   function returns R_NOTIMPL.
    ...
    @param[in] ActivatableClassId - Activatable class ID to look up the
   interface from.
...
_Must_inspect_result_ HRESULT RoGetActivationFactory(
    _In_ wchar_t const (&DllName)[t_SizeDestName],
    _In_ wchar_t const (&ActivatableClassId)[t_SizeDestClass],
    _COM_Outptr_ I **Interface) {
  HRESULT hr = PhGetActivationFactory(DllName, ActivatableClassId, __uuidof(I),
...
using IToastActivatedHandler =
    ITypedEventHandler<ToastNotification *, IInspectable *>;
using IToastDismissedHandler =
    ITypedEventHandler<ToastNotification *, ToastDismissedEventArgs *>;
using IToastFailedHandler =
...
*/
class ToastEventHandler final
    : public RuntimeClass<RuntimeClassFlags<ClassicCom>, IToastActivatedHandler,
                          IToastDismissedHandler, IToastFailedHandler> {
public:
  HRESULT RuntimeClassInitialize(PPH_TOAST_CALLBACK ToastCallback,
                                 PVOID Context) {
  ...
  HRESULT STDMETHODCALLTYPE Invoke(_In_ IToastNotification *Sender,
                                   _In_ IInspectable *Args) override;

  HRESULT STDMETHODCALLTYPE
  Invoke(_In_ IToastNotification *Sender,
         _In_ IToastDismissedEventArgs *Args) override;

  HRESULT STDMETHODCALLTYPE Invoke(_In_ IToastNotification *Sender,
                                   _In_ IToastFailedEventArgs *Args) override;
...
public:
  ...
  Initialize(_In_ PPH_STRINGREF ApplicationId, _In_ PPH_STRINGREF ToastXml,
             _In_opt_ ULONG TimeoutMilliseconds,
             _In_opt_ PPH_TOAST_CALLBACK ToastCallback, _In_opt_ PVOID Context);
...
_Must_inspect_result_ HRESULT PH::Toast::Initialize(
    _In_ PPH_STRINGREF ApplicationId, _In_ PPH_STRINGREF ToastXml,
    _In_opt_ ULONG TimeoutMilliseconds,
    _In_opt_ PPH_TOAST_CALLBACK ToastCallback, _In_opt_ PVOID Context) {
  ...
  RETURN_IF_FAILED(PH::RoGetActivationFactory<IToastNotificationManagerStatics>(
      L"WpnApps.dll", L"Windows.UI.Notifications.ToastNotificationManager",
      &manager));

  ComPtr<IToastNotificationFactory> factory;
  RETURN_IF_FAILED(PH::RoGetActivationFactory<IToastNotificationFactory>(
      L"WpnApps.dll", L"Windows.UI.Notifications.ToastNotification", &factory));

  ComPtr<IToastNotifier> notifier;
  RETURN_IF_FAILED(
      PhCreateWindowsRuntimeStringRef(ApplicationId, &stringAppIdRef));
  RETURN_IF_FAILED(manager->CreateToastNotifierWithId(
      HSTRING_FROM_STRING(stringAppIdRef), &notifier));
  ...
  RETURN_IF_FAILED(manager->GetTemplateContent(
      ToastTemplateType::ToastTemplateType_ToastImageAndText02, &xmlDocument));
  ...
  //
  RETURN_IF_FAILED(
      PhCreateWindowsRuntimeStringRef(ToastXml, &stringToastXmlRef));
  ...
  if (TimeoutMilliseconds > 0) {
    ...
    RETURN_IF_FAILED(PH::RoGetActivationFactory<IPropertyValueStatics>(
        L"WinTypes.dll", L"Windows.Foundation.PropertyValue", &propertyStats));
...
HRESULT STDMETHODCALLTYPE PH::ToastEventHandler::Invoke(
    _In_ IToastNotification *Sender, _In_ IToastDismissedEventArgs *Args) {
...
HRESULT STDMETHODCALLTYPE PH::ToastEventHandler::Invoke(
    _In_ IToastNotification *Sender, _In_ IInspectable *Args) {
...
HRESULT STDMETHODCALLTYPE PH::ToastEventHandler::Invoke(
    _In_ IToastNotification *Sender, _In_ IToastFailedEventArgs *Args) {
  HRESULT err;
...
/*!
    ...
    @details Each successful call to PhInitializeToastRuntime should be paired
   with a call to PhUninitializeToastRuntime (including S_FALSE). This function
   must be called prior to a call to PhShowToast. Generally, this should be
   called once during start of the process.

    @return Appropriate result status. On success (including S_FALSE) this call
   must be paired with a call to PhUninitializeToastRuntime.
*/
_Must_inspect_result_ HRESULT PhInitializeToastRuntime() {
  //    HRESULT res;
  //
...
/*!
    @brief This is a wrapper to call RoUninitialize. This should only be called
   when PhInitializeToastRuntime return a success (including S_FALSE).
*/
...
/*!
    ...
    @details PhInitializeToastRuntime must be called prior. This is an
   asynchronous operation. If the caller supplies a context and the function
   returns a success, the context is owned by the toast object, any allocated
   memory should be freed when the toast callback is invoked. On failure, the
   caller should free any memory allocated for the toast callback context,
   callback will not be invoked. Note, if you ToastXml is valid but contains
   something like an image that can't be located by the runtime this function
   will still return success, don't write . See comments in
   PH::Toast::Initialize.
    ...
    @code
            hr = PhShowToast(L"System Informer",
                             ...
                             L"    <visual>"
                             L"       <binding
   template=\"ToastImageAndText02\">" L"            <image id=\"1\"
   src=\".\\ToastImage.png\" alt=\"red graphic\"/>" L"            <text
   id=\"1\">Example Title</text>" L"            <text id=\"2\">This is an
   example toast notification, hello world!</text>" L"        </binding>" L"
   </visual>" L"</toast>", 30 * 1000, ToastCallback, NULL);
    @endcode

    @param[in] ApplicationId - Name for the application showing the toast.
    @param[in] ToastXml - XAML formatted \<toast\> to display.
    @param[in] TimeoutMilliseconds - Optional, when elapsed the toast is
   considered timed out. If 0 is passed the default system defined timeout for
   toast notifications is used..
    @param[in] ToastCallback - Optional, if supplied the toast registers
   handlers for toast interaction and the callback is invoked with the relevant
   PH_TOAST_REASON.
    @param[in] Context - Option, context passed to the toast notification
   callback.

    @return Appropriate result status. On S_OK the toast is dispatched and the
   callback will be invoked when the interaction occurs. On failure the callback
   will not be invoked the caller should free any context.
*/
_Must_inspect_result_ HRESULT PhShowToastStringRef(
    _In_ PPH_STRINGREF ApplicationId, _In_ PPH_STRINGREF ToastXml,
    _In_opt_ ULONG TimeoutMilliseconds,
    _In_opt_ PPH_TOAST_CALLBACK ToastCallback, _In_opt_ PVOID Context) {
  auto toast = std::make_unique<PH::Toast>();

  RETURN_IF_FAILED(toast->Initialize(
      ApplicationId, ToastXml, TimeoutMilliseconds, ToastCallback, Context));
  return toast->Show();
}

_Must_inspect_result_ HRESULT
PhShowToast(_In_ PCWSTR ApplicationId, _In_ PCWSTR ToastXml,
            _In_opt_ ULONG TimeoutMilliseconds,
            _In_opt_ PPH_TOAST_CALLBACK ToastCallback, _In_opt_ PVOID Context) {
  ...
  return PhShowToastStringRef(&applicationIdStringRef, &toastXmlStringRef,
                              TimeoutMilliseconds, ToastCallback, Context);
}
