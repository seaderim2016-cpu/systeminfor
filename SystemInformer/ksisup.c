...
VOID KsiShowKernelSupportCheckDialog(_In_opt_ HWND WindowHandle);
#endif
...
extern // ksidbg.c
    VOID KsiDebugLogInitialize(VOID);

extern // ksidbg.c
    VOID KsiDebugLogFinalize(VOID);
#endif
 ...
 */
PCWSTR KsiGetWindowsVersionString(VOID) {
  ...
  static_assert(WINDOWS_MAX == WINDOWS_11_26H1,
                "KsiGetWindowsVersionString must include all versions");
 ...
 */
PPH_STRING KsiGetWindowsBuildString(VOID) {
  PH_FORMAT format[5];
...
PPH_STRING KsiGetKernelVersionString(VOID) {
  ...
  if (PhBeginInitOnce(&initOnce)) {
    ...
    if (fileName = KsiGetKernelFileName()) {
      if (NT_SUCCESS(PhInitializeImageVersionInfoEx(&versionInfo, &fileName->sr,
                                                    FALSE))) {
...
/**
 * Populate `SupportData` with kernel image attributes useful for dynamic
 ...
 */
VOID KsiGetKernelSupportData(_Out_ PKSI_SUPPORT_DATA SupportData) {
  static PH_INITONCE initOnce = PH_INITONCE_INIT;
  ...
  if (PhBeginInitOnce(&initOnce)) {
    ...
    if (fileName = KsiGetKernelFileName()) {
      ...
      if (NT_SUCCESS(PhLoadMappedImageHeaderPageSize(&fileName->sr, NULL,
                                                     &mappedImage))) {
        KsiSupportData.Machine = mappedImage.NtHeaders->FileHeader.Machine;
        KsiSupportData.TimeDateStamp =
            mappedImage.NtHeaders->FileHeader.TimeDateStamp;
        KsiSupportData.SizeOfImage =
            mappedImage.NtHeaders->OptionalHeader.SizeOfImage;
...
/**
 * Get a formatted support string that uniquely identifies the kernel image for
 ...
 */
PPH_STRING KsiGetKernelSupportString(VOID) {
  static PH_INITONCE initOnce = PH_INITONCE_INIT;
  ...
  if (PhBeginInitOnce(&initOnce)) {
    ...
    KsiSupportString = PhFormatString(
        L"%02x-%04x-%08x-%08x", (BYTE)supportData.Class, supportData.Machine,
        supportData.TimeDateStamp, supportData.SizeOfImage);
...
VOID PhShowKsiStatus(VOID) {
  ...
  if ((processState != 0) &&
    ...
    if (!BooleanFlagOn(processState, KPH_PROCESS_VERIFIED_PROCESS)) {
      PhAppendStringBuilder2(&stringBuilder,
                             L"    - unverified primary image\r\n");
    }
    ...
    if (!BooleanFlagOn(processState, KPH_PROCESS_NO_UNTRUSTED_IMAGES)) {
      PhAppendStringBuilder2(
          &stringBuilder,
          L"    - unsigned images (likely an unsigned plugin)\r\n");
    }
    if (!BooleanFlagOn(processState, KPH_PROCESS_NOT_BEING_DEBUGGED)) {
      PhAppendStringBuilder2(&stringBuilder,
                             L"    - process is being debugged\r\n");
    }
    if ((processState & KPH_PROCESS_STATE_MINIMUM) !=
        KPH_PROCESS_STATE_MINIMUM) {
      PhAppendStringBuilder2(&stringBuilder,
                             L"    - tampered primary image\r\n");
    }
    ...
    PhAppendStringBuilder2(
        &stringBuilder,
        L"You will be unable to use more advanced features, view details about "
        L"system processes or terminate malicious software.");
    infoString = PhFinalStringBuilderString(&stringBuilder);

    PhShowKsiMessageEx(NULL, TD_SHIELD_ERROR_ICON, 0, FALSE,
                       L"Access to the kernel driver is restricted.", L"%s",
 ...
 */
PPH_STRING PhpGetKsiMessage(_In_opt_ NTSTATUS Status, _In_ BOOLEAN Force,
                            _In_ PCWSTR Format, _In_ va_list ArgPtr) {
  ...
  if (Status != 0) {
    ...
    if (errorMessage) {
      ...
      while (
          PhSplitStringRefAtChar(&secondPart, L'%', &firstPart, &secondPart)) {
  ...
  PhAppendStringBuilder2(&stringBuilder,
                         PhGetStringOrDefault(kernelVersion, L"Unknown"));
  PhAppendStringBuilder2(&stringBuilder, L"\r\n");
...
#if defined(KSI_ONLINE_PLATFORM_SUPPORT)
PPH_STRING PhpGetKsiMessage2(_In_opt_ NTSTATUS Status, _In_ BOOLEAN Force,
                             _In_ PCWSTR Format, _In_ va_list ArgPtr) {
  PPH_STRING buildString;
  PPH_STRING versionString;
  PPH_STRING kernelVersion;
 ...
 */
LONG PhpShowKsiMessage(_In_opt_ HWND WindowHandle, _In_ ULONG Buttons,
                       _In_opt_ PCWSTR Icon, _In_opt_ NTSTATUS Status,
                       _In_ BOOLEAN Force, _In_ PCWSTR Title,
                       _In_ PCWSTR Format, _In_ va_list ArgPtr) {
  LONG result;
  PPH_STRING errorMessage;
  ...
  if (Force) {
    result = PhShowMessage2(WindowHandle, Buttons, Icon, Title,
    ...
    if (Status != STATUS_SUCCESS) {
      ...
      bufferLength =
          _snwprintf(buffer, ARRAYSIZE(buffer) - sizeof(UNICODE_NULL),
                     L"%s (0x%08x)", Title, Status);
      buffer[bufferLength] = UNICODE_NULL;

      result = PhShowMessageOneTime2(WindowHandle, Buttons, Icon, buffer,
                                     &checked, PhGetString(errorMessage));
    } else {
      result = PhShowMessageOneTime2(WindowHandle, Buttons, Icon, Title,
                                     &checked, PhGetString(errorMessage));
    }
 ...
 */
PPH_STRING PhGetKsiMessage(_In_opt_ NTSTATUS Status, _In_ BOOLEAN Force,
                           _In_ PCWSTR Format, ...) {
  PPH_STRING message;
  va_list argptr;
 ...
 *
 * Calls PhpShowKsiMessage with variable argument list and returns dialog
 * result.
 */
LONG PhShowKsiMessage2(_In_opt_ HWND WindowHandle, _In_ ULONG Buttons,
                       _In_opt_ PCWSTR Icon, _In_opt_ NTSTATUS Status,
                       _In_ BOOLEAN Force, _In_ PCWSTR Title,
                       _In_ PCWSTR Format, ...) {
  LONG result;
  ...
  va_start(argptr, Format);
  result = PhpShowKsiMessage(WindowHandle, Buttons, Icon, Status, Force, Title,
                             Format, argptr);
  va_end(argptr);
 ...
 */
VOID PhShowKsiMessageEx(_In_opt_ HWND WindowHandle, _In_opt_ PCWSTR Icon,
                        _In_opt_ NTSTATUS Status, _In_ BOOLEAN Force,
                        _In_ PCWSTR Title, _In_ PCWSTR Format, ...) {
  va_list argptr;

  va_start(argptr, Format);
  PhpShowKsiMessage(WindowHandle, TD_OK_BUTTON, Icon, Status, Force, Title,
                    Format, argptr);
  va_end(argptr);
}
 ...
 *
 * Calls PhpShowKsiMessage forcing the message to be shown and using
 * TD_OK_BUTTON.
 */
VOID PhShowKsiMessage(_In_opt_ HWND WindowHandle, _In_opt_ PCWSTR Icon,
                      _In_ PCWSTR Title, _In_ PCWSTR Format, ...) {
  va_list argptr;

  va_start(argptr, Format);
  PhpShowKsiMessage(WindowHandle, TD_OK_BUTTON, Icon, 0, TRUE, Title, Format,
                    argptr);
  va_end(argptr);
}
 ...
 */
VOID KsiCreateRandomizedName(_In_ PCWSTR Prefix, _Out_ PPH_STRING *Name,
 ...
 */
NTSTATUS KsiSvcConnectToServer(VOID) {
  NTSTATUS status;
  ...
  commandLine = PhFormatString(L"\"%s\" -ras \"%s\"", fileName->Buffer,
                               PhGetString(serviceName));

  status = PhCreateService(
      &serviceHandle, PhGetString(serviceName), PhGetString(serviceName),
      SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS, SERVICE_DEMAND_START,
      SERVICE_ERROR_IGNORE, PhGetString(commandLine), L"LocalSystem", L"");
  ...
  portName =
      PhConcatStrings2(L"\\BaseNamedObjects\\", PhGetString(serviceName));
 ...
 */
NTSTATUS PhRestartSelf(_In_ PCPH_STRINGREF AdditionalCommandLine) {
  ...
  commandline = PhConcatStringRef2(&commandlineSr, AdditionalCommandLine);

  status = KsiSvcConnectToServer();
  if (NT_SUCCESS(status)) {
    status =
        PhSvcCallCreateProcessForKsi(PhGetString(commandline), mitigationFlags);
  ...
  if (WindowsVersion >= WINDOWS_10_22H2) {
    status = PhUpdateProcThreadAttribute(
        attributeList, PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY, mitigationFlags,
        sizeof(ULONG64) * 2);
  } else {
    status = PhUpdateProcThreadAttribute(
        attributeList, PROC_THREAD_ATTRIBUTE_MITIGATION_POLICY, mitigationFlags,
  ...
  status = PhCreateProcessWin32Ex(NULL, PhGetString(commandline), NULL, NULL,
                                  &startupInfo,
 ...
 */
PPH_STRING PhGetKsiServiceName(VOID) {
  PPH_STRING string;
 ...
 */
_Function_class_(KPH_COMMS_CALLBACK) BOOLEAN
    KsiCommsCallback(_In_ ULONG_PTR ReplyToken, _In_ PCKPH_MESSAGE Message) {
  if (Message->Header.MessageId == KphMsgRequiredStateFailure &&
      Message->Kernel.RequiredStateFailure.ClientId.UniqueProcess ==
          NtCurrentProcessId()) {
 ...
 */
NTSTATUS KsiReadConfiguration(_In_ PCPH_STRINGREF FileName, _Out_ PBYTE *Data,
                              _Out_ PULONG Length) {
  NTSTATUS status;
  PPH_STRING fileName;
  HANDLE fileHandle;
  ...
  *Length = 0;
  ...
  if (fileName = PhGetApplicationDirectoryFileName(FileName, TRUE)) {
    status =
        PhCreateFile(&fileHandle, &fileName->sr, FILE_GENERIC_READ,
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS KsiValidateDynamicConfiguration(_In_ PBYTE DynData,
  ...
  if (fileName = KsiGetKernelFileName()) {
    ...
    status = KphDynDataLookup((PKPH_DYN_CONFIG)DynData, DynDataLength,
                              supportData.Class, supportData.Machine,
                              supportData.TimeDateStamp,
                              supportData.SizeOfImage, NULL, NULL);
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS KsiGetDynData(_Out_ PBYTE *DynData, _Out_ PULONG DynDataLength,
                       _Out_ PBYTE *Signature, _Out_ PULONG SignatureLength) {
 ...
 */
VOID KsiActivateDynData(_In_opt_ HWND WindowHandle, _In_ KPH_LEVEL Level) {
  NTSTATUS status;
  BOOLEAN showMessage;
  PBYTE dynData = NULL;
  ULONG dynDataLength;
  ...
  status =
      KsiGetDynData(&dynData, &dynDataLength, &signature, &signatureLength);
  ...
  if (!showMessage) {
    ...
    if (PhGetPhReleaseChannel() < PhCanaryChannel) {
      PhShowKsiMessageEx(
          WindowHandle, TD_WARNING_ICON, 0, FALSE,
    ...
    PhShowKsiMessageEx(WindowHandle, TD_WARNING_ICON, 0, FALSE,
                       L"Reduced driver functionality",
                       L"The dynamic configuration was not found.");
  } else if (!NT_SUCCESS(status)) {
    PhShowKsiMessageEx(WindowHandle, TD_WARNING_ICON, status, FALSE,
                       L"Reduced driver functionality",
...
/**
 * Determine the on-disk kernel driver filename for the current application
 * context.
 *
 * \return Referenced `PPH_STRING` containing the full driver filename.
 */
PPH_STRING PhGetKsiFileName(VOID) {
  ...
  PhInitFormatS(&format[1], L"\\sPicyaDham");
  PhInitFormatS(&format[2], L"\\sPicyaDham");
  ...
  applicationDirectory =
      PhGetBaseNameChangeExtension(&fileName->sr, &DriverExtension);
  PhDereferenceObject(fileName);
#endif
 ...
 */
PPH_STRING PhGetTemporaryKsiDirectory(VOID) {
  PPH_STRING applicationDirectory;
...
#else
  applicationDirectory =
      PhGetApplicationDirectoryFileNameZ(L"temp-drivers\\", FALSE);
#endif
...
/**
 * Create a temporary copy of a driver file in the specified temporary
 * directory.
 *
 ...
 * \param[out] TempFileName Receives referenced `PPH_STRING` of the created
 * file.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS KsiCreateTemporaryDriverFile(_In_ PPH_STRING FileName,
  ...
  *TempFileName = NULL;
  ...
  if (NT_SUCCESS(status = PhCreateDirectoryWin32(&TempDirectory->sr))) {
    if (NT_SUCCESS(status = PhCopyFileWin32(PhGetString(FileName),
                                            PhGetString(tempFileName), TRUE))) {
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS KsiConnect(_In_opt_ HWND WindowHandle) {
  NTSTATUS status;
  ...
  if (!PhDoesFileExistWin32(PhGetString(KsiFileName))) {
    PhShowKsiMessageEx(WindowHandle, TD_SHIELD_ERROR_ICON, 0, FALSE,
  ...
  if (PhIsNullOrEmptyString(portName =
                                PhGetStringSetting(SETTING_KSI_PORT_NAME)))
    PhClearReference(&portName);
  if (PhIsNullOrEmptyString(altitude =
                                PhGetStringSetting(SETTING_KSI_ALTITUDE)))
    PhClearReference(&altitude);
  if (PhIsNullOrEmptyString(systemProcessName = PhGetStringSetting(
                                SETTING_KSI_SYSTEM_PROCESS_NAME)))
  ...
  config.SystemProcessName =
      (systemProcessName ? &systemProcessName->sr : NULL);
  ...
  config.Flags.DisableImageLoadProtection =
      !!PhGetIntegerSetting(SETTING_KSI_DISABLE_IMAGE_LOAD_PROTECTION);
  config.Flags.RandomizedPoolTag =
      !!PhGetIntegerSetting(SETTING_KSI_RANDOMIZED_POOL_TAG);
  config.Flags.DynDataNoEmbedded =
      !!PhGetIntegerSetting(SETTING_KSI_DYN_DATA_NO_EMBEDDED);
  config.Flags.DisableSystemProcess =
      !!PhGetIntegerSetting(SETTING_KSI_DISABLE_SYSTEM_PROCESS);
  config.Flags.DisableThreadNames =
      !!PhGetIntegerSetting(SETTING_KSI_DISABLE_THREAD_NAMES);
  ...
  if (status == STATUS_NO_SUCH_FILE) {
    ...
    tempFileName =
        PhGetStringSetting(SETTING_KSI_PREVIOUS_TEMPORARY_DRIVER_FILE);
    if (tempFileName) {
    ...
    if (!NT_SUCCESS(status) && tempDriverDir) {
      if (NT_SUCCESS(status = KsiCreateTemporaryDriverFile(
                         ksiFileName, tempDriverDir, &tempFileName))) {
        PhSetStringSetting(SETTING_KSI_PREVIOUS_TEMPORARY_DRIVER_FILE,
  ...
  if (status == STATUS_SI_KSIDLL_VERSION_MISMATCH) {
    PhShowKsiMessageEx(NULL, TD_SHIELD_ERROR_ICON,
                       STATUS_SI_KSIDLL_VERSION_MISMATCH, FALSE,
                       L"Unable to load kernel driver",
                       L"The last sPicyaDham update requires a reboot.");
    goto CleanupExit;
  }
  ...
  if (!NT_SUCCESS(status)) {
    ...
    if (PhShowKsiMessage2(
            WindowHandle, TD_YES_BUTTON | TD_NO_BUTTON, TD_SHIELD_ERROR_ICON,
            status, FALSE, L"Unable to load kernel driver",
            L"Try again with alternate driver load method?") != IDYES) {
      goto CleanupExit;
    }
    ...
    if (NT_SUCCESS(status)) {
      ...
      PhShowKsiMessage(
          WindowHandle, TD_INFORMATION_ICON, L"Kernel driver loaded",
  ...
  if (!NT_SUCCESS(status)) {
    PhShowKsiMessageEx(WindowHandle, TD_SHIELD_ERROR_ICON, status, FALSE,
                       L"Unable to load kernel driver",
  ...
  if (!NtCurrentPeb()->BeingDebugged && (level != KphLevelMax)) {
    if ((level == KphLevelHigh) && !PhStartupParameters.KphStartupMax) {
    ...
    if (!NT_SUCCESS(status)) {
      PhShowKsiMessageEx(WindowHandle, TD_ERROR_ICON, status, FALSE,
                         L"Unable to load kernel driver",
                         L"Unable to restart.");
      goto CleanupExit;
    }
...
/**
 * Thread callback routine that initializes KSI and notifies the originating
 * window.
 * \param[in] CallbackContext Optional HWND passed as a void*; if provided
 * the routine sends TDM_CLICK_BUTTON to the window when initialization
 * completes.
 * \return NTSTATUS Successful or errant status.
 */
_Function_class_(USER_THREAD_START_ROUTINE) NTSTATUS
    KsiInitializeCallbackThread(_In_opt_ PVOID CallbackContext) {
...
static HRESULT CALLBACK KsiSplashScreenDialogCallbackProc(
    _In_ HWND WindowHandle, _In_ UINT Notification, _In_ WPARAM wParam,
    _In_ LPARAM lParam, _In_ LONG_PTR Context) {
  switch (Notification) {
  case TDN_CREATED: {
    NTSTATUS status;
    HANDLE threadHandle;
    ...
    if (NT_SUCCESS(status = PhCreateThreadEx(&threadHandle,
  ...
  case TDN_TIMER: {
    ...
    SendMessage(WindowHandle, TDM_SET_ELEMENT_TEXT, TDE_CONTENT,
                (LPARAM)timeSpan->Buffer);
 ...
 * splash-screen setting is enabled the initialization will be done
 * asynchronously with a UI; otherwise initialization is performed
 * synchronously.
 */
VOID PhInitializeKsi(VOID) {
  if (!PhGetOwnTokenAttributes().Elevated)
    return;

  if (WindowsVersion < WINDOWS_10) {
    PhShowKsiMessageEx(
        NULL, TD_SHIELD_ERROR_ICON, 0, FALSE, L"Unable to load kernel driver",
  ...
  if (USER_SHARED_DATA->NativeProcessorArchitecture !=
          PROCESSOR_ARCHITECTURE_AMD64 &&
      USER_SHARED_DATA->NativeProcessorArchitecture !=
  ...
  if (PhIsNullOrEmptyString(KsiObjectName =
                                PhGetStringSetting(SETTING_KSI_OBJECT_NAME)))
...
_Success_(return) BOOLEAN
    PhParseKsiSettingsBlob(_In_ PPH_STRING KsiSettingsBlob,
...
/**
 * Create a small configuration blob containing the KSI directory and service
 * name.
 *
 ...
 */
PVOID PhCreateKsiSettingsBlob(VOID) {
  PVOID object;
  PPH_BYTES value;
  PPH_STRING string;
 ...
 */
NTSTATUS PhQueryKphCounters(_Out_ PULONG64 Duration,
  ...
  if (KsiLevel() < KphLevelLow) {
    ...
    *DurationUp = 0;
    ...
    performanceCounterDuration =
        performanceCounterStop.QuadPart - performanceCounterStart.QuadPart;
    performanceCounterDown =
        performanceCounter.QuadPart - performanceCounterStart.QuadPart;
    performanceCounterUp =
        performanceCounterStop.QuadPart - performanceCounter.QuadPart;
...
#if defined(KSI_ONLINE_PLATFORM_SUPPORT)
PPH_STRING KsiCreateBuildString(VOID) {
  ...
  PhGetPhVersionNumbers(&majorVersion, &minorVersion, &buildVersion,
                        &revisionVersion);
...
NTSTATUS KsiCreatePlatformSupportInformation(
    _In_ PCPH_STRINGREF FileName, _Out_ PUSHORT ImageMachine,
    _Out_ PULONG TimeDateStamp, _Out_ PULONG SizeOfImage,
    _Out_writes_bytes_to_(OutputLength, *ReturnLength) PWSTR OutputBuffer,
    _In_ SIZE_T OutputLength, _Out_opt_ PSIZE_T ReturnLength) {
  NTSTATUS status;
  HANDLE fileHandle;
  USHORT imageMachine;
  ...
  status = PhCreateFile(&fileHandle, FileName, FILE_GENERIC_READ,
  ...
  while (bytesRemaining) {
    status =
        PhReadFile(fileHandle, buffer, bufferLength, NULL, &numberOfBytesRead);

    if (!NT_SUCCESS(status))
      break;

    status = PhUpdateHash(&hashContext, buffer, numberOfBytesRead);
  ...
  status = PhFinalHash(&hashContext, hash, sizeof(hash), NULL);

  if (!NT_SUCCESS(status))
    goto CleanupExit;

  if (!PhBufferToHexStringBuffer(hash, sizeof(hash), FALSE, OutputBuffer,
                                 OutputLength, ReturnLength)) {
    status = STATUS_FAIL_CHECK;
    goto CleanupExit;
  }

  status = PhLoadMappedImageHeaderPageSize(NULL, fileHandle, &mappedImage);
...
PPH_STRING KsiCreatePlatformBuildString(VOID) {
  static CONST PH_STRINGREF platformHeader =
      PH_STRINGREF_INIT(L"KsiPlatformSupport: ");
  static CONST KSI_PLATFORM_BUILD_ENTRY platformFiles[] = {
      {KPH_DYN_CLASS_NTOSKRNL,
       PH_STRINGREF_INIT(L"\\SystemRoot\\System32\\ntoskrnl.exe")},
      {KPH_DYN_CLASS_NTKRLA57,
       PH_STRINGREF_INIT(L"\\SystemRoot\\System32\\ntkrla57.exe")},
      {KPH_DYN_CLASS_LXCORE,
       PH_STRINGREF_INIT(L"\\SystemRoot\\System32\\drivers\\lxcore.sys")},
  };
  ...
  for (ULONG i = 0; i < RTL_NUMBER_OF(platformFiles); i++) {
    ...
    if (NT_SUCCESS(KsiCreatePlatformSupportInformation(
            &platformFiles[i].FileName, &imageMachine, &timeDateStamp,
            &sizeOfImage, outputBuffer, sizeof(outputBuffer), &outputLength))) {
      PPH_STRING string;
...
PPH_STRING KsiCreateUpdateWindowsString(VOID) {
...
_Function_class_(USER_THREAD_START_ROUTINE) NTSTATUS
    KsiCheckKernelSupportThread(_In_ PVOID Parameter) {
  ...
  if (!NT_SUCCESS(status = PhHttpInitialize(&httpContext)))
    goto CleanupExit;
  if (!NT_SUCCESS(status = PhHttpConnect(httpContext, L"systeminformer.io",
...
HRESULT CALLBACK KsiKernelSupportCheckDialogCallbackProc(
    _In_ HWND hwndDlg, _In_ UINT uMsg, _In_ WPARAM wParam, _In_ LPARAM lParam,
    _In_ LONG_PTR dwRefData) {
  PKSI_KERNEL_SUPPORT_CHECK_CONTEXT context =
      (PKSI_KERNEL_SUPPORT_CHECK_CONTEXT)dwRefData;
      ...
      if (ReadBooleanAcquire(&context->IsSupported)) {
        ...
        if (context->IsCanaryChannel) {
          ...
          config.pszContent = L"This kernel version is not yet supported. "
                              L"Your kernel version is pending review review "
                              L"on the development branch.";
        } else {
          config.pszMainInstruction = L"Kernel version not supported";
          config.pszContent = L"This kernel version is not yet supported. "
                              L"For the latest kernel support switch to the "
                              L"Canary update channel "
...
VOID KsiShowKernelSupportCheckDialog(_In_opt_ HWND WindowHandle) {
  ...
  statusMessage =
      PhpGetKsiMessage2(STATUS_SI_DYNDATA_UNSUPPORTED_KERNEL, FALSE,
                        L"Checking for pending platform update...", NULL);
