...
_IRQL_requires_max_(DISPATCH_LEVEL) _Must_inspect_result_ NTSTATUS
    KphpQuerySectionMappings(_In_ PVOID SectionObject,
  ...
  if (!dyn || (dyn->MmSectionControlArea == ULONG_MAX) ||
  ...
  for (PLIST_ENTRY link = listHead->Flink; link != listHead;
       link = link->Flink) {
    status =
        RtlULongAdd(returnLength, sizeof(KPH_SECTION_MAP_ENTRY), &returnLength);
    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                    "RtlULongAdd failed: %!STATUS!", status);
  ...
  for (PLIST_ENTRY link = listHead->Flink; link != listHead;
    ...
    if (vad->ViewMapType == VIEW_MAP_TYPE_PROCESS) {
      ...
      process =
          (PEPROCESS)((ULONG_PTR)vad->VadsProcess & ~VIEW_MAP_TYPE_PROCESS);
      if (process) {
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  if (Add2Ptr(BaseAddress, BufferSize) > MmHighestUserAddress) {
    ...
    status = MmCopyMemory(buffer, copyAddress, BufferSize,
                          MM_COPY_MEMORY_VIRTUAL, &numberOfBytesRead);
  ...
  } else {
    {
      ...
      if (KphGetCurrentProcessState() == KPH_PROCESS_STATE_MAXIMUM) {
      ...
      status = MmCopyVirtualMemory(process, BaseAddress, PsGetCurrentProcess(),
                                   Buffer, BufferSize, accessMode,
                                   &numberOfBytesRead);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  status = ObReferenceObjectByHandle(SectionHandle, 0, *MmSectionObjectType,
                                     KernelMode, &sectionObject, NULL);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObReferenceObjectByHandle failed: %!STATUS!", status);
  ...
  case KphSectionMappingsInformation: {
    if (SectionInformation) {
      buffer = KphAllocateNPagedA(SectionInformationLength,
                                  KPH_TAG_SECTION_QUERY, stackBuffer);
      if (!buffer) {
        status = STATUS_INSUFFICIENT_RESOURCES;
        goto Exit;
      }
    ...
    status = KphpQuerySectionMappings(sectionObject, buffer,
    ...
    status =
        KphCopyToMode(SectionInformation, buffer, returnLength, AccessMode);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  case KphMemoryImageSection: {
    ...
    if (!MemoryInformation || (MemoryInformationLength < sizeof(HANDLE))) {
    ...
    status = ZwQueryVirtualMemory(ProcessHandle, BaseAddress,
                                  MemoryMappedFilenameInformation,
                                  mappedFileName, MAX_PATH, &length);
    if ((status == STATUS_BUFFER_OVERFLOW) && (length > 0)) {
      ...
      status = ZwQueryVirtualMemory(ProcessHandle, BaseAddress,
                                    MemoryMappedFilenameInformation,
                                    mappedFileName, length, &length);
    }

    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                    "ZwQueryVirtualMemory failed: %!STATUS!", status);

      goto Exit;
    }

    InitializeObjectAttributes(&objectAttributes, mappedFileName,
    ...
    status = KphCreateFile(
        ...
        IO_IGNORE_SHARE_ACCESS_CHECK, KernelMode);
    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
    ...
    InitializeObjectAttributes(&objectAttributes, NULL,
                               (AccessMode ? 0 : OBJ_KERNEL_HANDLE), NULL,
                               NULL);

    status = ZwCreateSection(&sectionHandle, SECTION_QUERY | SECTION_MAP_READ,
                             &objectAttributes, NULL, PAGE_READONLY,
                             SEC_IMAGE_NO_EXECUTE, fileHandle);

    ObCloseHandle(fileHandle, KernelMode);

    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                    "ZwCreateSection failed: %!STATUS!", status);

      goto Exit;
    }

    status = KphWriteHandleToMode(MemoryInformation, sectionHandle, AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(HANDLE);
    }
  ...
  case KphMemoryDataSection: {
    ...
    status =
        ZwQueryVirtualMemory(ProcessHandle, BaseAddress,
    ...
    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                    "KPH_VM_TLS_CREATE_DATA_SECTION status: %!STATUS!", status);
    ...
    status =
        KphCopyToMode(&memoryInformation->SectionFileSize, &tls.SectionFileSize,
                      sizeof(LARGE_INTEGER), AccessMode);
    if (!NT_SUCCESS(status)) {
  ...
  case KphMemoryMappedInformation: {
    ...
    status = KphCopyToMode(MemoryInformation, &memoryInformation,
                           sizeof(KPH_MEMORY_MAPPED_INFORMATION), AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(KPH_MEMORY_MAPPED_INFORMATION);
    }
...
/**
 * \brief Copies memory from the current process to a target process.
 *
 * \param[in] ProcessHandle A handle to the target process.
 * \param[in] BaseAddress The base address in the target process to write to.
 * \param[in] Buffer The buffer in the current process containing the data.
 * \param[in] BufferSize The number of bytes to copy.
 * \param[out] NumberOfBytesWritten Receives the number of bytes written.
 * \param[in] AccessMode The mode in which to perform access checks.
 *
 * \return Successful or errant status.
 */
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  //
  // For now, only allow writing to user-mode addresses.
  //
  if (Add2Ptr(BaseAddress, BufferSize) > MmHighestUserAddress) {
    status = STATUS_ACCESS_VIOLATION;
    goto Exit;
  ...
  status =
      MmCopyVirtualMemory(PsGetCurrentProcess(), Buffer, process, BaseAddress,
                          BufferSize, accessMode, &numberOfBytesWritten);
...
Exit:
  ...
  if (NumberOfBytesWritten) {
    KphWriteSizeTToMode(NumberOfBytesWritten, numberOfBytesWritten, AccessMode);
  }
