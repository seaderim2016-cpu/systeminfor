...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  status = KphCopyFromMode(&clientId, ClientId, sizeof(CLIENT_ID), AccessMode);
  ...
  if (clientId.UniqueThread) {
    ...
    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                    "PsLookupProcessThreadByCid failed: %!STATUS!", status);
    ...
    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                    "PsLookupProcessByProcessId failed: %!STATUS!", status);
  ...
  if ((DesiredAccess & KPH_PROCESS_READ_ACCESS) != DesiredAccess) {
    status = KphDominationCheck(PsGetCurrentProcess(), process, AccessMode);
    if (!NT_SUCCESS(status)) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                    "KphDominationCheck failed: %!STATUS!", status);
  ...
  //
  status = ObOpenObjectByPointer(process, (AccessMode ? 0 : OBJ_KERNEL_HANDLE),
                                 NULL, DesiredAccess, *PsProcessType,
                                 KernelMode, &processHandle);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObOpenObjectByPointer failed: %!STATUS!", status);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  status = ObOpenObjectByPointer(
      primaryToken, (AccessMode ? 0 : OBJ_KERNEL_HANDLE), NULL, DesiredAccess,
      *SeTokenObjectType, KernelMode, &tokenHandle);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObOpenObjectByPointer failed: %!STATUS!", status);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  //
  status =
      ObOpenObjectByPointer(job, (AccessMode ? 0 : OBJ_KERNEL_HANDLE), NULL,
                            DesiredAccess, *PsJobType, KernelMode, &jobHandle);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObOpenObjectByPointer failed: %!STATUS!", status);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  status = KphDominationAndPrivilegeCheck(
      KPH_TOKEN_PRIVILEGE_TERMINATE, PsGetCurrentThread(), process, AccessMode);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "KphDominationAndPrivilegeCheck failed: %!STATUS!", status);
  ...
  //
  status =
      ObOpenObjectByPointer(process, OBJ_KERNEL_HANDLE, NULL, PROCESS_TERMINATE,
  ...
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ZwTerminateProcess failed: %!STATUS!", status);
  }
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  case KphProcessBasicInformation: {
    ...
    info.NumberOfMicrosoftImageLoads =
        ReadSizeTNoFence(&process->NumberOfMicrosoftImageLoads);
    info.NumberOfAntimalwareImageLoads =
        ReadSizeTNoFence(&process->NumberOfAntimalwareImageLoads);
    info.NumberOfVerifiedImageLoads =
        ReadSizeTNoFence(&process->NumberOfVerifiedImageLoads);
    info.NumberOfUntrustedImageLoads =
        ReadSizeTNoFence(&process->NumberOfUntrustedImageLoads);

    info.UserWritableReferences = 0;
    if (process->FileObject && process->FileObject->SectionObjectPointer) {
      info.UserWritableReferences = MmDoesFileHaveUserWritableReferences(
    ...
    status = KphCopyToMode(ProcessInformation, &info,
                           sizeof(KPH_PROCESS_BASIC_INFORMATION), AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(KPH_PROCESS_BASIC_INFORMATION);
    }
  ...
  case KphProcessStateInformation: {
    ...
    status = KphWriteULongToMode(ProcessInformation, processState, AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(KPH_PROCESS_STATE);
    }
  ...
  case KphProcessWSLProcessId: {
    ...
    status = KphQueryInformationProcessContext(process,
                                               ...
                                               &processId, sizeof(ULONG), NULL);
    if (!NT_SUCCESS(status)) {
      goto Exit;
    }

    status = KphWriteULongToMode(ProcessInformation, processId, AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(ULONG);
    }
  ...
  case KphProcessSequenceNumber: {
    ...
    if (!ProcessInformation || (ProcessInformationLength < sizeof(ULONG64))) {
      status = STATUS_INFO_LENGTH_MISMATCH;
    ...
    status =
        KphWriteULong64ToMode(ProcessInformation, sequenceNumber, AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(ULONG64);
    }
  ...
  case KphProcessStartKey: {
    ...
    status = KphWriteULong64ToMode(ProcessInformation, startKey, AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(ULONG64);
    }
  ...
  case KphProcessImageSection: {
    ...
    processSectionObject =
        *(PVOID *)Add2Ptr(processObject, dyn->EpSectionObject);
    ...
    status = ObOpenObjectByPointer(
        processSectionObject, (AccessMode ? 0 : OBJ_KERNEL_HANDLE), NULL,
        SECTION_QUERY | SECTION_MAP_READ, *MmSectionObjectType, KernelMode,
    ...
    status = KphWriteHandleToMode(ProcessInformation, processSectionHandle,
                                  AccessMode);
    if (NT_SUCCESS(status)) {
      returnLength = sizeof(HANDLE);
    }
  ...
  case KphProcessImageFileName: {
    ...
    status = KphCopyUnicodeStringToMode(
        ProcessInformation, ProcessInformationLength, process->ImageFileName,
        &returnLength, AccessMode);
...
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  if (AccessMode != KernelMode) {
    processInformation = KphAllocatePagedA(ProcessInformationLength,
                                           KPH_TAG_PROCESS_INFO, stackBuffer);
    if (!processInformation) {
      KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
    ...
    __try {
      CopyFromUser(processInformation, ProcessInformation,
  ...
  status = ObOpenObjectByPointer(process, OBJ_KERNEL_HANDLE, NULL,
                                 PROCESS_SET_INFORMATION, *PsProcessType,
                                 KernelMode, &processHandle);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObOpenObjectByPointer failed: %!STATUS!", status);
  ...
  case KphProcessEmptyWorkingSet: {
    ...
    status = ZwSetInformationProcess(processHandle, ProcessQuotaLimits,
                                     &quotaLimits, sizeof(QUOTA_LIMITS_EX));
    //
  ...
  status =
      ZwSetInformationProcess(processHandle, processInformationClass,
...
/**
 * \brief Escalates handle access by creating a new handle with more access.
 *
 * \param[in] ProcessHandle Handle to the process containing the handle.
 * \param[in] Handle Handle to escalate.
 * \param[in] AccessMask Requested access.
 * \param[out] NewHandle Receives the new handle.
 * \param[in] AccessMode The mode in which to perform access checks.
 *
 * \return Successful or errant status.
 */
_IRQL_requires_max_(PASSIVE_LEVEL) _Must_inspect_result_ NTSTATUS
  ...
  status =
      ObReferenceObjectByHandle(ProcessHandle, PROCESS_DUP_HANDLE,
                                *PsProcessType, AccessMode, &process, NULL);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObReferenceObjectByHandle (Process) failed: %!STATUS!",
                  status);
    goto Exit;
  }

  status = ObReferenceObjectByHandleWithTag(Handle, 0, NULL, AccessMode, 'hpsK',
                                            &object, NULL);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObReferenceObjectByHandleWithTag (Object) failed: %!STATUS!",
                  status);
    goto Exit;
  }
  ...
  //
  status = ObOpenObjectByPointer(
      object, ((AccessMode == KernelMode) ? OBJ_KERNEL_HANDLE : 0), NULL,
      AccessMask, NULL, KernelMode, &newHandle);
  if (!NT_SUCCESS(status)) {
    KphTracePrint(TRACE_LEVEL_VERBOSE, GENERAL,
                  "ObOpenObjectByPointer failed: %!STATUS!", status);
    goto Exit;
  ...
  status = KphWriteHandleToMode(NewHandle, newHandle, AccessMode);

Exit:

  if (object) {
    ObDereferenceObjectWithTag(object, 'hpsK');
  }
