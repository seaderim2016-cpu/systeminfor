...
} KPHM_ACQUIRE_DRIVER_UNLOAD_PROTECTION,
    *PKPHM_ACQUIRE_DRIVER_UNLOAD_PROTECTION;
...
typedef struct _KPHM_RELEASE_DRIVER_UNLOAD_PROTECTION {
  ...
  LONG ClientPreviousCount;
    ...
    *PKPHM_RELEASE_DRIVER_UNLOAD_PROTECTION;
...
typedef struct _KPHM_ESCALATE_HANDLE {
  NTSTATUS Status;
  HANDLE ProcessHandle;
  HANDLE Handle;
  ACCESS_MASK AccessMask;
  HANDLE NewHandle;
} KPHM_ESCALATE_HANDLE, *PKPHM_ESCALATE_HANDLE;

typedef struct _KPHM_WRITE_VIRTUAL_MEMORY {
  NTSTATUS Status;
  HANDLE ProcessHandle;
  PVOID BaseAddress;
  PVOID Buffer;
...
} KPHM_WRITE_VIRTUAL_MEMORY, *PKPHM_WRITE_VIRTUAL_MEMORY;
...
typedef struct _KPHM_FILE {
  ...
  union {
    ...
    struct {
      ...
      ULONG InStackFileObject
          : 1; // IoGetStackLimits FLT_RELATED_OBJECTS.FileObject
  ...
  KPHM_COPY_INFORMATION
  CopyInformation; // FltGetCopyInformationFromCallbackData

  union {
    ULONG64 Information2;
    struct {
      ULONG64 OpenedAsCopySource : 1; // IoCheckFileObjectOpenedAsCopySource
      ULONG64 OpenedAsCopyDestination
          : 1; // IoCheckFileObjectOpenedAsCopyDestination
      ULONG64 Spare2 : 62;
    };
  };
  ...
  PVOID
  ImageSectionObject; // FILE_OBJECT.SectionObjectPointer.ImageSectionObject
