...
typedef enum _KPH_MESSAGE_ID {
  ...
  KphMsgEscalateHandle,
  KphMsgWriteVirtualMemory,
...
typedef enum _KPH_MESSAGE_FIELD_ID {
  ...
  MaxKphMsgField
} KPH_MESSAGE_FIELD_ID,
    *PKPH_MESSAGE_FIELD_ID;
...
typedef const KPH_MESSAGE_DYNAMIC_TABLE_ENTRY
...
typedef struct _KPH_MESSAGE {
  ...
  union {
    ...
    union {
      ...
      KPHM_ESCALATE_HANDLE EscalateHandle;
      KPHM_WRITE_VIRTUAL_MEMORY WriteVirtualMemory;
    } User;
...
VOID KphMsgQuerySystemTime(_Out_ PLARGE_INTEGER SystemTime);

VOID KphMsgInit(_Out_writes_bytes_(KPH_MESSAGE_MIN_SIZE) PKPH_MESSAGE Message,
                _In_ KPH_MESSAGE_ID MessageId);

_Must_inspect_result_ NTSTATUS KphMsgValidate(_In_ PCKPH_MESSAGE Message);
