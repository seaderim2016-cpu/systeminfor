...
_Function_class_(PH_WINDOW_ENUM_CALLBACK) BOOLEAN CALLBACK
    PhpThemeWindowEnumChildWindows(_In_ HWND WindowHandle,
                                   _In_opt_ PVOID Context);

_Function_class_(PH_WINDOW_ENUM_CALLBACK) BOOLEAN CALLBACK
    PhpReInitializeThemeWindowEnumChildWindows(_In_ HWND WindowHandle,
                                               _In_opt_ PVOID Context);

LRESULT CALLBACK PhpThemeWindowSubclassProc(_In_ HWND hWnd, _In_ UINT uMsg,
                                            _In_ WPARAM wParam,
                                            _In_ LPARAM lParam);
LRESULT CALLBACK PhpThemeWindowGroupBoxSubclassProc(_In_ HWND WindowHandle,
                                                    _In_ UINT uMsg,
                                                    _In_ WPARAM wParam,
                                                    _In_ LPARAM lParam);
LRESULT CALLBACK PhpThemeWindowTabControlWndSubclassProc(_In_ HWND WindowHandle,
                                                         _In_ UINT uMsg,
                                                         _In_ WPARAM wParam,
                                                         _In_ LPARAM lParam);

LRESULT CALLBACK PhpThemeWindowListBoxControlSubclassProc(
    _In_ HWND WindowHandle, _In_ UINT uMsg, _In_ WPARAM wParam,
    _In_ LPARAM lParam);
...
LRESULT CALLBACK PhpThemeWindowACLUISubclassProc(_In_ HWND WindowHandle,
                                                 _In_ UINT uMsg,
                                                 _In_ WPARAM wParam,
                                                 _In_ LPARAM lParam);

// Win10-RS5 (uxtheme.dll ordinal 132)
BOOL(WINAPI *ShouldAppsUseDarkMode_I)(VOID) = NULL;
// Win10-RS5 (uxtheme.dll ordinal 138)
BOOL(WINAPI *ShouldSystemUseDarkMode_I)(VOID) = NULL;
// Win10-RS5 (uxtheme.dll ordinal 136)
BOOL(WINAPI *FlushMenuThemes_I)(VOID) = NULL;
...
BOOL(WINAPI *SetPreferredAppMode_I)(_In_ PreferredAppMode AppMode) = NULL;

// Win10-RS5 (uxtheme.dll ordinal 139)
BOOL(WINAPI *IsDarkModeAllowedForApp_I)(_In_ HWND WindowHandle) = NULL;
...
BOOLEAN PhEnableThemeSupport = TRUE; // Default ON for premium dark look
BOOLEAN PhEnableThemeAcrylicSupport = TRUE;
BOOLEAN PhEnableThemeAcrylicWindowSupport = TRUE;
BOOLEAN PhEnableThemeNativeButtons = TRUE;
BOOLEAN PhEnableThemeListviewBorder = FALSE;
HBRUSH PhThemeWindowBackgroundBrush = NULL;
// ===== PREMIUM MIDNIGHT PALETTE =====
// Deep midnight black - main window background
COLORREF PhThemeWindowForegroundColor = RGB(12, 12, 16);
// Slightly lighter for panels/sidebars
COLORREF PhThemeWindowBackgroundColor = RGB(14, 14, 20);
// Cards / sub-panels
COLORREF PhThemeWindowBackground2Color = RGB(22, 22, 32);
// Hover highlight - electric indigo-blue accent
COLORREF PhThemeWindowHighlightColor = RGB(60, 100, 220);
// Selected item highlight
COLORREF PhThemeWindowHighlight2Color = RGB(80, 120, 240);
// Text - soft near-white for readability
COLORREF PhThemeWindowTextColor = RGB(220, 225, 235);
// ====================================

VOID PhInitializeWindowTheme(_In_ HWND WindowHandle,
  ...
  if (EnableThemeSupport && WindowsVersion >= WINDOWS_10_RS5) {
    ...
    if (PhBeginInitOnce(&initOnce)) {
      if (WindowsVersion >= WINDOWS_10_19H2) {
        ...
        if (baseAddress) {
          SetPreferredAppMode_I =
              PhGetDllBaseProcedureAddress(baseAddress, NULL, 135);
          // FlushMenuThemes_I = PhGetDllBaseProcedureAddress(baseAddress, NULL,
          // 136);
        }
  ...
  if (!PhThemeWindowBackgroundBrush) {
    ...
    PhThemeWindowBackgroundBrush =
        CreateSolidBrush(PhThemeWindowBackgroundColor);
  ...
  if (EnableThemeSupport) {
    ...
    if (defaultWindowProc != PhpThemeWindowSubclassProc) {
      ...
      SetWindowLongPtr(WindowHandle, GWLP_WNDPROC,
                       (LONG_PTR)PhpThemeWindowSubclassProc);

      if (WindowsVersion >= WINDOWS_10_RS5) {
        WCHAR windowClassName[MAX_PATH];
        if (!GetClassName(WindowHandle, windowClassName,
                          RTL_NUMBER_OF(windowClassName)))
          windowClassName[0] = UNICODE_NULL;
        if (PhEqualStringZ(windowClassName, L"PhTreeNew", FALSE) ||
            PhEqualStringZ(windowClassName, WC_LISTVIEW, FALSE))
          PhAllowDarkModeForWindow(
              WindowHandle, TRUE); // HACK for dynamically generated plugin tabs
      }
    }

    PhEnumChildWindows(WindowHandle, PhpThemeWindowEnumChildWindows, NULL);
...
VOID PhInitializeWindowThemeEx(_In_ HWND WindowHandle) {
  static CONST PH_STRINGREF keyPath = PH_STRINGREF_INIT(
      L"Software\\Microsoft\\Windows\\CurrentVersion\\Themes\\Personalize");
  HANDLE keyHandle;
  BOOLEAN enableThemeSupport = FALSE;

  if (NT_SUCCESS(
          PhOpenKey(&keyHandle, KEY_READ, PH_KEY_CURRENT_USER, &keyPath, 0))) {
    enableThemeSupport =
        !PhQueryRegistryUlongZ(keyHandle, L"AppsUseLightTheme");
    NtClose(keyHandle);
  }
...
VOID PhReInitializeWindowTheme(_In_ HWND WindowHandle) {
  ...
  PhEnumChildWindows(WindowHandle, PhpReInitializeThemeWindowEnumChildWindows,
                     NULL);
  ...
  do {
    if (currentWindow = FindWindowEx(NULL, currentWindow, NULL, NULL)) {
      ...
      if (UlongToHandle(processID) == NtCurrentProcessId()) {
        ...
        if (!GetClassName(currentWindow, windowClassName,
                          RTL_NUMBER_OF(windowClassName)))
          windowClassName[0] = UNICODE_NULL;
        ...
        if (currentWindow != WindowHandle) {
          if (PhEqualStringZ(windowClassName, L"#32770", FALSE)) {
            PhEnumChildWindows(currentWindow,
...
HRESULT PhGetWindowThemeAttribute(_In_ HWND WindowHandle,
                                  _In_ ULONG AttributeId,
                                  _Out_writes_bytes_(AttributeLength)
                                      PVOID Attribute,
                                  _In_ ULONG AttributeLength) {
  static PH_INITONCE initOnce = PH_INITONCE_INIT;
  static HRESULT(WINAPI * DwmGetWindowAttribute_I)(
      _In_ HWND WindowHandle, _In_ ULONG AttributeId,
  ...
  return DwmGetWindowAttribute_I(WindowHandle, AttributeId, Attribute,
                                 AttributeLength);
}

HRESULT PhSetWindowThemeAttribute(_In_ HWND WindowHandle,
                                  _In_ ULONG AttributeId,
                                  _In_reads_bytes_(AttributeLength)
                                      PVOID Attribute,
                                  _In_ ULONG AttributeLength) {
  static PH_INITONCE initOnce = PH_INITONCE_INIT;
  ...
  if (PhBeginInitOnce(&initOnce)) {
    ...
    if (baseAddress = PhLoadLibrary(L"dwmapi.dll")) {
      DwmSetWindowAttribute_I =
          PhGetDllBaseProcedureAddress(baseAddress, "DwmSetWindowAttribute", 0);
    }
  ...
  return DwmSetWindowAttribute_I(WindowHandle, AttributeId, Attribute,
                                 AttributeLength);
}

VOID PhInitializeThemeWindowFrame(_In_ HWND WindowHandle) {
  if (WindowsVersion >= WINDOWS_10_RS5) {
    BOOL boolAttribute;
    ...
    if (PhEnableThemeSupport) {
      ...
      //         boolAttribute = FALSE;
      //
      //         if (FAILED(PhSetWindowThemeAttribute(WindowHandle,
      //         DWMWA_USE_IMMERSIVE_DARK_MODE, &boolAttribute, sizeof(BOOL))))
      //         {
      //             PhSetWindowThemeAttribute(WindowHandle,
      //             DWMWA_USE_IMMERSIVE_DARK_MODE_BEFORE_20H1, &boolAttribute,
      //             sizeof(BOOL));
      //         }
      //
      ...
      if (WindowsVersion >= WINDOWS_11) {
        PhSetWindowThemeAttribute(WindowHandle, DWMWA_CAPTION_COLOR,
    ...
    if (WindowsVersion >= WINDOWS_11_22H2) {
      ulongAttribute = 1;
      PhSetWindowThemeAttribute(WindowHandle, DWMWA_SYSTEMBACKDROP_TYPE,
                                &ulongAttribute, sizeof(ULONG));
    }
  }
}

VOID PhWindowThemeSetDarkMode(_In_ HWND WindowHandle,
...
HBRUSH PhWindowThemeControlColor(_In_ HWND WindowHandle, _In_ HDC Hdc,
                                 _In_ HWND ChildWindowHandle, _In_ LONG Type) {
  ...
  case CTLCOLOR_EDIT: {
    if (PhEnableThemeSupport) {
      ...
      SetDCBrushColor(Hdc, RGB(22, 22, 32)); // Midnight panel color
  ...
  case CTLCOLOR_SCROLLBAR: {
    if (PhEnableThemeSupport) {
      SetDCBrushColor(Hdc, RGB(8, 8, 12)); // Deep void for scrollbars
      return PhGetStockBrush(DC_BRUSH);
    } else {
  ...
  return (HBRUSH)DefWindowProc(WindowHandle, Type, (WPARAM)Hdc,
                               (LPARAM)ChildWindowHandle);
}

VOID PhWindowThemeMainMenuBorder(_In_ HWND WindowHandle) {
...
VOID PhInitializeThemeWindowTabControl(_In_ HWND TabControlWindow) {
  PPHP_THEME_WINDOW_TAB_CONTEXT context;

  context = PhAllocateZero(sizeof(PHP_THEME_WINDOW_TAB_CONTEXT));
  context->DefaultWindowProc =
      (WNDPROC)GetWindowLongPtr(TabControlWindow, GWLP_WNDPROC);
  context->CursorPos.x = LONG_MIN;
  context->CursorPos.y = LONG_MIN;

  PhSetWindowContext(TabControlWindow, LONG_MAX, context);
  SetWindowLongPtr(TabControlWindow, GWLP_WNDPROC,
                   (LONG_PTR)PhpThemeWindowTabControlWndSubclassProc);

  InvalidateRect(TabControlWindow, NULL, FALSE);
}

VOID PhInitializeThemeWindowGroupBox(_In_ HWND GroupBoxHandle) {
  ...
  SetWindowLongPtr(GroupBoxHandle, GWLP_WNDPROC,
                   (LONG_PTR)PhpThemeWindowGroupBoxSubclassProc);

  InvalidateRect(GroupBoxHandle, NULL, FALSE);
}

VOID PhInitializeWindowThemeMainMenu(_In_ HMENU MenuHandle) {
  MENUINFO menuInfo;
...
VOID PhInitializeWindowThemeListboxControl(_In_ HWND ListBoxControl) {
  PPHP_THEME_WINDOW_STATUSBAR_CONTEXT context;

  context = PhAllocateZero(sizeof(PHP_THEME_WINDOW_STATUSBAR_CONTEXT));
  context->DefaultWindowProc =
      (WNDPROC)GetWindowLongPtr(ListBoxControl, GWLP_WNDPROC);
  context->CursorPos.x = LONG_MIN;
  context->CursorPos.y = LONG_MIN;

  PhSetWindowContext(ListBoxControl, LONG_MAX, context);
  SetWindowLongPtr(ListBoxControl, GWLP_WNDPROC,
                   (LONG_PTR)PhpThemeWindowListBoxControlSubclassProc);

  InvalidateRect(ListBoxControl, NULL, FALSE);
  SetWindowPos(ListBoxControl, HWND_TOP, 0, 0, 0, 0,
               SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER | SWP_NOOWNERZORDER |
                   SWP_FRAMECHANGED);
}

VOID PhInitializeWindowThemeComboboxControl(_In_ HWND ComboBoxControl) {
  PPHP_THEME_WINDOW_COMBO_CONTEXT context;

  context = PhAllocateZero(sizeof(PHP_THEME_WINDOW_COMBO_CONTEXT));
  context->DefaultWindowProc =
      (WNDPROC)GetWindowLongPtr(ComboBoxControl, GWLP_WNDPROC);
  context->ThemeHandle = PhOpenThemeData(ComboBoxControl, VSCLASS_COMBOBOX,
                                         PhGetWindowDpi(ComboBoxControl));
  context->CursorPos.x = LONG_MIN;
  context->CursorPos.y = LONG_MIN;

  PhSetWindowContext(ComboBoxControl, LONG_MAX, context);
  SetWindowLongPtr(ComboBoxControl, GWLP_WNDPROC,
                   (LONG_PTR)PhpThemeWindowComboBoxControlSubclassProc);

  InvalidateRect(ComboBoxControl, NULL, FALSE);
}

VOID PhInitializeWindowThemeACLUI(_In_ HWND ACLUIControl) {
  PhSetWindowContext(ACLUIControl, LONG_MAX,
                     (PVOID)GetWindowLongPtr(ACLUIControl, GWLP_WNDPROC));
  SetWindowLongPtr(ACLUIControl, GWLP_WNDPROC,
                   (LONG_PTR)PhpThemeWindowACLUISubclassProc);
...
_Function_class_(PH_WINDOW_ENUM_CALLBACK) BOOLEAN CALLBACK
  ...
  if (PhEqualStringZ(windowClassName, L"#32770", TRUE)) {
    ...
    //     break;
    // case 1: // Old colors
    ListView_SetBkColor(WindowHandle,
                        PhThemeWindowBackgroundColor); // RGB(30, 30, 30)
    ListView_SetTextBkColor(WindowHandle,
                            PhThemeWindowBackgroundColor); // RGB(30, 30, 30)
    ...
    TreeView_SetBkColor(WindowHandle,
                        PhThemeWindowBackgroundColor); // RGB(30, 30, 30));
...
#define EM_SETBKGNDCOLOR (WM_USER + 67)
    ...
    //{
    // case 0: // New colors
    //     SendMessage(WindowHandle, EM_SETBKGNDCOLOR, 0, RGB(0xff, 0xff,
    //     0xff)); break;
    // case 1: // Old colors
  ...
  } else if (PhEqualStringZ(windowClassName, WC_LISTBOX, FALSE) ||
    ...
    if (SendMessage(WindowHandle, CB_GETCOMBOBOXINFO, 0, (LPARAM)&info)) {
      // if (info.hwndItem)
      //{
      //     SendMessage(info.hwndItem, EM_SETMARGINS, EC_LEFTMARGIN,
      //     MAKELPARAM(0, 0));
      // }
    ...
    // if ((PhGetWindowStyle(WindowHandle) & CBS_DROPDOWNLIST) !=
    // CBS_DROPDOWNLIST)
    {
...
BOOLEAN PhThemeWindowDrawItem(_In_ HWND WindowHandle,
  ...
  case ODT_MENU: {
    ...
    if (DrawInfo->itemState & ODS_HOTLIGHT) {
      ...
      //     PhGetStockBrush(DC_BRUSH)); break;
    ...
    if (isChecked) {
      ...
      DrawText(DrawInfo->hDC, menuCheckText.Buffer,
               (UINT)menuCheckText.Length / sizeof(WCHAR), &DrawInfo->rcItem,
    ...
    if (menuItemInfo->Flags & PH_EMENU_SEPARATOR) {
      ...
      // (GetSystemMetrics(SM_CXEDGE) * 2) + 5 + 1;
      ...
      PatBlt(DrawInfo->hDC, DrawInfo->rcItem.left,
             DrawInfo->rcItem.top + cyEdge,
             DrawInfo->rcItem.right - DrawInfo->rcItem.left, 1, PATCOPY);
      ...
      if (menuItemInfo->Bitmap) {
        ...
        GdiAlphaBlend(DrawInfo->hDC, DrawInfo->rcItem.left + 4,
                      DrawInfo->rcItem.top + 4,
                      PhGetSystemMetrics(SM_CXSMICON, dpiValue),
                      PhGetSystemMetrics(SM_CYSMICON, dpiValue), bufferDc, 0, 0,
                      PhGetSystemMetrics(SM_CXSMICON, dpiValue),
                      PhGetSystemMetrics(SM_CYSMICON, dpiValue), blendFunction);
      ...
      if ((menuItemInfo->Flags & PH_EMENU_MAINMENU) == PH_EMENU_MAINMENU) {
        if (firstPart.Length) {
          DrawText(DrawInfo->hDC, firstPart.Buffer,
                   (UINT)firstPart.Length / sizeof(WCHAR), &DrawInfo->rcItem,
                   DT_LEFT | DT_SINGLELINE | DT_CENTER | DT_VCENTER |
                       drawTextFlags);
        }
      } else {
        if (firstPart.Length) {
      ...
      if (secondPart.Length) {
        DrawText(DrawInfo->hDC, secondPart.Buffer,
                 (UINT)secondPart.Length / sizeof(WCHAR), &DrawInfo->rcItem,
    ...
    if (menuItemInfo->Items && menuItemInfo->Items->Count &&
        (menuItemInfo->Flags & PH_EMENU_MAINMENU) != PH_EMENU_MAINMENU) {
      HTHEME themeHandle;

      if (themeHandle =
              PhOpenThemeData(DrawInfo->hwndItem, VSCLASS_MENU, dpiValue)) {
        // if (IsThemeBackgroundPartiallyTransparent(themeHandle,
        // MENU_POPUPSUBMENU, isDisabled ? MSM_DISABLED : MSM_NORMAL))
        //     DrawThemeParentBackground(DrawInfo->hwndItem, DrawInfo->hDC,
        //     NULL);

        rect.left = rect.right - PhGetDpi(25, dpiValue);

        PhDrawThemeBackground(themeHandle, DrawInfo->hDC, MENU_POPUPSUBMENU,
                              isDisabled ? MSM_DISABLED : MSM_NORMAL, &rect,
                              NULL);
    ...
    ExcludeClipRect(DrawInfo->hDC, rect.left, rect.top, rect.right,
                    rect.bottom); // exclude last
  ...
  case ODT_COMBOBOX: {
    ...
    if (length < MAX_PATH) {
      ...
      if (ComboBox_GetLBText(DrawInfo->hwndItem, DrawInfo->itemID, comboText) ==
          CB_ERR)
        break;

      DrawText(DrawInfo->hDC, comboText, (UINT)PhCountStringZ(comboText),
               &DrawInfo->rcItem, DT_LEFT | DT_END_ELLIPSIS | DT_SINGLELINE);
    }
...
BOOLEAN PhThemeWindowMeasureItem(_In_ HWND WindowHandle,
    ...
    if ((menuItemInfo->Flags & PH_EMENU_SEPARATOR) == PH_EMENU_SEPARATOR) {
      ...
      if (hdc = GetDC(WindowHandle)) {
        ...
        if ((menuItemInfo->Flags & PH_EMENU_MAINMENU) == PH_EMENU_MAINMENU) {
          ...
          if (GetTextExtentPoint32(hdc, text, (ULONG)textCount, &textSize)) {
            DrawInfo->itemWidth =
                textSize.cx + (cyborder * 2) + PhGetDpi(90, dpiValue); // HACK
            DrawInfo->itemHeight =
                cymenu + (cyborder * 2) + PhGetDpi(1, dpiValue);
          }
        }
...
//    {
//        SIZE size;
//        PhGetThemePartSize(htheme, hdcScreen, BP_CHECKBOX,
//        CBS_UNCHECKEDNORMAL, NULL, TS_DRAW, &size); cx = size.cx; cy =
//        size.cy;
//    }
//
...
//    void* p;
//    HBITMAP hbmCheckboxes = CreateDIBSection(hdcScreen, &bi, DIB_RGB_COLORS,
//    &p, NULL, 0);
//
...
//    {
//        // Frame 1: Unchecked.
//        PhDrawThemeBackground(htheme, hdcMem, BP_CHECKBOX,
//        CBS_UNCHECKEDNORMAL, &rc, NULL); PhOffsetRect(&rc, cx, 0);
//
//        // Frame 2: Checked.
//        PhDrawThemeBackground(htheme, hdcMem, BP_CHECKBOX, CBS_CHECKEDNORMAL,
//        &rc, NULL); PhOffsetRect(&rc, cx, 0);
//
//        // Frame 3: Indeterminate.
//        PhDrawThemeBackground(htheme, hdcMem, BP_CHECKBOX, CBS_MIXEDNORMAL,
//        &rc, NULL); PhOffsetRect(&rc, cx, 0);
//
//        // Frame 4: Disabled, unchecked.
//        PhDrawThemeBackground(htheme, hdcMem, BP_CHECKBOX,
//        CBS_UNCHECKEDDISABLED, &rc, NULL); PhOffsetRect(&rc, cx, 0);
//
//        // Frame 5: Disabled, checked.
//        PhDrawThemeBackground(htheme, hdcMem, BP_CHECKBOX,
//        CBS_CHECKEDDISABLED, &rc, NULL);
//
//        // Done with the theme.
...
//        // Frame 3: Indeterminate.
//        DrawFrameControl(hdcMem, &rc, DFC_BUTTON, baseFlags | DFCS_CHECKED |
//        DFCS_BUTTON3STATE); PhOffsetRect(&rc, cx, 0);
//
//        // Frame 4: Disabled, unchecked.
...
VOID PhThemeDrawButtonIcon(_In_ LPNMCUSTOMDRAW DrawInfo, _In_ HICON ButtonIcon,
                           _In_ PRECT ButtonRect, _In_ LONG WindowDpi) {
  BOOL result;
  ICONINFO iconInfo;
  BITMAP bmp;
  ...
  DrawIconEx(
      ...
      ButtonIcon, width, height, 0, NULL, DI_NORMAL);

  if (!result) // HACK
  {
    BUTTON_IMAGELIST imageList = {0};

    if (Button_GetImageList(DrawInfo->hdr.hwndFrom, &imageList) &&
        imageList.himl) {
      ButtonRect->left += PhGetDpi(1, WindowDpi);

      PhImageListDrawIcon(
          imageList.himl, 0, DrawInfo->hdc,
          ButtonRect
              ->left, // + ((ButtonRect->right - ButtonRect->left) - width) / 2,
          ButtonRect->top +
              ((ButtonRect->bottom - ButtonRect->top) - height) / 2,
          ILD_NORMAL, FALSE);
...
LRESULT CALLBACK PhThemeWindowDrawButton(_In_ LPNMCUSTOMDRAW DrawInfo) {
  ULONG buttonStyle;

  buttonStyle = PhGetWindowStyle(DrawInfo->hdr.hwndFrom);
  // COMMANDLINK unsupported
  if ((buttonStyle & BS_COMMANDLINK) == BS_COMMANDLINK ||
      (buttonStyle & BS_DEFCOMMANDLINK) == BS_DEFCOMMANDLINK)
  ...
  BOOLEAN isMixed =
      (DrawInfo->uItemState & CDIS_INDETERMINATE) == CDIS_INDETERMINATE;
  ...
  BOOLEAN isKeyboardFocused =
      isFocused &&
      (DrawInfo->uItemState & CDIS_SHOWKEYBOARDCUES) == CDIS_SHOWKEYBOARDCUES;
  RECT bufferRect = {0, 0, DrawInfo->rc.right - DrawInfo->rc.left,
  ...
  case CDDS_PREPAINT: {
    ...
    BOOLEAN isCheckbox = (buttonStyle & BS_AUTOCHECKBOX) == BS_AUTOCHECKBOX ||
                         (buttonStyle & BS_AUTO3STATE) == BS_AUTO3STATE;
    BOOLEAN isRadio = (buttonStyle & BS_AUTORADIOBUTTON) == BS_AUTORADIOBUTTON;

    if (!isCheckbox && !isRadio && PhEnableThemeNativeButtons &&
    ...
    if (!(buttonIcon = Static_GetIcon(DrawInfo->hdr.hwndFrom, 0)))
      buttonIcon = (HICON)SendMessage(DrawInfo->hdr.hwndFrom, BM_GETIMAGE,
                                      IMAGE_ICON, 0);

    // Add support for disabled and tristate checkboxes, support for radio with
    // multiline (ex. TaskDialog) (Dart Vanya)
    if (isCheckbox || isRadio) {
      ...
      if (isCheckbox) {
        if (isDisabled)
          state = isChecked ? CBS_CHECKEDDISABLED
                  : isMixed ? CBS_MIXEDDISABLED
                            : CBS_UNCHECKEDDISABLED;
        else if (isSelected)
          state = isChecked ? CBS_CHECKEDPRESSED
                  : isMixed ? CBS_MIXEDPRESSED
                            : CBS_UNCHECKEDPRESSED;
        else if (isHighlighted)
          state = isChecked ? CBS_CHECKEDHOT
                  : isMixed ? CBS_MIXEDHOT
                            : CBS_UNCHECKEDHOT;
        else
          state = isChecked ? CBS_CHECKEDNORMAL
                  : isMixed ? CBS_MIXEDNORMAL
                            : CBS_UNCHECKEDNORMAL;
      } else {
        if (isDisabled)
      ...
      if (buttonIcon) {
        if (isSelected || isChecked) {
          ...
          SetTextColor(DrawInfo->hdc, !isDisabled ? PhThemeWindowTextColor
                                                  : RGB(0x9B, 0x9B, 0x9B));
          // SetDCBrushColor(DrawInfo->hdc, PhThemeWindowBackgroundColor); //
        ...
        if (themeHandle = PhOpenThemeData(DrawInfo->hdr.hwndFrom,
                                          VSCLASS_BUTTON, dpiValue)) {
          SIZE checkBoxSize = {0};
          SIZE textSize = {0};
          INT linesCount;

          PhGetThemePartSize(themeHandle, DrawInfo->hdc,
                             isCheckbox ? BP_CHECKBOX : BP_RADIOBUTTON, state,
                             &bufferRect, THEMEPARTSIZE_TRUE, &checkBoxSize);
          ...
          if (linesCount > 1)
            bufferRect.bottom -=
                textSize.cy * (linesCount - 1); // HACK (very sensitive value)

          // if (IsThemeBackgroundPartiallyTransparent(themeHandle, isCheckbox ?
          // BP_CHECKBOX : BP_RADIOBUTTON, state))
          //     DrawThemeParentBackground(DrawInfo->hdr.hwndFrom,
          //     DrawInfo->hdc, NULL);

          PhDrawThemeBackground(themeHandle, DrawInfo->hdc,
                                isCheckbox ? BP_CHECKBOX : BP_RADIOBUTTON,
                                state, &bufferRect, NULL);

          bufferRect = DrawInfo->rc;
          bufferRect.left = checkBoxSize.cx + 4; // TNP_ICON_RIGHT_PADDING

          if (linesCount == 1) {
            DrawText(DrawInfo->hdc, buttonText->Buffer,
                     (UINT)buttonText->Length / sizeof(WCHAR), &bufferRect,
                     DT_LEFT | DT_SINGLELINE | DT_VCENTER |
                         (!isKeyboardFocused ? DT_HIDEPREFIX : 0));
          } else {
            DrawText(DrawInfo->hdc, buttonText->Buffer,
                     (UINT)buttonText->Length / sizeof(WCHAR), &bufferRect,
            ...
            bufferRect.top = (DrawInfo->rc.bottom - DrawInfo->rc.top) / 2 -
                             (bufferRect.bottom - bufferRect.top) / 2 - 1;
            bufferRect.bottom = DrawInfo->rc.bottom,
          ...
          if (isKeyboardFocused) {
            ...
            if (bufferRect.bottom > DrawInfo->rc.bottom - 1)
              bufferRect.bottom = DrawInfo->rc.bottom - 1;

            for (INT i = 0; i < bufferRect.right - bufferRect.left - 1; i += 2)
              SetPixel(DrawInfo->hdc, bufferRect.left + i + 1,
                       bufferRect.bottom, PhThemeWindowHighlight2Color);
            for (INT i = 0; i < bufferRect.bottom - bufferRect.top - 1; i += 2)
              SetPixel(DrawInfo->hdc, bufferRect.right,
                       bufferRect.bottom - i - 1, PhThemeWindowHighlight2Color);
            for (INT i = 0; i < bufferRect.right - bufferRect.left - 1; i += 2)
              SetPixel(DrawInfo->hdc, bufferRect.right - i - 1, bufferRect.top,
                       PhThemeWindowHighlight2Color);
            for (INT i = 0; i < bufferRect.bottom - bufferRect.top - 1; i += 2)
              SetPixel(DrawInfo->hdc, bufferRect.left, bufferRect.top + i + 1,
                       PhThemeWindowHighlight2Color);
          }
          ...
          if (isChecked) {
            HFONT newFont =
                PhDuplicateFontWithNewHeight(PhApplicationFont, 16, dpiValue);
            HFONT oldFont;

            oldFont = SelectFont(DrawInfo->hdc, newFont);
            DrawText(DrawInfo->hdc, L"\u2611", 1, &DrawInfo->rc,
                     DT_LEFT | DT_SINGLELINE | DT_VCENTER);
            SelectFont(DrawInfo->hdc, oldFont);
            DeleteFont(newFont);
          } else {
            HFONT newFont =
                PhDuplicateFontWithNewHeight(PhApplicationFont, 22, dpiValue);
            HFONT oldFont;
      ...
      SetBkMode(DrawInfo->hdc, TRANSPARENT);
      SetDCBrushColor(DrawInfo->hdc, !isFocused ? PhThemeWindowBackground2Color
...
LRESULT CALLBACK PhThemeWindowDrawRebar(_In_ LPNMCUSTOMDRAW DrawInfo) {
  ...
  switch (DrawInfo->dwDrawStage) {
  case CDDS_PREPAINT: {
    // REBARBANDINFO bandinfo = { sizeof(REBARBANDINFO), RBBIM_CHILD |
    // RBBIM_STYLE | RBBIM_CHEVRONLOCATION | RBBIM_CHEVRONSTATE }; BOOL
    // havebandinfo = (BOOL)SendMessage(DrawInfo->hdr.hwndFrom, RB_GETBANDINFO,
    // DrawInfo->dwItemSpec, (LPARAM)&bandinfo);
...
LRESULT CALLBACK PhThemeWindowDrawToolbar(_In_ LPNMTBCUSTOMDRAW DrawInfo) {
  switch (DrawInfo->nmcd.dwDrawStage) {
  case CDDS_PREPAINT:
    return CDRF_NOTIFYITEMDRAW | CDRF_NOTIFYPOSTPAINT;
  case CDDS_ITEMPREPAINT: {
    TBBUTTONINFO buttonInfo = {sizeof(TBBUTTONINFO), TBIF_STYLE | TBIF_COMMAND |
                                                         TBIF_STATE |
                                                         TBIF_IMAGE};
    ...
    ULONG currentIndex =
        (ULONG)SendMessage(DrawInfo->nmcd.hdr.hwndFrom, TB_COMMANDTOINDEX,
                           DrawInfo->nmcd.dwItemSpec, 0);
    BOOLEAN isHighlighted = SendMessage(DrawInfo->nmcd.hdr.hwndFrom,
                                        TB_GETHOTITEM, 0, 0) == currentIndex;
    BOOLEAN isMouseDown =
        SendMessage(DrawInfo->nmcd.hdr.hwndFrom, TB_ISBUTTONPRESSED,
                    DrawInfo->nmcd.dwItemSpec, 0) == 0;
    BOOLEAN isEnabled =
        SendMessage(DrawInfo->nmcd.hdr.hwndFrom, TB_ISBUTTONENABLED,
                    DrawInfo->nmcd.dwItemSpec, 0) != 0;

    if (SendMessage(DrawInfo->nmcd.hdr.hwndFrom, TB_GETBUTTONINFO,
                    (ULONG)DrawInfo->nmcd.dwItemSpec,
                    (LPARAM)&buttonInfo) == INT_ERROR) {
      break;
    }
    ...
    //     FillRect(DrawInfo->nmcd.hdc, &DrawInfo->nmcd.rc,
    ...
    if (isHighlighted) {
      ...
      // case 0: // New colors
      //     SetTextColor(DrawInfo->nmcd.hdc, PhThemeWindowTextColor); //
      //     RGB(0x0, 0x0, 0x0)); SetDCBrushColor(DrawInfo->nmcd.hdc,
      //     PhThemeWindowBackgroundColor); // GetSysColor(COLOR_3DFACE));//
      //     RGB(0xff, 0xff, 0xff)); FillRect(DrawInfo->nmcd.hdc,
      //     &DrawInfo->nmcd.rc, PhGetStockBrush(DC_BRUSH)); break;
      // case 1: // Old colors
      //     SetTextColor(DrawInfo->nmcd.hdc, PhThemeWindowTextColor);
      //     SetDCBrushColor(DrawInfo->nmcd.hdc, PhThemeWindowBackgroundColor);
      //     //PhThemeWindowForegroundColor); FillRect(DrawInfo->nmcd.hdc,
      //     &DrawInfo->nmcd.rc, PhGetStockBrush(DC_BRUSH)); break;
      // }

      BOOLEAN isPressed = buttonInfo.fsState & TBSTATE_PRESSED;
      SetTextColor(DrawInfo->nmcd.hdc,
                   PhThemeWindowTextColor); // RGB(0x0, 0x0, 0x0));
      // SetDCBrushColor(DrawInfo->nmcd.hdc, PhThemeWindowBackgroundColor); //
      // GetSysColor(COLOR_3DFACE));// RGB(0xff, 0xff, 0xff));
      if (!isPressed)
        FillRect(DrawInfo->nmcd.hdc, &DrawInfo->nmcd.rc,
                 PhThemeWindowBackgroundBrush);
      else {
    ...
    if (buttonInfo.iImage != I_IMAGECALLBACK) {
      ...
      if (toolbarImageList = (HIMAGELIST)SendMessage(
              DrawInfo->nmcd.hdr.hwndFrom, TB_GETIMAGELIST, 0, 0)) {
        LONG x;
        LONG y;
        ...
        if (buttonInfo.fsStyle & BTNS_SHOWTEXT) {
          DrawInfo->nmcd.rc.left +=
              PhGetSystemMetrics(SM_CXEDGE, dpiValue); // PhGetDpi(5, dpiValue);
          x = DrawInfo->nmcd.rc
                  .left; // + ((DrawInfo->nmcd.rc.right -
                         // DrawInfo->nmcd.rc.left) - PhSmallIconSize.X) / 2;
          y = DrawInfo->nmcd.rc.top +
              ((DrawInfo->nmcd.rc.bottom - DrawInfo->nmcd.rc.top) -
               bitmapHeight) /
                  2;
        } else {
          x = DrawInfo->nmcd.rc.left +
              ((DrawInfo->nmcd.rc.right - DrawInfo->nmcd.rc.left) -
               bitmapWidth) /
                  2 -
              (isDropDown * 4);
          y = DrawInfo->nmcd.rc.top +
        ...
        PhImageListDrawIcon(toolbarImageList, buttonInfo.iImage,
                            DrawInfo->nmcd.hdc, x, y, ILD_NORMAL, !isEnabled);

        if (isDropDown) {
          HDC hdc = DrawInfo->nmcd.hdc;
          LPRECT rc = &DrawInfo->nmcd.rc;
          int triangleLeft = rc->right - 11,
              triangleTop = (rc->bottom - rc->top) / 2 - 2;
          POINT vertices[] = {{triangleLeft, triangleTop},
                              {triangleLeft + 6, triangleTop},
                              {triangleLeft + 3, triangleTop + 3}};
    ...
    if (buttonInfo.fsStyle & BTNS_SHOWTEXT) {
      ...
      SendMessage(DrawInfo->nmcd.hdr.hwndFrom, TB_GETBUTTONTEXT,
                  (ULONG)DrawInfo->nmcd.dwItemSpec, (LPARAM)buttonText);

      textRect.left +=
          bitmapWidth - (isDropDown * 12); // PhGetDpi(10, dpiValue);
      DrawText(DrawInfo->nmcd.hdc, buttonText, (UINT)PhCountStringZ(buttonText),
               &textRect,
...
LRESULT CALLBACK
PhpThemeWindowDrawListViewGroup(_In_ LPNMLVCUSTOMDRAW DrawInfo) {
  switch (DrawInfo->nmcd.dwDrawStage) {
  case CDDS_PREPAINT: {
...

    {
      ...
      if (PhGetSystemParametersInfo(SPI_GETNONCLIENTMETRICS, sizeof(metrics),
                                    &metrics, dpiValue)) {
    ...
    SetBkMode(DrawInfo->nmcd.hdc, TRANSPARENT);
    // SelectFont(DrawInfo->nmcd.hdc,
    // GetWindowFont(DrawInfo->nmcd.hdr.hwndFrom));
    if (fontHandle) {
    ...
    if (ListView_GetGroupInfo(DrawInfo->nmcd.hdr.hwndFrom,
                              (ULONG)DrawInfo->nmcd.dwItemSpec,
                              &groupInfo) != INT_ERROR) {
      SetTextColor(DrawInfo->nmcd.hdc, PhThemeWindowTextColor);
      ...
      FillRect(DrawInfo->nmcd.hdc, &DrawInfo->rcText,
               PhGetStockBrush(DC_BRUSH));
      DrawInfo->rcText.top -= PhGetDpi(2, dpiValue);
      DrawInfo->rcText.bottom += PhGetDpi(2, dpiValue);
      ...
      if (groupInfo.pszHeader) {
        ...
        DrawText(DrawInfo->nmcd.hdc, groupInfo.pszHeader,
                 (UINT)PhCountStringZ(groupInfo.pszHeader), &DrawInfo->rcText,
...
LRESULT CALLBACK PhpThemeWindowSubclassProc(_In_ HWND hWnd, _In_ UINT uMsg,
  ...
  case WM_NOTIFY: {
    ...
    case NM_CUSTOMDRAW: {
      ...
      if (!GetClassName(customDraw->hdr.hwndFrom, className,
...
VOID ThemeWindowRenderGroupBoxControl(_In_ HWND WindowHandle, _In_ HDC bufferDc,
  ...
  if (NT_SUCCESS(PhGetWindowTextToBuffer(WindowHandle,
                                         PH_GET_WINDOW_TEXT_INTERNAL, text,
                                         RTL_NUMBER_OF(text), &returnLength))) {
    SIZE nameSize = {0};
    RECT bufferRect;

    GetTextExtentPoint32(bufferDc, text, returnLength, &nameSize);
    ...
    DrawText(bufferDc, text, returnLength, &bufferRect,
...
LRESULT CALLBACK PhpThemeWindowGroupBoxSubclassProc(_In_ HWND WindowHandle,
  ...
  case WM_ERASEBKGND: {
    ...
    ThemeWindowRenderGroupBoxControl(WindowHandle, hdc, &clientRect,
                                     oldWndProc);
  }
    return TRUE;
  case WM_ENABLE:
...
VOID ThemeWindowRenderTabControl(_In_ PPHP_THEME_WINDOW_TAB_CONTEXT Context,
                                 _In_ HWND WindowHandle, _In_ HDC bufferDc,
                                 _In_ PRECT clientRect,
                                 _In_ WNDPROC WindowProcedure) {
  // RECT windowRect;
  ...
  // CallWindowProc(WindowProcedure, WindowHandle, WM_PRINTCLIENT,
  // (WPARAM)bufferDc, PRF_CLIENT);

  // TabCtrl_AdjustRect(WindowHandle, FALSE, clientRect); // Make sure we don't
  // paint in the client area. ExcludeClipRect(bufferDc, clientRect->left,
  ...
  //     PhThemeWindowTextColor); FillRect(hdc, &clientRect,
  //     PhGetStockBrush(DC_BRUSH)); break;
  // case 1: // Old colors
  //     //SetTextColor(hdc, PhThemeWindowTextColor);
  ...
  clientRect->top +=
      (itemRect.bottom - itemRect.top) * TabCtrl_GetRowCount(WindowHandle) + 2;
  ...
  for (INT i = 0; i < count; i++) {
    ...
    if (PhPtInRect(&itemRect, &Context->CursorPos)) {
      ...
      //             SetDCBrushColor(bufferDc, GetSysColor(COLOR_3DFACE));//
      //             PhThemeWindowTextColor); FillRect(bufferDc, &itemRect,
      //             PhGetStockBrush(DC_BRUSH));
      //         }
      //     }
      //     break;
...

    {
      if (TabCtrl_GetItem(WindowHandle, i, &tabItem)) {
        DrawText(bufferDc, tabItem.pszText,
                 (UINT)PhCountStringZ(tabItem.pszText), &itemRect,
                 DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_HIDEPREFIX);
      }
    }
  }

  {
    ...
    itemRect.bottom -= 1;
    SetDCBrushColor(bufferDc, PhPtInRect(&itemRect, &Context->CursorPos)
                                  ? PhThemeWindowHighlightColor
                                  : RGB(0x50, 0x50, 0x50));
    FillRect(bufferDc, &itemRect, PhGetStockBrush(DC_BRUSH));

    if (TabCtrl_GetItem(WindowHandle, currentSelection, &tabItem)) {
      DrawText(bufferDc, tabItem.pszText, (UINT)PhCountStringZ(tabItem.pszText),
               &itemRect,
               DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_HIDEPREFIX);
    }
...
LRESULT CALLBACK PhpThemeWindowTabControlWndSubclassProc(_In_ HWND WindowHandle,
  ...
  case WM_MOUSEMOVE: {
    ...
    //     entry.dwState = PhPtInRect(&rect, context->CursorPos) ?
    //     TCIS_HIGHLIGHTED : 0; TabCtrl_SetItem(WindowHandle, i, &entry);
    // }
  ...
  case WM_PAINT: {
    ...
    //     ExcludeClipRect(ps.hdc, clientRect.left, clientRect.top,
    //     clientRect.right, clientRect.bottom);
    // }
    //
    // if (BufferedPaint = BeginBufferedPaint(ps.hdc, &ps.rcPaint,
    // BPBF_COMPATIBLEBITMAP, NULL, &BufferedHDC))
    //{
    //     ThemeWindowRenderTabControl(context, WindowHandle, BufferedHDC,
    //     &ps.rcPaint, oldWndProc); EndBufferedPaint(BufferedPaint, TRUE);
    // }
    // else
    {
      RECT clientRect;
...

      {
        ...
        ExcludeClipRect(hdc, clientRect2.left, clientRect2.top,
                        clientRect2.right, clientRect2.bottom);
      }

      ThemeWindowRenderTabControl(context, WindowHandle, bufferDc, &clientRect,
                                  oldWndProc);

      BitBlt(hdc, clientRect.left, clientRect.top, clientRect.right,
             clientRect.bottom, bufferDc, 0, 0, SRCCOPY);
...
LRESULT CALLBACK PhpThemeWindowListBoxControlSubclassProc(
  ...
  case WM_NCPAINT: {
    ...
    {
      HRGN rectregion =
          CreateRectRgn(windowRect.left + cxEdge, windowRect.top + cyEdge,
                        windowRect.right - cxEdge, windowRect.bottom - cyEdge);
    ...
    if (hdc = GetDCEx(WindowHandle, updateRegion, flags)) {
      ...
      ExcludeClipRect(hdc, clientRect.left, clientRect.top, clientRect.right,
                      clientRect.bottom);
...
VOID ThemeWindowRenderComboBox(_In_ PPHP_THEME_WINDOW_COMBO_CONTEXT Context,
                               _In_ HWND WindowHandle, _In_ HDC bufferDc,
                               _In_ PRECT clientRect,
                               _In_ WNDPROC WindowProcedure) {
  RECT bufferRect = {0, 0, clientRect->right - clientRect->left,
  ...
  SetBkMode(bufferDc, TRANSPARENT);
  SelectFont(bufferDc,
             CallWindowProc(WindowProcedure, WindowHandle, WM_GETFONT, 0, 0));
  SetDCBrushColor(bufferDc, PhThemeWindowBackground2Color);
  ...
  if (PhPtInRect(clientRect, &Context->CursorPos)) {
    SetDCBrushColor(
        bufferDc,
        PhThemeWindowHighlight2Color); // RGB(0, 120, 212) : RGB(68, 68, 68));
  } else {
    SetDCBrushColor(
        bufferDc,
        GetSysColor(COLOR_WINDOWFRAME)); // RGB(0, 120, 212) : RGB(68, 68, 68));
  }
  ...
  if (Context->ThemeHandle) {
    ...
    PhGetThemePartSize(Context->ThemeHandle, bufferDc, CP_DROPDOWNBUTTONRIGHT,
                       CBXSR_NORMAL, NULL, THEMEPARTSIZE_TRUE, &dropdownSize);

    bufferRect.left = clientRect->right - dropdownSize.cy;

    PhDrawThemeBackground(Context->ThemeHandle, bufferDc,
                          CP_DROPDOWNBUTTONRIGHT, CBXSR_DISABLED, &bufferRect,
                          NULL);
  ...
  if ((windowStyle & CBS_DROPDOWNLIST) == CBS_DROPDOWNLIST) {
    ...
    if (length < MAX_PATH) {
      ...
      DrawText(bufferDc, comboText, (UINT)PhCountStringZ(comboText),
               &bufferRect,
...
VOID ThemeWindowComboBoxExcludeRect(
    _In_ PPHP_THEME_WINDOW_COMBO_CONTEXT Context, _In_ HWND WindowHandle,
    _In_ HDC Hdc, _In_ PRECT clientRect, _In_ WNDPROC WindowProcedure) {
  ULONG windowStyle = PhGetWindowStyle(WindowHandle);

  if ((windowStyle & CBS_DROPDOWNLIST) != CBS_DROPDOWNLIST ||
      (windowStyle & CBS_DROPDOWN) != CBS_DROPDOWN) {
    COMBOBOXINFO info = {sizeof(COMBOBOXINFO)};
    ...
    if (CallWindowProc(WindowProcedure, WindowHandle, CB_GETCOMBOBOXINFO, 0,
      ...
      // if (Context->ThemeHandle)
      //{
      //     if (PhGetThemeInt(Context->ThemeHandle, 0, 0, TMT_BORDERSIZE,
      //     &borderSize))
      //     {
      ...
      ExcludeClipRect(Hdc, info.rcItem.left, info.rcItem.top, info.rcItem.right,
                      info.rcItem.bottom);
    }
  }
}
...
LRESULT CALLBACK PhpThemeWindowComboBoxControlSubclassProc(
  ...
  case WM_THEMECHANGED: {
    ...
    context->ThemeHandle = PhOpenThemeData(WindowHandle, VSCLASS_COMBOBOX,
                                           PhGetWindowDpi(WindowHandle));
  } break;
  case WM_ERASEBKGND:
    return TRUE;
  case WM_MOUSEMOVE: {
  ...
  case WM_PAINT: {
    ...
    // BPBF_COMPATIBLEBITMAP, NULL, &BufferedHDC))
    //{
    //     ThemeWindowComboBoxExcludeRect(WindowHandle, ps.hdc, &ps.rcPaint,
    //     oldWndProc, context); ThemeWindowRenderComboBox(WindowHandle,
    //     BufferedHDC, &ps.rcPaint, oldWndProc, context);
    ...
    // else
    {
      ...
      ThemeWindowComboBoxExcludeRect(context, WindowHandle, hdc, &clientRect,
                                     oldWndProc);
      ThemeWindowRenderComboBox(context, WindowHandle, bufferDc, &clientRect,
                                oldWndProc);
    ...
    if (data->code == NM_CUSTOMDRAW) {
      ...
      if (customDraw->dwDrawStage == CDDS_PREPAINT &&
        ...
        if (PhEqualStringZ(className, WC_BUTTON, FALSE)) {
          ...
          FillRect(hdc, &rectControl,
                   PhThemeWindowBackgroundBrush); // fix the annoying white
                                                  // border left by the previous
                                                  // active control
          ReleaseDC(WindowHandle, hdc);
        }
      }
