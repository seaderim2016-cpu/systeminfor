 ...
 *
 * \param Handle A handle to the object whose security descriptor is to be
 * queried.
 * \param SecurityInformation The type of security information to be queried.
 * \param SecurityDescriptor A copy of the specified security descriptor in
...
NTSTATUS PhGetObjectSecurity(_In_ HANDLE Handle,
  ...
  // This is required (especially for File objects) because some drivers don't
  // seem to handle QuerySecurity properly. (wj32)
  memset(buffer, 0, bufferSize);

  status = NtQuerySecurityObject(Handle, SecurityInformation, buffer,
 ...
 * The function preserves ACEs not covered by the change from the lower SACL and
 * replaces other ACEs with the ones from the higher SACL.
 *
 * Example:
 ...
 * Result: a SACL with the trust label from the higher SACL and the mandatory
 * label from the lower SACL.
 *
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhMergeSystemAcls(_In_opt_ PACL LowerSacl, _In_opt_ PACL HigherSacl,
                           _In_ SECURITY_INFORMATION SecurityInformation,
  ...
  if (LowerSacl && (LowerSacl->AclRevision < MIN_ACL_REVISION ||
                    LowerSacl->AclRevision > MAX_ACL_REVISION))
    return STATUS_UNKNOWN_REVISION;

  if (HigherSacl && (HigherSacl->AclRevision < MIN_ACL_REVISION ||
                     HigherSacl->AclRevision > MAX_ACL_REVISION))
    return STATUS_UNKNOWN_REVISION;
...
#define START_FOR_EACH_ACE(_ACL_, _ACE_)                                       \
  if (_ACL_) {                                                                 \
    ...
    for (USHORT _i = 0; _i < (_ACL_)->AceCount;                                \
         _i++, _ACE_ = (PACE_HEADER)PTR_ADD_OFFSET(_ACE_, _ACE_->AceSize)) {   \
      if ((ULONG_PTR)_ACE_ >= (ULONG_PTR)_end)                                 \
        return STATUS_UNSUCCESSFUL; /* malformed */                            \
      if (_ACE_->AceSize < sizeof(ACE_HEADER) ||                               \
          PTR_ADD_OFFSET(_ACE_, _ACE_->AceSize) > _end)                        \
        return STATUS_UNSUCCESSFUL; /* malformed */
  ...
  if (LowerSacl) {
    START_FOR_EACH_ACE(LowerSacl, ace) {
      ...
      if (!(bit & aceTypesToReplace)) {
        ...
        mergedSacl->AceCount++;
        PhEnsureAclRevision((PULONG_PTR)&mergedSacl->AclRevision,
                            mergedAce->AceType);
...
/**
 * Merges two security descriptors according to the provided security
 * information. The function preserves parts not covered by the change from the
 * lower security descriptor and replaces other parts with the ones from the
 * higher security descriptor.
 *
 * Example:
 *  - A lower security descriptor containing a DACL, an owner, and a SACL with a
 * trust label.
 *  - A higher security descriptor containing a DACL, and a SACL with a
 * mandatory label.
 *  - Security information specifies DACL_SECURITY_INFORMATION and
 * LABEL_SECURITY_INFORMATION. Result: a security descriptor with the higher
 * DACL, the lower owner, and a merged SACL with the lower trust label and the
 * higher mandatory label.
 *
 * \param LowerSecurityDescriptor An existing security descriptor.
 * \param HigherSecurityDescriptor An overlaying security descriptor for the
 * specified security information.
 * \param SecurityInformation The type of security information to be set.
...
NTSTATUS PhMergeSecurityDescriptors(
  ...
  status = PhCreateSecurityDescriptor(&mergedSecurityDescriptor,
  ...
  status = PhSetDaclSecurityDescriptor(&mergedSecurityDescriptor, present, acl,
                                       defaulted);
  ...
  status =
      PhSetOwnerSecurityDescriptor(&mergedSecurityDescriptor, sid, defaulted);
  ...
  status =
      PhSetGroupSecurityDescriptor(&mergedSecurityDescriptor, sid, defaulted);
  ...
  status = PhGetSaclSecurityDescriptor(LowerSecurityDescriptor, &present,
                                       &lowerSacl, &defaulted);
  ...
  status = PhGetSaclSecurityDescriptor(HigherSecurityDescriptor, &present,
                                       &higherSacl, &defaulted);
  ...
  status = PhMergeSystemAcls(lowerSacl, higherSacl, SecurityInformation,
                             &mergedSacl);

  if (!NT_SUCCESS(status))
    return status;

  status = PhSetSaclSecurityDescriptor(&mergedSecurityDescriptor, !!mergedSacl,
                                       mergedSacl, FALSE);
  ...
  status = RtlAbsoluteToSelfRelativeSD(&mergedSecurityDescriptor, NULL,
                                       &requiredLength);
  ...
  status = RtlAbsoluteToSelfRelativeSD(
      &mergedSecurityDescriptor, relativeSecurityDescriptor, &requiredLength);
...
NTSTATUS PhEnumProcessEnvironmentVariables(
    _In_ PVOID Environment, _In_ ULONG EnvironmentLength,
    _Inout_ PULONG EnumerationKey, _Out_ PPH_ENVIRONMENT_VARIABLE Variable) {
  ULONG length;
  ULONG startIndex;
  PWCHAR name;
  ULONG nameLength;
  ...
  while (TRUE) {
    ...
    if (*currentChar == L'=' && startIndex != currentIndex)
      break; // equality sign is considered as a delimiter unless it is the
             // first character (diversenok)
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhQueryEnvironmentVariableStringRef(_In_opt_ PVOID Environment,
  ...
  status = RtlQueryEnvironmentVariable(
      Environment, Name->Buffer, Name->Length / sizeof(WCHAR),
      Value ? Value->Buffer : NULL, Value ? Value->Length / sizeof(WCHAR) : 0,
      &returnLength);
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhQueryEnvironmentVariable(_In_opt_ PVOID Environment,
                                    _In_ PCPH_STRINGREF Name,
  ...
  status =
      RtlQueryEnvironmentVariable_U(Environment, &variableName, &variableValue);
  ...
  if (Value && status == STATUS_BUFFER_TOO_SMALL) {
    ...
    status = RtlQueryEnvironmentVariable_U(Environment, &variableName,
                                           &variableValue);
  }
  ...
  status = RtlQueryEnvironmentVariable(Environment, Name->Buffer,
                                       Name->Length / sizeof(WCHAR), NULL, 0,
                                       &returnLength);
  ...
  if (Value && status == STATUS_BUFFER_TOO_SMALL) {
    ...
    status = RtlQueryEnvironmentVariable(
        Environment, Name->Buffer, Name->Length / sizeof(WCHAR), buffer->Buffer,
        buffer->Length / sizeof(WCHAR), &returnLength);
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhSetEnvironmentVariable(_In_opt_ PVOID Environment,
                                  _In_ PCPH_STRINGREF Name,
  ...
  status =
      RtlSetEnvironmentVariable(Environment, &variableName, &variableValue);
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetProcessSectionFileName(_In_ HANDLE SectionHandle,
  ...
  status = PhMapViewOfSection(SectionHandle, ProcessHandle, &baseAddress, 0,
                              NULL, &viewSize, ViewUnmap, 0, PAGE_READONLY);

  if (!NT_SUCCESS(status))
    return status;

  status = PhGetProcessMappedFileName(ProcessHandle, baseAddress, FileName);
 ...
 */
NTSTATUS PhGetProcessUnloadedDlls(_In_ HANDLE ProcessId,
  ...
  if (!NT_SUCCESS(
          status = PhOpenProcess(&processHandle, PROCESS_VM_READ, ProcessId)))
    goto CleanupExit;
  ...
  if (!NT_SUCCESS(status = PhReadVirtualMemory(processHandle, elementSize,
                                               &capturedElementSize,
                                               sizeof(ULONG), NULL)))
    goto CleanupExit;
  ...
  if (!NT_SUCCESS(status = PhReadVirtualMemory(processHandle, eventTrace,
  ...
  if (!NT_SUCCESS(status = PhReadVirtualMemory(
                      processHandle, capturedEventTracePointer,
                      capturedEventTrace, eventTraceSize, NULL)))
    goto CleanupExit;
 ...
 */
NTSTATUS
PhTraceControl(_In_ ETWTRACECONTROLCODE TraceInformationClass,
               _In_reads_bytes_opt_(InputBufferLength) PVOID InputBuffer,
               _In_ ULONG InputBufferLength,
               _Out_writes_bytes_opt_(OutputBufferLength) PVOID OutputBuffer,
               _In_ ULONG OutputBufferLength, _Out_ PULONG ReturnLength) {
  NTSTATUS status;

  status = NtTraceControl(TraceInformationClass, InputBuffer, InputBufferLength,
 ...
 * \param OutputBufferLength A pointer to a variable that receives the output
 * buffer length.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhTraceControlVariableSize(
  ...
  if (status == STATUS_BUFFER_TOO_SMALL) {
    ...
    status =
        NtTraceControl(TraceInformationClass, InputBuffer, InputBufferLength,
 ...
 *
 * \param WindowHandle The handle to the window whose client ID is to be
 * retrieved.
 * \param ClientId A pointer to a CLIENT_ID structure that receives the process
 * and thread IDs.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetWindowClientId(_In_ HWND WindowHandle,
...
_Function_class_(PH_ENUM_DIRECTORY_OBJECTS) NTSTATUS NTAPI
    PhpOpenDriverByBaseAddressCallback(
        _In_ HANDLE RootDirectory, _In_ PPH_STRINGREF Name,
  ...
  InitializeObjectAttributes(&objectAttributes, &driverName,
                             OBJ_CASE_INSENSITIVE, RootDirectory, NULL);
  ...
  status = KphQueryInformationDriver(
      driverHandle, KphDriverBasicInformation, &basicInfo,
      sizeof(KPH_DRIVER_BASIC_INFORMATION), NULL);
 ...
 * \return NTSTATUS Successful or errant status.
 * \remarks STATUS_OBJECT_NAME_NOT_FOUND is returned if the driver could not be
 * found.
 */
NTSTATUS PhOpenDriverByBaseAddress(_Out_ PHANDLE DriverHandle,
  ...
  InitializeObjectAttributes(&objectAttributes, &objectDirectory,
                             OBJ_CASE_INSENSITIVE, NULL, NULL);

  status = NtOpenDirectoryObject(&directoryHandle, DIRECTORY_QUERY,
                                 &objectAttributes);
 ...
 *
 * \param DriverHandle Pointer to a variable that receives the handle to the
 * driver object.
 * \param DesiredAccess Specifies the desired access rights to the driver
 * object.
 * \param RootDirectory Optional handle to the root directory for the object
 * name (can be NULL).
 * \param ObjectName Pointer to a string reference that specifies the name of
 * the driver object.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhOpenDriver(_Out_ PHANDLE DriverHandle,
  ...
  if (KsiLevel() == KphLevelMax) {
    ...
    return KphOpenDriver(DriverHandle, DesiredAccess, &objectAttributes);
  } else {
...
/**
 * Queries variable-sized information for a driver. The function allocates a
 * buffer to contain the information.
 *
 * \param DriverHandle A handle to a driver. The access required depends on the
 * information class specified.
 * \param DriverInformationClass The information class to retrieve.
 * \param Buffer A variable which receives a pointer to a buffer containing the
 * information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhpQueryDriverVariableSize(_In_ HANDLE DriverHandle,
                                    _In_ KPH_DRIVER_INFORMATION_CLASS
  ...
  status = KphQueryInformationDriver(DriverHandle, DriverInformationClass, NULL,
                                     0, &returnLength);
  ...
  if (status == STATUS_BUFFER_TOO_SMALL) {
    ...
    status = KphQueryInformationDriver(DriverHandle, DriverInformationClass,
 ...
 * \param Name A variable which receives a pointer to a string containing the
 * object name.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetDriverName(_In_ HANDLE DriverHandle, _Out_ PPH_STRING *Name) {
  ...
  if (!NT_SUCCESS(status = PhpQueryDriverVariableSize(
                      DriverHandle, KphDriverNameInformation, &unicodeString)))
    return status;
 ...
 * \param DriverHandle A handle to a driver.
 * \param Name A variable which receives a pointer to a string containing the
 * driver image file name.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetDriverImageFileName(_In_ HANDLE DriverHandle,
                                  _Out_ PPH_STRING *Name) {
  NTSTATUS status;
  PUNICODE_STRING unicodeString;

  if (!NT_SUCCESS(
          status = PhpQueryDriverVariableSize(
              DriverHandle, KphDriverImageFileNameInformation, &unicodeString)))
    return status;
...
NTSTATUS PhGetDriverServiceKeyName(_In_ HANDLE DriverHandle,
  ...
  if (!NT_SUCCESS(status = PhpQueryDriverVariableSize(
                      DriverHandle, KphDriverServiceKeyNameInformation,
                      &unicodeString)))
    return status;
...
static NTSTATUS PhpUnloadDriver(_In_ PCPH_STRINGREF ServiceKeyName,
                                _In_ PCPH_STRINGREF DriverFileName) {
  static CONST PH_STRINGREF fullServicesKeyName = PH_STRINGREF_INIT(
  ...
  if (!PhStringRefToUnicodeString(&fullServiceKeyName->sr,
                                  &fullServiceKeyNameUs)) {
    PhDereferenceObject(fullServiceKeyName);
    return STATUS_NAME_TOO_LONG;
  }

  if (NT_SUCCESS(status = PhCreateKey(&serviceKeyHandle, KEY_WRITE | DELETE,
                                      NULL, &fullServiceKeyName->sr, 0, 0,
                                      &disposition))) {
    if (disposition == REG_CREATED_NEW_KEY) {
      ULONG regValue = 0;

      // Set up the required values.
      PhSetValueKeyZ(serviceKeyHandle, L"ErrorControl", REG_DWORD, &regValue,
                     sizeof(ULONG));
      PhSetValueKeyZ(serviceKeyHandle, L"Start", REG_DWORD, &regValue,
                     sizeof(ULONG));
      PhSetValueKeyZ(serviceKeyHandle, L"Type", REG_DWORD, &regValue,
                     sizeof(ULONG));
      PhSetValueKeyZ(serviceKeyHandle, L"ImagePath", REG_SZ,
                     DriverFileName->Buffer,
                     (ULONG)DriverFileName->Length + sizeof(UNICODE_NULL));
    }
 ...
 * Unloads a driver.
 *
 * \param BaseAddress The base address of the driver. This parameter can be NULL
 * if a value is specified in \c Name.
 * \param Name The base name of the driver. This parameter can be NULL if a
 * value is specified in
 * \c BaseAddress and KSystemInformer is loaded.
 * \return NTSTATUS Successful or errant status.
 * \retval STATUS_INVALID_PARAMETER_MIX Both \c BaseAddress and \c Name were
 * null, or \c Name was not specified and KSystemInformer is not loaded.
...
NTSTATUS PhUnloadDriver(_In_opt_ PVOID BaseAddress,
  ...
  if ((level == KphLevelMax) && BaseAddress) {
    if (PhGetOwnTokenAttributes().Elevated) {
      if (NT_SUCCESS(PhOpenDriverByBaseAddress(&driverHandle, BaseAddress))) {
 ...
 *
 * \param Processes A variable which receives a pointer to a buffer containing
 * process information. You must free the buffer using PhFree() when you no
 * longer need it.
 * \return NTSTATUS Successful or errant status.
 * \remarks You can use the \ref PH_FIRST_PROCESS and \ref PH_NEXT_PROCESS
 * macros to process the information contained in the buffer.
 */
 ...
 * process information. You must free the buffer using PhFree() when you no
 * longer need it.
 * \param SystemInformationClass A variable which indicates the kind of system
 ...
 */
NTSTATUS
PhEnumProcessesEx(_Out_ PVOID *Processes,
  ...
  while (TRUE) {
    status = NtQuerySystemInformation(SystemInformationClass, buffer,
                                      bufferSize, &bufferSize);
  ...
  if (bufferSize <= 0x100000)
    initialBufferSize[classIndex] = bufferSize;
  *Processes = buffer;
 ...
 * \param Callback The callback function to be called for each enumerated
 * thread.
 * \param Context An optional context parameter to be passed to the callback
 * function.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumProcessThreads(_In_ HANDLE ProcessId,
  ...
  if (process = PhFindProcessInformation(processes, ProcessId)) {
    status = Callback(process->NumberOfThreads, process->Threads, Context);
  } else {
 ...
 *
 * \param ProcessHandle The handle to the current process. Pass NULL to start
 ...
 *         If there are no more processes to enumerate, it returns
 * STATUS_NO_MORE_ENTRIES. Otherwise, it returns an appropriate NTSTATUS error
 * code.
 */
NTSTATUS PhEnumNextProcess(_In_opt_ HANDLE ProcessHandle,
                           _In_ ACCESS_MASK DesiredAccess,
  ...
  status = NtGetNextProcess(ProcessHandle, DesiredAccess, 0, 0, &processHandle);
  ...
  while (TRUE) {
    ...
    status =
        NtGetNextProcess(processHandle, DesiredAccess, 0, 0, &newProcessHandle);
 ...
 * \param ThreadHandle The handle to the current thread. Pass NULL to start
 * enumeration from the beginning.
 ...
 * \return Returns the status of the enumeration operation.
 *         If the enumeration is successful, it returns STATUS_SUCCESS.
 *         If there are no more threads to enumerate, it returns
 * STATUS_NO_MORE_ENTRIES. Otherwise, it returns an appropriate NTSTATUS error
 * code.
 */
NTSTATUS PhEnumNextThread(_In_ HANDLE ProcessHandle,
  ...
  status = NtGetNextThread(ProcessHandle, ThreadHandle, DesiredAccess, 0, 0,
                           &threadHandle);
...
NTSTATUS PhEnumProcessesForSession(_Out_ PVOID *Processes,
  ...
  while (TRUE) {
    ...
    status = NtQuerySystemInformation(
        SystemSessionProcessInformation, &sessionProcessInfo,
 ...
 * \param ProcessId The ID of the process.
 * \return A pointer to the process information structure for the specified
 * process, or NULL if the structure could not be found.
 */
...
PhFindProcessInformationByImageName(_In_ PVOID Processes,
 ...
 *
 * \param Handles A variable which receives a pointer to a structure containing
 * information about all opened handles. You must free the structure using
 * PhFree() when you no longer need it.
 * \return NTSTATUS Successful or errant status.
 * \retval STATUS_INSUFFICIENT_RESOURCES The handle information returned by the
 * kernel is too large.
 */
__declspec(deprecated("The SystemHandleInformation class is deprecated on "
                      "64-bit Windows. Use PhEnumHandlesEx instead.")) NTSTATUS
PhEnumHandles(_Out_ PSYSTEM_HANDLE_INFORMATION *Handles) {
  ...
  while ((status = NtQuerySystemInformation(SystemHandleInformation, buffer,
                                            bufferSize, NULL)) ==
...
NTSTATUS PhEnumHandlesEx(_Out_ PSYSTEM_HANDLE_INFORMATION_EX *Handles) {
  ...
  if (!NT_SUCCESS(status)) {
    // Fall back to using the previous code that we've used since Windows XP
    // (dmex)
    bufferSize = initialBufferSize;
    buffer = PhAllocate(bufferSize);
 ...
 * Enumerates all open handles.
 *
 * \param ProcessHandle A handle to the process. The handle must have
 * PROCESS_QUERY_INFORMATION access.
 * \param Handles A variable which receives a pointer to a structure containing
 * information about handles opened by the process. You must free the structure
 ...
 */
NTSTATUS
PhEnumProcessHandles(_In_ HANDLE ProcessHandle,
  ...
  status = NtQueryInformationProcess(ProcessHandle, ProcessHandleInformation,
                                     buffer, bufferSize, &returnLength);
  ...
  if (NT_SUCCESS(status)) {
    ...
    // NOTE: 21H1 and above no longer set NumberOfHandles to zero before
    // returning STATUS_SUCCESS so we first zero the entire buffer using
    // PhAllocateZero. (dmex)
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumHandlesGeneric(_In_ HANDLE ProcessId, _In_ HANDLE ProcessHandle,
  ...
  // * On Windows 8 and later, NtQueryInformationProcess with
  // ProcessHandleInformation is the most efficient method.
  // * On Windows XP and later, NtQuerySystemInformation with
  // SystemExtendedHandleInformation.
  // * Otherwise, NtQuerySystemInformation with SystemHandleInformation can be
  // used.
  ...
  if ((KsiLevel() >= KphLevelMed) && ProcessHandle) {
    ...
    // Enumerate handles using KSystemInformer. Unlike with
    // NtQuerySystemInformation, this only enumerates handles for a single
    // process and saves a lot of processing.

    if (NT_SUCCESS(status =
                       KsiEnumerateProcessHandles(ProcessHandle, &handles))) {
      convertedHandles = PhAllocate(UFIELD_OFFSET(
          SYSTEM_HANDLE_INFORMATION_EX, Handles[handles->HandleCount]));
      ...
      for (i = 0; i < handles->HandleCount; i++) {
        ...
        convertedHandles->Handles[i].HandleAttributes =
  ...
  if (!NT_SUCCESS(status) && WindowsVersion >= WINDOWS_8 && ProcessHandle &&
    ...
    if (NT_SUCCESS(status = PhEnumProcessHandles(ProcessHandle, &handles))) {
      convertedHandles = PhAllocate(UFIELD_OFFSET(
          SYSTEM_HANDLE_INFORMATION_EX, Handles[handles->NumberOfHandles]));
  ...
  if (!NT_SUCCESS(status)) {
    ...
    if (NT_SUCCESS(status = PhEnumHandlesEx(&handles))) {
      ...
      if (numberOfHandles) {
        convertedHandles = PhAllocate(UFIELD_OFFSET(
            SYSTEM_HANDLE_INFORMATION_EX, Handles[numberOfHandles]));
        convertedHandles->NumberOfHandles = numberOfHandles;

        if (lastIndex == firstIndex + numberOfHandles - 1) // consecutive
        {
          memcpy(&convertedHandles->Handles[0], &handles->Handles[firstIndex],
          ...
          *Handles = convertedHandles;
          ...
          for (i = firstIndex; i <= lastIndex; i++) {
            ...
            if (handle->UniqueProcessId == ProcessId) {
              memcpy(&convertedHandles->Handles[j++], handle,
 ...
 *
 * \param Pagefiles A variable which receives a pointer to a buffer containing
 * information about all active pagefiles. You must free the structure using
 * PhFree() when you no longer need it.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumPagefiles(_Out_ PVOID *Pagefiles) {
  ...
  while ((status = NtQuerySystemInformation(SystemPageFileInformation, buffer,
                                            bufferSize, NULL)) ==
         STATUS_INFO_LENGTH_MISMATCH) {
    PhFree(buffer);
    bufferSize *= 2;
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumPagefilesEx(_Out_ PVOID *Pagefiles) {
  NTSTATUS status;
  PVOID buffer;
  ULONG bufferSize = 0x200;
  ...
  while ((status = NtQuerySystemInformation(SystemPageFileInformationEx, buffer,
                                            bufferSize, NULL)) ==
         STATUS_INFO_LENGTH_MISMATCH) {
    PhFree(buffer);
    bufferSize *= 2;
 ...
 *
 * \param Buffer A pointer to a buffer that will receive the pool tag
 * information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumPoolTagInformation(_Out_ PVOID *Buffer) {
  NTSTATUS status;
  PVOID buffer;
  ULONG bufferSize;
  ULONG attempts;
  ...
  status = NtQuerySystemInformation(SystemPoolTagInformation, buffer,
                                    bufferSize, &bufferSize);
  attempts = 0;
 ...
 *
 * \param Buffer A pointer to a variable that receives the buffer containing the
 * big pool information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumBigPoolInformation(_Out_ PVOID *Buffer) {
  NTSTATUS status;
  PVOID buffer;
  ULONG bufferSize;
  ULONG attempts;
...
_Function_class_(PH_ENUM_KEY_CALLBACK) static BOOLEAN NTAPI
    PhEnumHostComputeServiceKeyCallback(_In_ HANDLE RootDirectory,
  ...
  if (string = PhConcatStringRef2(&objectNamePrefix, &keyName)) {
    ...
    if (NT_SUCCESS(PhOpenJobObject(&objectHandle, JOB_OBJECT_QUERY, NULL,
                                   &string->sr))) {
      ...
      if (NT_SUCCESS(PhGetJobProcessIdList(objectHandle, &processIdList))) {
        for (ULONG i = 0; i < processIdList->NumberOfProcessIdsInList; i++) {
          if (context->ProcessObject ==
              (HANDLE)processIdList->ProcessIdList[i]) {
            context->Found = TRUE;
            break;
          }
        }
      ...
      // JobObjectAssociateCompletionPortInformation for process start/stop
      // notifications (dmex)
...
NTSTATUS PhEnumHostComputeService(_In_ HANDLE ProcessId) {
  static CONST PH_STRINGREF keyName = PH_STRINGREF_INIT(
      L"Software\\Microsoft\\Windows "
      L"NT\\CurrentVersion\\HostComputeService\\VolatileStore\\ComputeSystem");
  NTSTATUS status;
  HANDLE keyHandle;
  ...
  if (NT_SUCCESS(status)) {
    ...
    status = PhEnumerateKey(keyHandle, KeyBasicInformation,
                            PhEnumHostComputeServiceKeyCallback, &context);
...
_Function_class_(PH_ENUM_DIRECTORY_OBJECTS) static NTSTATUS NTAPI
    PhIsContainerEnumCallback(_In_ HANDLE RootDirectory,
                              _In_ PPH_STRINGREF Name,
                              _In_ PPH_STRINGREF TypeName, _In_ PVOID Context) {
  PPH_IS_CONTAINER_CONTEXT context = (PPH_IS_CONTAINER_CONTEXT)Context;
  HANDLE objectHandle;
  ...
  if (NT_SUCCESS(PhOpenJobObject(&objectHandle, JOB_OBJECT_QUERY, RootDirectory,
                                 Name))) {
 ...
 * \param ProcessId The ID of the process.
 * \param ProcessHandle A handle to the process.
 * \param IsContainer A variable which receives a boolean indicating whether the
 * process is managed.
 * \return NTSTATUS Successful or errant status.
 */
_Use_decl_annotations_ NTSTATUS
PhGetProcessIsContainer(_In_ HANDLE ProcessId, _In_opt_ HANDLE ProcessHandle,
                        _Out_opt_ PBOOLEAN IsContainer) {
  {
    static CONST PH_STRINGREF keyName =
        PH_STRINGREF_INIT(L"Software\\Microsoft\\Windows "
                          L"NT\\CurrentVersion\\HostComputeService\\VolatileSto"
                          L"re\\ComputeSystem");
    HANDLE keyHandle;
    PH_IS_CONTAINER_CONTEXT context;
    ...
    if (NT_SUCCESS(PhOpenKey(&keyHandle, KEY_READ, PH_KEY_LOCAL_MACHINE,
                             &keyName, 0))) {
      PhEnumerateKey(keyHandle, KeyBasicInformation,
                     PhEnumHostComputeServiceKeyCallback, &context);
...

  {
    ...
    status = PhOpenProcess(&processHandle, PROCESS_QUERY_LIMITED_INFORMATION,
                           ProcessId);

    if (NT_SUCCESS(status)) {
      status = PhOpenDirectoryObject(&directoryHandle, DIRECTORY_QUERY, NULL,
                                     &directoryName);
      ...
      if (NT_SUCCESS(status)) {
        ...
        status = PhEnumDirectoryObjects(directoryHandle,
                                        PhIsContainerEnumCallback, NULL);
 ...
 *
 * \param ProcessId The ID of the process.
 * \param IsDotNet A variable which receives a boolean indicating whether the
 * process is managed.
 * \return NTSTATUS Successful or errant status.
 */
...
_Function_class_(PH_ENUM_PROCESS_MODULES_CALLBACK) static BOOLEAN NTAPI
    PhpIsDotNetEnumProcessModulesCallback(_In_ PLDR_DATA_TABLE_ENTRY Module,
                                          _In_ PVOID Context) {
  ...
  static CONST PH_STRINGREF mscorlibNiString =
      PH_STRINGREF_INIT(L"mscorlib.ni.dll");
  static CONST PH_STRINGREF frameworkString =
      PH_STRINGREF_INIT(L"\\Microsoft.NET\\Framework\\");
  static CONST PH_STRINGREF framework64String =
      PH_STRINGREF_INIT(L"\\Microsoft.NET\\Framework64\\");
  PH_STRINGREF baseDllName;

  PhUnicodeStringToStringRef(&Module->BaseDllName, &baseDllName);

  if (PhEqualStringRef(&baseDllName, &clrString, TRUE) ||
    ...
    if (PhStartsWithStringRef(&fileName, &systemRoot, TRUE)) {
      ...
      if (PhStartsWithStringRef(&fileName, frameworkPart, TRUE)) {
        fileName.Buffer =
            PTR_ADD_OFFSET(fileName.Buffer, frameworkPart->Length);
  ...
  } else if (PhEqualStringRef(&baseDllName, &mscorlibString, TRUE) ||
...
_Function_class_(PH_ENUM_DIRECTORY_FILE) static BOOLEAN NTAPI
    PhpDotNetCorePipeHashCallback(_In_ HANDLE RootDirectory,
  ...
  if (PhHashStringRefEx(&objectName, FALSE, PH_STRING_HASH_X65599) ==
      context->NameHash) {
    context->Found = TRUE;
    return FALSE;
  }
 ...
 * \li \c PH_CLR_USE_SECTION_CHECK Checks for the existence of related section
 * objects to determine whether the process is managed.
 * \li \c PH_CLR_NO_WOW64_CHECK Instead of a separate query, uses the presence
 * of the
 * \c PH_CLR_KNOWN_IS_WOW64 flag to determine whether the process is running
 * under WOW64.
...
NTSTATUS PhGetProcessIsDotNetEx(_In_ HANDLE ProcessId,
                                ...
                                _In_ ULONG InFlags, _Out_opt_ PBOOLEAN IsDotNet,
  ...
  if (InFlags & PH_CLR_USE_SECTION_CHECK) {
    ...
    // \BaseNamedObjects\Cor_Private_IPCBlock(_v4)_<ProcessId>. This is the same
    // object used by the ICorPublish::GetProcess function. Instead of calling
    // that function, we simply check for the existence of that section object.
    // This means:
    // * Better performance.
    // * No need for admin rights to get .NET status of processes owned by other
    // users.
    ...
    if (PhFormatToBuffer(format, RTL_NUMBER_OF(format), formatBuffer,
                         sizeof(formatBuffer), &returnLength)) {
    ...
    status = PhOpenSection(&sectionHandle, SECTION_QUERY, NULL, &objectName);
    ...
    if (PhFormatToBuffer(format, RTL_NUMBER_OF(format), formatBuffer,
      ...
      status = PhEnumDirectoryNamedPipe(
          &objectName, PhpDotNetCorePipeHashCallback, &context);
      ...
      // NOTE: NtQueryAttributesFile and other query functions connect to the
      // pipe and should be avoided. (dmex)
      //
    ...
    if (!ProcessHandle) {
      if (!NT_SUCCESS(status = PhOpenProcess(&processHandle,
    ...
    if (isWow64) {
      ...
      status = PhEnumProcessModules32(
          ProcessHandle, PhpIsDotNetEnumProcessModulesCallback, &flags);
    } else {
#endif
    ...
    if (IsDotNet)
      *IsDotNet = (flags & PH_CLR_VERSION_MASK) &&
                  (flags & (PH_CLR_MSCORLIB_PRESENT | PH_CLR_CORELIB_PRESENT));
 ...
 *
 * \param DirectoryHandle A variable which receives a handle to the directory
 * object.
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhOpenDirectoryObject(_Out_ PHANDLE DirectoryHandle,
                               _In_ ACCESS_MASK DesiredAccess,
                               _In_opt_ HANDLE RootDirectory,
                               _In_ PCPH_STRINGREF ObjectName) {
  NTSTATUS status;
 ...
 *
 * \param DirectoryHandle A handle to a directory. The handle must have
 * DIRECTORY_QUERY access.
...
NTSTATUS PhEnumDirectoryObjects(_In_ HANDLE DirectoryHandle,
  ...
  while (TRUE) {
    ...
    while ((status = NtQueryDirectoryObject(DirectoryHandle, buffer, bufferSize,
                                            FALSE, firstTime, &context,
                                            NULL)) == STATUS_MORE_ENTRIES) {
      // Check if we have at least one entry. If not, we'll double the buffer
      // size and try again.
    ...
    while (TRUE) {
      ...
      status = Callback(DirectoryHandle, &name, &typeName, Context);
 ...
 *
 * \param LinkHandle Pointer to a variable that receives the handle to the
 * symbolic link object.
 * \param DesiredAccess Specifies the desired access rights for the symbolic
 * link object.
 * \param RootDirectory Optional handle to the root directory for the symbolic
 * link object name.
 * \param FileName Pointer to a string that specifies the target file or object
 * to which the symbolic link points.
 * \param LinkName Pointer to a string that specifies the name of the symbolic
 * link object to be created.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhCreateSymbolicLinkObject(_Out_ PHANDLE LinkHandle,
                                    _In_ ACCESS_MASK DesiredAccess,
                                    _In_opt_ HANDLE RootDirectory,
                                    _In_ PCPH_STRINGREF FileName,
  ...
  status = NtCreateSymbolicLinkObject(&linkHandle, DesiredAccess,
                                      &objectAttributes, &objectTarget);
 ...
 *
 * \param LinkTarget Pointer to a variable that receives the target of the
 * symbolic link as a PPH_STRING.
 * \param RootDirectory Optional handle to the root directory for the object
 * name. Can be NULL.
 * \param ObjectName Pointer to a string reference that specifies the name of
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhQuerySymbolicLinkObject(_Out_ PPH_STRING *LinkTarget,
                                   _In_opt_ HANDLE RootDirectory,
                                   _In_ PCPH_STRINGREF ObjectName) {
  NTSTATUS status;
  HANDLE linkHandle;
  ...
  status = NtQuerySymbolicLinkObject(linkHandle, &targetName, &returnLength);
 ...
 */
VOID PhpInitializeDevicePrefixes(VOID) {
  ULONG i;
  PWCHAR buffer;
...
VOID PhUpdateMupDevicePrefixes(VOID) {
  static CONST PH_STRINGREF orderKeyName = PH_STRINGREF_INIT(
      L"System\\CurrentControlSet\\Control\\NetworkProvider\\Order");
  static CONST PH_STRINGREF servicesStringPart =
      PH_STRINGREF_INIT(L"System\\CurrentControlSet\\Services");
  static CONST PH_STRINGREF networkProviderStringPart =
      PH_STRINGREF_INIT(L"\\NetworkProvider");
  ...
  // Each name can then be looked up, its device name in the DeviceName value
  // in: HKLM\System\CurrentControlSet\Services\<ProviderName>\NetworkProvider

  // Note that we assume the providers only claim their device name. Some
  // providers such as DFS claim an extra part, and are not resolved correctly
  // here.

  if (NT_SUCCESS(PhOpenKey(&orderKeyHandle, KEY_READ, PH_KEY_LOCAL_MACHINE,
                           &orderKeyName, 0))) {
  ...
  PhDeviceMupPrefixes[PhDeviceMupPrefixesCount++] =
      PhCreateString(L"\\Device\\Mup");

  // DFS claims an extra part of file names, which we don't handle.
  // PhDeviceMupPrefixes[PhDeviceMupPrefixesCount++] =
  // PhCreateString(L"\\Device\\DfsClient");
  ...
  while (remainingPart.Length != 0) {
    ...
    if (part.Length != 0) {
      serviceKeyName =
          PhConcatStringRef4(&servicesStringPart, &PhNtPathSeparatorString,
                             &part, &networkProviderStringPart);

      if (NT_SUCCESS(PhOpenKey(&networkProviderKeyHandle, KEY_READ,
                               PH_KEY_LOCAL_MACHINE, &serviceKeyName->sr, 0))) {
        if (deviceName = PhQueryRegistryStringZ(networkProviderKeyHandle,
                                                L"DeviceName")) {
...
VOID PhUpdateDosDevicePrefixes(VOID) {
  ...
  for (ULONG i = 0; i < 0x1A; i++) {
    ...
    if (NT_SUCCESS(NtOpenSymbolicLinkObject(&linkHandle, SYMBOLIC_LINK_QUERY,
                                            &objectAttributes))) {
      if (!NT_SUCCESS(NtQuerySymbolicLinkObject(linkHandle,
                                                &PhDevicePrefixes[i], NULL))) {
        PhDevicePrefixes[i].Length = 0;
      }
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetVolumeMountPoints(_In_ HANDLE DeviceHandle,
  ...
  do {
    status = NtDeviceIoControlFile(
        DeviceHandle, NULL, NULL, NULL, &isb, IOCTL_MOUNTMGR_QUERY_POINTS,
...
/**
 * \brief Retrieves a list of drive letters and mounted folder paths for the
 * specified volume.
 *
 * \param DeviceHandle A handle to the MountPointManager.
 * \param VolumeName A volume GUID path for the volume.
 * \param VolumePathNames A pointer to a buffer that receives the list of drive
 * letters and mounted folder paths.
 * \a The list is an array of null-terminated strings terminated by an
 * additional NULL character.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetVolumePathNamesForVolumeName(
    _In_ HANDLE DeviceHandle, _In_ PCPH_STRINGREF VolumeName,
  ...
  inputBufferLength =
      UFIELD_OFFSET(MOUNTMGR_TARGET_NAME, DeviceName[VolumeName->Length]) +
      sizeof(UNICODE_NULL);
  inputBuffer = PhAllocateStack(
      inputBufferLength); // Volume{guid}, CM_Get_Device_Interface_List,
                          // SymbolicLinks, [??]
  if (!inputBuffer)
    return STATUS_NO_MEMORY;
  inputBuffer->DeviceNameLength = (USHORT)VolumeName->Length;
  RtlCopyMemory(inputBuffer->DeviceName, VolumeName->Buffer,
                VolumeName->Length);

  outputBufferLength =
      UFIELD_OFFSET(MOUNTMGR_VOLUME_PATHS, MultiSz[DOS_MAX_PATH_LENGTH]) +
      sizeof(UNICODE_NULL);
  outputBuffer = PhAllocate(outputBufferLength);

  do {
    status = NtDeviceIoControlFile(DeviceHandle, NULL, NULL, NULL, &isb,
    ...
    if (status == STATUS_BUFFER_OVERFLOW) {
      outputBufferLength =
          (outputBuffer->MultiSzLength * sizeof(WCHAR)) + sizeof(UNICODE_NULL);
      PhFree(outputBuffer);
      outputBuffer = PhAllocate(outputBufferLength);
    } else {
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhFlushVolumeCache(VOID) {
  NTSTATUS status;
  HANDLE deviceHandle;
  UNICODE_STRING objectName;
  OBJECT_ATTRIBUTES objectAttributes;
  ...
  status = NtCreateFile(
      &deviceHandle, FILE_READ_ATTRIBUTES | SYNCHRONIZE, &objectAttributes,
  ...
  status = PhGetVolumeMountPoints(deviceHandle, &objectMountPoints);
  ...
  for (ULONG i = 0; i < objectMountPoints->NumberOfMountPoints; i++) {
    ...
    objectName.MaximumLength =
        mountPoint->SymbolicLinkNameLength + sizeof(UNICODE_NULL);
    objectName.Buffer =
        PTR_ADD_OFFSET(objectMountPoints, mountPoint->SymbolicLinkNameOffset);
    ...
    if (MOUNTMGR_IS_VOLUME_NAME(&objectName)) // \\??\\Volume{1111-2222}
    {
      ...
      status = NtCreateFile(
          &volumeHandle, FILE_WRITE_DATA | SYNCHRONIZE, &objectAttributes,
          &ioStatusBlock, NULL, FILE_ATTRIBUTE_NORMAL,
          FILE_SHARE_READ | FILE_SHARE_WRITE, FILE_OPEN,
          FILE_NON_DIRECTORY_FILE | FILE_SYNCHRONOUS_IO_NONALERT, NULL, 0);
      ...
      if (NT_SUCCESS(status)) {
        ...
        //{
        //     status = NtFlushBuffersFileEx(volumeHandle, 0, 0, 0,
        //     &ioStatusBlock);
        // }
        // else
        {
...
//                 continue;
//             if (deviceName.Length >= PhDevicePrefixes[index].MaximumLength -
//             sizeof(UNICODE_NULL))
//                 continue;
//
...
//         //
//         //    if (NT_SUCCESS(PhGetVolumePathNamesForVolumeName(deviceHandle,
//         &volumeLinkName, &volumePaths)))
//         //    {
//         //        for (PWSTR path = volumePaths->MultiSz; *path; path +=
//         PhCountStringZ(path) + 1)
//         //        {
 ...
 *
 * \param Name A pointer to a constant PPH_STRINGREF structure that specifies
 * the file name to resolve.
 * \param NativeFileName A BOOLEAN value indicating whether the file name is in
 * native NT format (TRUE) or DOS/Win32 format (FALSE).
 * \return A pointer to a PPH_STRING structure containing the resolved mount
 * prefix, or NULL if the resolution fails.
 */
PPH_STRING PhResolveMountPrefix(_In_ PCPH_STRINGREF Name,
  ...
  status = PhGetVolumeMountPoints(deviceHandle, &mountPoints);
  ...
  for (ULONG i = 0; i < mountPoints->NumberOfMountPoints; i++) {
    ...
    if (NativeFileName) {
      if (PhStartsWithStringRef(Name, &devicePrefix, TRUE)) {
        if (PATH_IS_WIN32_DOSDEVICES_PREFIX(&linkPrefix) &&
            linkPrefix.Buffer[1] == L'?') {
          // \??\Volume -> \\?\Volume
          linkPrefix.Buffer[1] = OBJ_NAME_PATH_SEPARATOR;
        }

        // \\Device\\VhdHardDisk{12345678-abcd-1234-abcd-123456789abc} ->
        // \\\\?\\Volume{12345678-abcd-1234-abcd-123456789abc}

        newName = PhCreateStringEx(
            NULL, linkPrefix.Length + (Name->Length - devicePrefix.Length));
        memcpy(newName->Buffer, linkPrefix.Buffer, linkPrefix.Length);
        memcpy(PTR_ADD_OFFSET(newName->Buffer, linkPrefix.Length),
      ...
      if (PhStartsWithStringRef(Name, &linkPrefix, TRUE)) {
        ...
        newName = PhCreateStringEx(
            NULL, devicePrefix.Length + (Name->Length - linkPrefix.Length));
        memcpy(newName->Buffer, devicePrefix.Buffer, devicePrefix.Length);
        memcpy(PTR_ADD_OFFSET(newName->Buffer, devicePrefix.Length),
 ...
 * \return A pointer to a string containing the Win32 path. You must free the
 * string using PhDereferenceObject() when you no longer need it.
 */
PPH_STRING PhResolveDevicePrefix(_In_ PCPH_STRINGREF Name) {
  ULONG i;
  PPH_STRING newName = NULL;
  ...
  for (i = 0; i < 26; i++) {
    ...
    if (prefix.Length != 0) {
      if (PhStartsWithStringRef(Name, &prefix, TRUE)) {
        // To ensure we match the longest prefix, make sure the next character
        // is a backslash or the path is equal to the prefix.
    ...
    if (isPrefix) {
      // <letter>:path
      newName = PhCreateStringEx(NULL, 2 * sizeof(WCHAR) + Name->Length -
                                           prefix.Length);
      newName->Buffer[0] = (WCHAR)('A' + i);
      newName->Buffer[1] = L':';
      memcpy(&newName->Buffer[2], &Name->Buffer[prefix.Length / sizeof(WCHAR)],
    ...
    for (i = 0; i < PhDeviceMupPrefixesCount; i++) {
      ...
      if (prefixLength != 0) {
        if (PhStartsWithStringRef(Name, &PhDeviceMupPrefixes[i]->sr, TRUE)) {
          // To ensure we match the longest prefix, make sure the next character
          // is a backslash. Don't resolve if the name *is* the prefix.
          // Otherwise, we will end up with a useless string like "\".
      ...
      if (isPrefix) {
        // \path
        newName = PhCreateStringEx(NULL, 1 * sizeof(WCHAR) + Name->Length -
                                             prefixLength);
        newName->Buffer[0] = OBJ_NAME_PATH_SEPARATOR;
        memcpy(&newName->Buffer[1], &Name->Buffer[prefixLength / sizeof(WCHAR)],
  ...
  if (PhIsNullOrEmptyString(newName) &&
      (Name->Length != 0 && Name->Buffer[0] == OBJ_NAME_PATH_SEPARATOR)) {
 ...
 * \return A pointer to a string containing the Win32 file name. You must free
 * the string using PhDereferenceObject() when you no longer need it.
 * \remarks This function may convert NT object name paths to invalid ones. If
 * the path to be converted is not necessarily a file name, use
...
PPH_STRING PhGetFileName(_In_ PPH_STRING FileName) {
  ...
  if (PhStartsWithString2(FileName, L"\\??\\", FALSE)) {
    ...
    memcpy(newFileName->Buffer, &FileName->Buffer[4],
  ...
  // "\SystemRoot" means "C:\Windows".
    ...
    PhGetSystemRoot(&systemRoot);
    newFileName = PhCreateStringEx(NULL, systemRoot.Length + FileName->Length -
                                             11 * sizeof(WCHAR));
    memcpy(newFileName->Buffer, systemRoot.Buffer, systemRoot.Length);
    memcpy(PTR_ADD_OFFSET(newFileName->Buffer, systemRoot.Length),
           &FileName->Buffer[11], FileName->Length - 11 * sizeof(WCHAR));
  }
  // System32, SysWOW64, SysArm32, and SyChpe32 are all identical length, fixup
  // is the same
  else if (
    ...
    PhGetSystemRoot(&systemRoot);
    newFileName = PhCreateStringEx(
        NULL, systemRoot.Length + sizeof(UNICODE_NULL) + FileName->Length);
    memcpy(newFileName->Buffer, systemRoot.Buffer, systemRoot.Length);
    newFileName->Buffer[systemRoot.Length / sizeof(WCHAR)] =
        OBJ_NAME_PATH_SEPARATOR;
    memcpy(PTR_ADD_OFFSET(newFileName->Buffer,
                          systemRoot.Length + sizeof(UNICODE_NULL)),
    ...
    if (resolvedName) {
      ...
      if (PhStartsWithString2(newFileName, L"\\Windows", TRUE)) {
        ...
        PhGetSystemRoot(&systemRoot);
        newFileName =
            PhCreateStringEx(NULL, FileName->Length + 2 * sizeof(WCHAR));
 ...
 *
 * \param Name A pointer to a PPH_STRINGREF structure that contains the DOS path
 * to convert.
 * \return A PPH_STRING containing the converted NT path, or NULL if the
 * conversion fails.
 */
PPH_STRING PhDosPathNameToNtPathName(_In_ PCPH_STRINGREF Name) {
  PPH_STRING newName = NULL;
  PH_STRINGREF prefix;
  ULONG index;
  ...
  if (PATH_IS_WIN32_DRIVE_PREFIX(Name)) {
    ...
    if (prefix.Length != 0) {
      ...
      newName = PhCreateStringEx(NULL, prefix.Length + Name->Length -
                                           sizeof(WCHAR[2]));
      memcpy(newName->Buffer, prefix.Buffer, prefix.Length);
      memcpy(PTR_ADD_OFFSET(newName->Buffer, prefix.Length),
    ...
    if (prefix.Length != 0) {
      ...
      newName =
          PhCreateStringEx(NULL, prefix.Length + Name->Length +
                                     systemRoot.Length - sizeof(L"SystemRoot"));
      memcpy(newName->Buffer, prefix.Buffer, prefix.Length);
      memcpy(PTR_ADD_OFFSET(newName->Buffer, prefix.Length), systemRoot.Buffer,
             systemRoot.Length);
      memcpy(PTR_ADD_OFFSET(newName->Buffer, prefix.Length + systemRoot.Length),
 ...
 *
 * \param DosFileName The DOS-style file path to convert (e.g.,
 * "C:\\Windows\\System32").
 * \param NtFileName A pointer to a UNICODE_STRING structure that receives the
 * resulting NT-style file path.
 * \param FilePart If specified, receives a pointer to the file part of the path
 * (the final component).
 * \param RelativeName If specified, receives a pointer to a RTL_RELATIVE_NAME_U
 * structure that describes the relative name.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhDosLongPathNameToNtPathNameWithStatus(
  ...
  if (WindowsVersion >= WINDOWS_10_RS1 && PhAreLongPathsEnabled() &&
      RtlDosLongPathNameToNtPathName_U_WithStatus_Import()) {
    status = RtlDosLongPathNameToNtPathName_U_WithStatus_Import()(
        DosFileName, NtFileName, FilePart, RelativeName);
  } else {
    status = RtlDosPathNameToNtPathName_U_WithStatus(DosFileName, NtFileName,
 ...
 *
 * \param Name A pointer to a PPH_STRINGREF structure that contains the NT path
 * string.
 * \return A PPH_STRING representing the root prefix of the NT path, or NULL if
 * the prefix cannot be determined.
 */
PPH_STRING PhGetNtPathRootPrefix(_In_ PCPH_STRINGREF Name) {
 ...
 *
 * This function examines the provided string reference, which is expected to
 * represent a file or directory path, and returns a PPH_STRING containing the
 * longest existing prefix of that path. If no part of the path exists, the
 * function may return NULL or an empty string, depending on implementation.
 *
 * \param Name A pointer to a PPH_STRINGREF structure that contains the path to
 * be examined.
 * \return A PPH_STRING containing the longest existing prefix of the specified
 * path, or NULL if no prefix exists.
 */
PPH_STRING PhGetExistingPathPrefix(_In_ PCPH_STRINGREF Name) {
  ...
  while (remainingPart.Length != 0) {
    PhSplitStringRefAtLastChar(&remainingPart, OBJ_NAME_PATH_SEPARATOR,
                               &directoryPart, &baseNamePart);
  ...
  // if (PhEqualStringRef(&existingPathPrefix, PhGetNtPathRootPrefix(Name),
  // FALSE))
  //     return NULL;
...
/**
 * Retrieves the existing path prefix from a given string reference representing
 * a path.
 *
 * This function examines the provided path and returns the longest existing
 * prefix (i.e., the deepest directory that actually exists on the filesystem).
 *
 * \param Name A pointer to a PPH_STRINGREF structure that contains the path to
 * be checked.
 * \return A PPH_STRING containing the longest existing path prefix, or NULL if
 * no part of the path exists.
 */
PPH_STRING PhGetExistingPathPrefixWin32(_In_ PCPH_STRINGREF Name) {
  PPH_STRING existingPathPrefix = NULL;
  PH_STRINGREF remainingPart;
  PH_STRINGREF directoryPart;
  PH_STRINGREF baseNamePart;
...
/**
 * Retrieves the long (non-8.3) path form of the specified file or directory
 * name.
 *
 * \param FileName A pointer to a PPH_STRINGREF structure that contains the file
 * or directory name for which to retrieve the long path name.
 * \return A PPH_STRING containing the long path name if successful, or NULL if
 * the operation fails.
 */
PPH_STRING PhGetLongPathName(_In_ PCPH_STRINGREF FileName) {
  ...
  status = PhOpenFile(&fileHandle, FileName,
                      FILE_READ_DATA | FILE_LIST_DIRECTORY | SYNCHRONIZE, NULL,
  ...
  status = NtQueryDirectoryFile(fileHandle, NULL, NULL, NULL, &ioStatusBlock,
                                directoryInfoBuffer, directoryInfoLength,
                                FileBothDirectoryInformation, TRUE, &baseNameUs,
                                FALSE);

  if (NT_SUCCESS(status)) {
    longPathName = PhCreateStringEx(directoryInfoBuffer->FileName,
                                    directoryInfoBuffer->FileNameLength);
  }
 ...
 * \param IsWow64 Specifies whether the target process is running under WOW64
 * (TRUE) or is native (FALSE).
 * \param HeapSignature A pointer to a variable that receives the heap signature
 * value.
 * \return NTSTATUS Successful or errant status.
 * \remarks The heap signature is a validation field in the _HEAP structure used
 * to verify heap integrity. This function reads the SegmentSignature field at
 * different offsets depending on the process architecture:
 ...
 */
NTSTATUS PhGetProcessHeapSignature(_In_ HANDLE ProcessHandle,
                                   _In_ PVOID HeapAddress, _In_ ULONG IsWow64,
  ...
  if (WindowsVersion >= WINDOWS_7) {
    ...
    status = PhReadVirtualMemory(
        ProcessHandle, PTR_ADD_OFFSET(HeapAddress, IsWow64 ? 0x8 : 0x10),
        &heapSignature, sizeof(ULONG), NULL);
  }
 ...
 * \param HeapFrontEndType A pointer to a UCHAR that receives the front-end type
 * of the heap. Possible values include 0x0 (no front-end), 0x1 (LFH), etc.,
 * depending on the heap configuration.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetProcessHeapFrontEndType(_In_ HANDLE ProcessHandle,
  ...
  if (WindowsVersion >= WINDOWS_10) {
    ...
    status = PhReadVirtualMemory(
        ProcessHandle, PTR_ADD_OFFSET(HeapAddress, IsWow64 ? 0x0ea : 0x1a2),
        &heapFrontEndType, sizeof(UCHAR), NULL);
  } else if (WindowsVersion >= WINDOWS_8_1) {
    status = PhReadVirtualMemory(
        ProcessHandle, PTR_ADD_OFFSET(HeapAddress, IsWow64 ? 0x0d6 : 0x17a),
        &heapFrontEndType, sizeof(UCHAR), NULL);
  } else if (WindowsVersion >= WINDOWS_7) {
    status = PhReadVirtualMemory(
        ProcessHandle, PTR_ADD_OFFSET(HeapAddress, IsWow64 ? 0x0da : 0x182),
        &heapFrontEndType, sizeof(UCHAR), NULL);
  }
 ...
 *
 * \param ProcessId Handle to the process whose heap information is to be
 * queried.
 * \param HeapInformation Pointer to a variable that receives a pointer to a
 * structure containing heap information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhQueryProcessHeapInformation(
  ...
  if (!debugBuffer->Heaps) {
    // The RtlQueryProcessDebugInformation function has two bugs on some
    // versions when querying the ProcessId for a frozen (suspended) immersive
    // process. (dmex)
    //
  ...
  if (WindowsVersion > WINDOWS_11) {
    heapDebugInfo = PhAllocateZero(
        sizeof(PH_PROCESS_DEBUG_HEAP_INFORMATION) +
        ((PRTL_PROCESS_HEAPS_V2)debugBuffer->Heaps)->NumberOfHeaps *
            sizeof(PH_PROCESS_DEBUG_HEAP_ENTRY));
    heapDebugInfo->NumberOfHeaps =
        ((PRTL_PROCESS_HEAPS_V2)debugBuffer->Heaps)->NumberOfHeaps;
  } else {
    heapDebugInfo = PhAllocateZero(
        sizeof(PH_PROCESS_DEBUG_HEAP_INFORMATION) +
        ((PRTL_PROCESS_HEAPS_V1)debugBuffer->Heaps)->NumberOfHeaps *
            sizeof(PH_PROCESS_DEBUG_HEAP_ENTRY));
    heapDebugInfo->NumberOfHeaps =
        ((PRTL_PROCESS_HEAPS_V1)debugBuffer->Heaps)->NumberOfHeaps;
  }
  ...
  for (ULONG i = 0; i < heapDebugInfo->NumberOfHeaps; i++) {
    ...
    if (WindowsVersion > WINDOWS_11) {
      ...
      RTL_HEAP_INFORMATION_V1 heapInfoV1 =
          ((PRTL_PROCESS_HEAPS_V1)debugBuffer->Heaps)->Heaps[i];
    ...
    // go through all heap entries and compute amount of allocated and committed
    // bytes (ge0rdi)
    ...
    // sometimes computed number if committed bytes is few pages smaller than
    // the one reported by API, lets use the higher value (ge0rdi)
    if (committed < heapInfo.BytesCommitted)
      committed = heapInfo.BytesCommitted;

    // make sure number of allocated bytes is not higher than number of
    // committed bytes (as that would make no sense) (ge0rdi)
    ...
    if (NT_SUCCESS(PhOpenProcess(
            &processHandle, PROCESS_QUERY_LIMITED_INFORMATION | PROCESS_VM_READ,
            ProcessId))) {
      ...
      if (NT_SUCCESS(PhGetProcessHeapSignature(
              processHandle, heapInfo.BaseAddress, isWow64, &signature))) {
        heapDebugInfo->Heaps[i].Signature = signature;
      }

      if (NT_SUCCESS(PhGetProcessHeapFrontEndType(
              processHandle, heapInfo.BaseAddress, isWow64, &frontEndType))) {
...
/**
 * Queries lock information for a specified process and invokes the provided
 * callback for each lock associated with the process.
 *
 * \param ProcessId The handle to the process for which to query lock
 * information.
 * \param Callback A pointer to a callback function of type
 * PPH_ENUM_PROCESS_LOCKS that will be called for each lock.
 ...
 */
NTSTATUS PhQueryProcessLockInformation(_In_ HANDLE ProcessId,
  ...
  for (ULONG i = 0x400000;; i *= 2) // rev from Heap32First/Heap32Next (dmex)
  {
    ...
    status = RtlQueryProcessDebugInformation(ProcessId, RTL_QUERY_PROCESS_LOCKS,
                                             debugBuffer);
  ...
  status = Callback(debugBuffer->Locks->NumberOfLocks,
                    debugBuffer->Locks->Locks, Context);
 ...
 * \param Attributes A pointer to a MACHINE_ATTRIBUTES structure that receives
 * the attributes.
 * \return NTSTATUS indicating success or failure.
 * \remarks Queries if the specified architecture is supported on the current
 * system, either natively or by any form of compatibility or emulation layer.
 */
NTSTATUS PhGetMachineTypeAttributes(_In_ USHORT Machine,
  ...
  if (NT_SUCCESS(status)) {
    ...
    for (ULONG i = 0;
         i < returnLength /
                 sizeof(SYSTEM_SUPPORTED_PROCESSOR_ARCHITECTURES_INFORMATION);
         i++) {
 ...
 */
BOOLEAN PhIsFirmwareSupported(VOID) {
  static CONST UNICODE_STRING variableName = RTL_CONSTANT_STRING(L" ");
  static CONST GUID vendorGuid = {0};
  ULONG variableValueLength = 0;

  if (NtQuerySystemEnvironmentValueEx(&variableName, &vendorGuid, NULL,
                                      &variableValueLength,
                                      NULL) == STATUS_VARIABLE_NOT_FOUND) {
    return TRUE;
  }
 ...
 *
 * \param VariableName A pointer to a PH_STRINGREF structure that specifies the
 * name of the variable to retrieve.
 * \param VendorGuid A pointer to a PH_STRINGREF structure that specifies the
 * GUID of the firmware vendor.
 * \param ValueBuffer A pointer to a buffer that receives the value of the
 * environment variable.
 * \param ValueLength An optional pointer to a ULONG that receives the length of
 * the value returned in ValueBuffer.
 * \param ValueAttributes An optional pointer to a ULONG that receives the
 * attributes of the environment variable.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetFirmwareEnvironmentVariable(_In_ PCPH_STRINGREF VariableName,
  ...
  status = PhStringToGuid(VendorGuid, &vendorGuid);

  if (!NT_SUCCESS(status))
    return status;

  status = NtQuerySystemEnvironmentValueEx(&variableName, &vendorGuid, NULL,
                                           &valueLength, &valueAttributes);
  ...
  status = NtQuerySystemEnvironmentValueEx(
      &variableName, &vendorGuid, valueBuffer, &valueLength, &valueAttributes);
 ...
 * \param ValueBuffer An optional pointer to the buffer containing the value to
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhSetFirmwareEnvironmentVariable(_In_ PCPH_STRINGREF VariableName,
                                          _In_ PCPH_STRINGREF VendorGuid,
                                          _In_reads_bytes_opt_(ValueLength)
                                              PVOID ValueBuffer,
  ...
  status = NtSetSystemEnvironmentValueEx(&variableName, &vendorGuid,
                                         ValueBuffer, ValueLength, Attributes);

  return status;
}

/**
 * Enumerates firmware environment values based on the specified information
 * class.
 *
 * \param InformationClass The type of system environment information to
 * enumerate.
 * \param Variables A pointer to a variable that receives the enumerated
...
NTSTATUS PhEnumFirmwareEnvironmentValues(
  ...
  while (TRUE) {
    status = NtEnumerateSystemEnvironmentValuesEx(InformationClass, buffer,
                                                  &bufferLength);
 ...
 */
NTSTATUS PhSetSystemEnvironmentBootToFirmware(VOID) {
  static CONST GUID EFI_GLOBAL_VARIABLE_GUID = {
      0x8be4df61,
      0x93ca,
      0x11d2,
      {0xaa, 0x0d, 0x00, 0xe0, 0x98, 0x03, 0x2b, 0x8c}};
  static CONST UNICODE_STRING OsIndicationsSupportedName =
      RTL_CONSTANT_STRING(L"OsIndicationsSupported");
  static CONST UNICODE_STRING OsIndicationsName =
      RTL_CONSTANT_STRING(L"OsIndications");
  ...
  status = NtQuerySystemEnvironmentValueEx(
      &OsIndicationsSupportedName, &EFI_GLOBAL_VARIABLE_GUID,
      &osIndicationsSupported, &osIndicationsLength, NULL);

  if (status == STATUS_VARIABLE_NOT_FOUND ||
      !(osIndicationsSupported & EFI_OS_INDICATIONS_BOOT_TO_FW_UI)) {
    status = STATUS_NOT_SUPPORTED;
  }

  if (NT_SUCCESS(status)) {
    status = NtQuerySystemEnvironmentValueEx(
        &OsIndicationsName, &EFI_GLOBAL_VARIABLE_GUID, &osIndicationsValue,
        &osIndicationsLength, &osIndicationsAttributes);
...
/**
 * Create a new power request object. The process will continue to run instead
 * of being suspended or terminated by PLM (Process Lifetime Management). This
 * is mandatory on Windows 8 and above to prevent threads created by
 * DebugActiveProcess, QueueUserAPC and RtlQueryProcessDebug* functions from
 * deadlocking the current application.
 *
 * \param ProcessHandle A handle to the process for which the power request is
 * to be created.
 * \param PowerRequestHandle A pointer to a variable that receives a handle to
 * the new power request.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhCreateExecutionRequiredRequest(_In_ HANDLE ProcessHandle,
  ...
  status = NtPowerInformation(PowerRequestAction, &powerRequestAction,
                              sizeof(POWER_REQUEST_ACTION), NULL, 0);
...
NTSTATUS PhDestroyExecutionRequiredRequest(
  ...
  NtPowerInformation(PowerRequestAction, &requestPowerAction,
                     sizeof(POWER_REQUEST_ACTION), NULL, 0);
 ...
 *
 * \param ProcessorNumber The processor number structure identifying the target
 * processor.
 * \param NominalFrequency Pointer to a variable that receives the nominal
 * frequency of the processor.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS
PhGetProcessorNominalFrequency(_In_ PPH_PROCESSOR_NUMBER ProcessorNumber,
  ...
  frequencyInput.ProcessorNumber.Group =
      ProcessorNumber->Group; // USHRT_MAX for max
  frequencyInput.ProcessorNumber.Number =
      (BYTE)ProcessorNumber->Number;           // UCHAR_MAX for max
  ...
  status = NtPowerInformation(
      PowerInformationInternal, &frequencyInput,
      sizeof(POWER_INTERNAL_PROCESSOR_BRANDED_FREQUENCY_INPUT),
      &frequencyOutput,
      sizeof(POWER_INTERNAL_PROCESSOR_BRANDED_FREQUENCY_OUTPUT));

  if (NT_SUCCESS(status)) {
    if (frequencyOutput.Version ==
        POWER_INTERNAL_PROCESSOR_BRANDED_FREQUENCY_VERSION) {
 ...
 *
 * \param[out] ProcessStateChangeHandle A pointer to a handle that will receive
 * the state change handle for the frozen process.
 * \param[in] ProcessHandle The handle to the process to be frozen.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhFreezeProcess(_Out_ PHANDLE ProcessStateChangeHandle,
                         _In_ HANDLE ProcessHandle) {
  NTSTATUS status;
  OBJECT_ATTRIBUTES objectAttributes;
  ...
  InitializeObjectAttributes(&objectAttributes, NULL, OBJ_EXCLUSIVE, NULL,
                             NULL);

  status = NtCreateProcessStateChange_Import()(
      &processStateChangeHandle, STATECHANGE_SET_ATTRIBUTES, &objectAttributes,
      ProcessHandle, 0);
  ...
  status =
      NtChangeProcessState_Import()(processStateChangeHandle, ProcessHandle,
                                    ProcessStateChangeSuspend, NULL, 0, 0);
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhFreezeProcessById(_Out_ PHANDLE ProcessStateChangeHandle,
                             _In_ HANDLE ProcessId) {
  NTSTATUS status;
  HANDLE processHandle;
  ...
  status = PhFreezeProcess(&processStateChangeHandle, processHandle);
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhThawProcess(_In_ HANDLE ProcessStateChangeHandle,
                       _In_ HANDLE ProcessHandle) {
  NTSTATUS status;
  ...
  status =
      NtChangeProcessState_Import()(ProcessStateChangeHandle, ProcessHandle,
                                    ProcessStateChangeResume, NULL, 0, 0);
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhThawProcessById(_In_ HANDLE ProcessStateChangeHandle,
                           _In_ HANDLE ProcessId) {
  NTSTATUS status;
  HANDLE processHandle;
  ...
  status = PhThawProcess(ProcessStateChangeHandle, processHandle);
 ...
 *
 * \param[out] ThreadStateChangeHandle A pointer to a handle that receives the
 * thread state change handle.
 * \param[in] ThreadHandle A handle to the thread to be frozen.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhFreezeThread(_Out_ PHANDLE ThreadStateChangeHandle,
  ...
  status = NtCreateThreadStateChange_Import()(
      &threadStateChangeHandle, STATECHANGE_SET_ATTRIBUTES, &objectAttributes,
      ThreadHandle, 0);

  if (!NT_SUCCESS(status))
    return status;

  status = NtChangeThreadState_Import()(threadStateChangeHandle, ThreadHandle,
                                        ThreadStateChangeSuspend, NULL, 0, 0);
 ...
 *
 * \param ThreadStateChangeHandle A pointer to a handle that will receive the
 * thread state change handle.
 * \param ThreadId The ID of the thread to freeze.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhFreezeThreadById(_Out_ PHANDLE ThreadStateChangeHandle,
  ...
  status = PhOpenThread(
      &threadHandle, THREAD_SET_INFORMATION | THREAD_SUSPEND_RESUME, ThreadId);

  if (!NT_SUCCESS(status))
    return status;

  status = PhFreezeThread(&threadStateChangeHandle, threadHandle);
 ...
 * obtained from a prior suspend operation.
 * \param ThreadHandle A handle to the thread to be resumed.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhThawThread(_In_ HANDLE ThreadStateChangeHandle,
                      _In_ HANDLE ThreadHandle) {
  NTSTATUS status;
  ...
  status = NtChangeThreadState_Import()(ThreadStateChangeHandle, ThreadHandle,
                                        ThreadStateChangeResume, NULL, 0, 0);

  return status;
}

/**
 * Resumes (thaws) a suspended thread by its ID using a thread state change
 * handle.
 *
 * \param ThreadStateChangeHandle A handle to the thread state change object,
 * obtained from a previous suspend operation.
 * \param ThreadId The ID of the thread to resume.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhThawThreadById(_In_ HANDLE ThreadStateChangeHandle,
                          _In_ HANDLE ThreadId) {
  NTSTATUS status;
  HANDLE threadHandle;
  ...
  status = PhThawThread(ThreadStateChangeHandle, threadHandle);
...
_Function_class_(PH_HASHTABLE_EQUAL_FUNCTION) static BOOLEAN NTAPI
    PhExecutionRequestHashtableEqualFunction(_In_ PVOID Entry1,
                                             _In_ PVOID Entry2) {
  return ((PPH_EXECUTIONREQUEST_CACHE_ENTRY)Entry1)->ProcessId ==
         ((PPH_EXECUTIONREQUEST_CACHE_ENTRY)Entry2)->ProcessId;
}

_Function_class_(PH_HASHTABLE_HASH_FUNCTION) static ULONG NTAPI
    PhExecutionRequestHashtableHashFunction(_In_ PVOID Entry) {
  return HandleToUlong(((PPH_EXECUTIONREQUEST_CACHE_ENTRY)Entry)->ProcessId) /
         4;
}
 ...
 */
BOOLEAN PhInitializeExecutionRequestTable(VOID) {
  if (PhBeginInitOnce(&PhExecutionRequestInitOnce)) {
    PhExecutionRequestHashtable =
        PhCreateHashtable(sizeof(PH_EXECUTIONREQUEST_CACHE_ENTRY),
                          PhExecutionRequestHashtableEqualFunction,
                          PhExecutionRequestHashtableHashFunction, 1);
 ...
 *
 * This function checks if the process identified by the given process ID
 * requires execution, typically in the context of process management or
 * debugging.
 *
 * \param ProcessId A handle to the process ID to check.
 * \return TRUE if execution is required for the process, FALSE otherwise.
 */
BOOLEAN PhIsProcessExecutionRequired(_In_ HANDLE ProcessId) {
 ...
 *
 * \param ProcessId The handle to the process for which to enable execution
 * required.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhProcessExecutionRequiredEnable(_In_ HANDLE ProcessId) {
  NTSTATUS status;
  HANDLE processHandle;
  ...
  status =
      PhOpenProcess(&processHandle, PROCESS_SET_LIMITED_INFORMATION, ProcessId);
 ...
 *
 * \param ProcessId A handle to the process for which the execution required
 * state should be disabled.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhProcessExecutionRequiredDisable(_In_ HANDLE ProcessId) {
  if (PhInitializeExecutionRequestTable()) {
    ...
    if (entry =
            PhFindEntryHashtable(PhExecutionRequestHashtable, &lookupEntry)) {
...
_Function_class_(PH_HASHTABLE_EQUAL_FUNCTION) static BOOLEAN NTAPI
    PhKnownDllsHashtableEqualFunction(_In_ PVOID Entry1, _In_ PVOID Entry2) {
  return PhEqualStringRef(&((PPH_KNOWNDLL_CACHE_ENTRY)Entry1)->FileName->sr,
                          &((PPH_KNOWNDLL_CACHE_ENTRY)Entry2)->FileName->sr,
                          FALSE);
}

_Function_class_(PH_HASHTABLE_HASH_FUNCTION) static ULONG NTAPI
    PhKnownDllsHashtableHashFunction(_In_ PVOID Entry) {
  return PhHashStringRefEx(&((PPH_KNOWNDLL_CACHE_ENTRY)Entry)->FileName->sr,
                           FALSE, PH_STRING_HASH_XXH32);
}

_Function_class_(PH_ENUM_DIRECTORY_OBJECTS) static NTSTATUS NTAPI
    PhpKnownDllObjectsCallback(_In_ HANDLE RootDirectory,
                               _In_ PPH_STRINGREF Name,
                               _In_ PPH_STRINGREF TypeName,
                               _In_ PVOID Context) {
  NTSTATUS status;
  ...
  status = PhOpenSection(&sectionHandle, SECTION_MAP_READ, RootDirectory, Name);
  ...
  status = PhMapViewOfSection(sectionHandle, NtCurrentProcess(), &baseAddress,
                              0, NULL, &viewSize, ViewUnmap,
  ...
  status =
      PhGetProcessMappedFileName(NtCurrentProcess(), baseAddress, &fileName);
 ...
 * \param ObjectName A pointer to a constant string reference specifying the
 * object name.
 */
VOID PhInitializeKnownDlls(_In_ PCPH_STRINGREF ObjectName) {
  HANDLE directoryHandle;

  if (NT_SUCCESS(PhOpenDirectoryObject(&directoryHandle, DIRECTORY_QUERY, NULL,
                                       ObjectName))) {
    PhEnumDirectoryObjects(directoryHandle, PhpKnownDllObjectsCallback, NULL);
    NtClose(directoryHandle);
  }
}
 ...
 * \return TRUE if the Known DLLs table was successfully initialized, FALSE
 * otherwise.
 */
BOOLEAN PhInitializeKnownDllsTable(VOID) {
  if (PhBeginInitOnce(&PhKnownDllsInitOnce)) {
    static const PH_STRINGREF KnownDllsObjectName =
        PH_STRINGREF_INIT(L"\\KnownDlls");
    static const PH_STRINGREF KnownDlls32ObjectName =
        PH_STRINGREF_INIT(L"\\KnownDlls32");
#ifdef _ARM64_
    static const PH_STRINGREF KnownDllsArm32ObjectName =
        PH_STRINGREF_INIT(L"\\KnownDllsArm32");
    static const PH_STRINGREF KnownDllsChpe32ObjectName =
        PH_STRINGREF_INIT(L"\\KnownDllsChpe32");
#endif

    PhKnownDllsHashtable = PhCreateHashtable(
        sizeof(PH_KNOWNDLL_CACHE_ENTRY), PhKnownDllsHashtableEqualFunction,
        PhKnownDllsHashtableHashFunction, 10);
 ...
 */
BOOLEAN PhIsKnownDllFileName(_In_ PPH_STRING FileName) {
 ...
 *
 * \param Buffer A pointer to the performance distribution data for all
...
/**
 * Retrieves the processor performance distribution information for a specified
 * processor group.
 *
 * \param ProcessorGroup The processor group number for which to retrieve
 * performance distribution information.
 * \param Buffer A pointer to a variable that receives a pointer to a
 * SYSTEM_PROCESSOR_PERFORMANCE_DISTRIBUTION structure containing the
...
NTSTATUS PhGetSystemProcessorPerformanceDistributionEx(
  ...
  status = NtQuerySystemInformationEx(SystemProcessorPerformanceDistribution,
                                      &ProcessorGroup, sizeof(USHORT), buffer,
                                      bufferSize, &bufferSize);
  attempts = 0;
 ...
 *
 * \param RelationshipType The relationship type to filter the logical processor
 * information (e.g., processor core, NUMA node).
 * \param Buffer A pointer to a variable that receives a pointer to a buffer
 * containing an array of SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX structures.
 * \param BufferLength A pointer to a variable that on input specifies the size
 * of the buffer pointed to by Buffer, and on output receives the number of
 * bytes returned or required.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetSystemLogicalProcessorInformation(
  ...
  status = NtQuerySystemInformationEx(
      SystemLogicalProcessorAndGroupInformation, &RelationshipType,
      sizeof(LOGICAL_PROCESSOR_RELATIONSHIP), buffer, bufferSize, &bufferSize);
  attempts = 0;
  ...
  while (status == STATUS_INFO_LENGTH_MISMATCH && attempts < 8) {
    ...
    status = NtQuerySystemInformationEx(
        SystemLogicalProcessorAndGroupInformation, &RelationshipType,
        sizeof(LOGICAL_PROCESSOR_RELATIONSHIP), buffer, bufferSize,
        &bufferSize);
    attempts++;
  }
...
/**
 * Retrieves information about the logical processor relationships in the
 * system.
 *
 * \param LogicalProcessorInformation A pointer to a
 * PH_LOGICAL_PROCESSOR_INFORMATION structure that receives the logical
 * processor relationship information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetSystemLogicalProcessorRelationInformation(
  ...
  status = PhGetSystemLogicalProcessorInformation(
      RelationAll, &logicalInformation, &logicalInformationLength);
  ...
  if (NT_SUCCESS(status)) {
    ...
    for (PSYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX processorInfo =
             logicalInformation;
         (ULONG_PTR)processorInfo <
         (ULONG_PTR)PTR_ADD_OFFSET(logicalInformation,
      ...
      case RelationProcessorCore: {
        ...
        for (USHORT j = 0; j < processorInfo->Processor.GroupCount; j++) {
          processorLogicalCount +=
              PhCountBitsUlongPtr(processorInfo->Processor.GroupMask[j]
                                      .Mask); // RtlNumberOfSetBitsUlongPtr
        }
      } break;
    ...
    memset(LogicalProcessorInformation, 0,
           sizeof(PH_LOGICAL_PROCESSOR_INFORMATION));
 ...
 *
 * This function first checks if the current Windows version is older than a
 * specified version and if the requested processor feature index is within the
 * valid range. If so, it queries the processor feature from the user shared
 * data. Otherwise, it calls the system API `IsProcessorFeaturePresent` to
 * determine the presence of the feature.
 *
 ...
 */
BOOLEAN PhIsProcessorFeaturePresent(_In_ ULONG ProcessorFeature) {
  if (WindowsVersion < WINDOWS_NEW &&
      ProcessorFeature < PROCESSOR_FEATURE_MAX) {
    return USER_SHARED_DATA->ProcessorFeatures[ProcessorFeature];
  }

  return !!IsProcessorFeaturePresent(
      ProcessorFeature); // RtlIsProcessorFeaturePresent
}

/**
 * Retrieves the processor number information for the current processor.
 *
 * \param ProcessorNumber A pointer to a PROCESSOR_NUMBER structure that
 * receives the processor number details.
 */
VOID PhGetCurrentProcessorNumber(_Out_ PPROCESSOR_NUMBER ProcessorNumber) {
 ...
 *
 * \param ProcessorGroup The processor group for which to retrieve the active
 * processor count.
 * \return The number of active processors in the specified group.
 */
USHORT PhGetActiveProcessorCount(_In_ USHORT ProcessorGroup) {
  if (PhSystemProcessorInformation.ActiveProcessorCount) {
    USHORT numberOfProcessors = 0;

    if (ProcessorGroup == ALL_PROCESSOR_GROUPS) {
      for (USHORT i = 0;
           i < PhSystemProcessorInformation.NumberOfProcessorGroups; i++) {
        numberOfProcessors +=
            PhSystemProcessorInformation.ActiveProcessorCount[i];
      }
    } else {
      if (ProcessorGroup <
          PhSystemProcessorInformation.NumberOfProcessorGroups) {
        numberOfProcessors =
            PhSystemProcessorInformation.ActiveProcessorCount[ProcessorGroup];
      }
    }
...
/**
 * Retrieves the processor number structure corresponding to a given processor
 * index.
 *
 * \param ProcessorIndex The zero-based index of the processor.
 * \param ProcessorNumber A pointer to a PPH_PROCESSOR_NUMBER structure that
 * receives the processor number information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS
PhGetProcessorNumberFromIndex(_In_ ULONG ProcessorIndex,
                              _Out_ PPH_PROCESSOR_NUMBER ProcessorNumber) {
  USHORT processorIndex = 0;

  for (USHORT processorGroup = 0;
       processorGroup < PhSystemProcessorInformation.NumberOfProcessorGroups;
       processorGroup++) {
    USHORT processorCount = PhGetActiveProcessorCount(processorGroup);

    for (USHORT processorNumber = 0; processorNumber < processorCount;
         processorNumber++) {
 ...
 *
 * \param ProcessorGroup The processor group number for which to retrieve the
 * active affinity mask.
 * \param ActiveProcessorMask A pointer to a variable that receives the active
 * processor affinity mask for the specified group.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS
PhGetProcessorGroupActiveAffinityMask(_In_ USHORT ProcessorGroup,
  ...
  status = PhGetSystemLogicalProcessorInformation(
      RelationGroup, &processorInformation, &processorInformationLength);

  if (NT_SUCCESS(status)) {
    if (ProcessorGroup < processorInformation->Group.ActiveGroupCount) {
      *ActiveProcessorMask =
          processorInformation->Group.GroupInfo[ProcessorGroup]
              .ActiveProcessorMask;
    } else {
      status = STATUS_UNSUCCESSFUL;
    }
 ...
 *
 * \param[out] ActiveProcessorsAffinityMask Pointer to a variable that receives
 * the affinity mask representing active processors in the system.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetProcessorSystemAffinityMask(
  ...
  if (PhSystemProcessorInformation.SingleProcessorGroup) {
    *ActiveProcessorsAffinityMask =
        PhSystemBasicInformation.ActiveProcessorsAffinityMask;
    return STATUS_SUCCESS;
  } else {
    ...
    return PhGetProcessorGroupActiveAffinityMask(processorNumber.Group,
                                                 ActiveProcessorsAffinityMask);
  }
}
...
/**
 * Retrieves the highest NUMA (Non-Uniform Memory Access) node number available
 * on the system.
 *
 * \param[out] NodeNumber A pointer to a variable that receives the highest NUMA
 * node number.
 * \return Returns an NTSTATUS code indicating success or failure of the
 * operation.
 * \remarks The function queries the system for NUMA topology information and
 * stores the highest node number in the variable pointed to by NodeNumber. The
 * value is zero-based; for example, if the system has two NUMA nodes, the
 * highest node number will be 1.
 */
NTSTATUS PhGetNumaHighestNodeNumber(_Out_ PUSHORT NodeNumber) {
  NTSTATUS status;
  SYSTEM_NUMA_INFORMATION numaProcessorMap;

  status = NtQuerySystemInformation(SystemNumaProcessorMap, &numaProcessorMap,
                                    sizeof(SYSTEM_NUMA_INFORMATION), NULL);
 ...
 *
 * \param ProcessorNumber A pointer to a PPH_PROCESSOR_NUMBER structure that
 * specifies the processor for which to retrieve the NUMA node.
 * \param NodeNumber A pointer to a variable that receives the NUMA node number
 * associated with the specified processor.
 * \return Returns TRUE if the NUMA node number was successfully retrieved;
 * otherwise, returns FALSE.
 */
BOOLEAN PhGetNumaProcessorNode(_In_ PPH_PROCESSOR_NUMBER ProcessorNumber,
                               _Out_ PUSHORT NodeNumber) {
  NTSTATUS status;
  SYSTEM_NUMA_INFORMATION numaProcessorMap;
  USHORT processorNode = 0;

  if (ProcessorNumber->Group >= 20 ||
      ProcessorNumber->Number >= MAXIMUM_PROC_PER_GROUP) {
  ...
  while (numaProcessorMap.ActiveProcessorsGroupAffinity[processorNode].Group !=
             ProcessorNumber->Group ||
         (numaProcessorMap.ActiveProcessorsGroupAffinity[processorNode].Mask &
          AFFINITY_MASK(ProcessorNumber->Number)) == 0) {
...
/**
 * Retrieves the NUMA (Non-Uniform Memory Access) node number associated with a
 * given proximity ID.
 *
 * \param ProximityId The proximity ID for which to retrieve the corresponding
 * NUMA node number.
 * \param NodeNumber A pointer to a variable that receives the NUMA node number
 * associated with the specified proximity ID.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetNumaProximityNode(_In_ ULONG ProximityId,
                                _Out_ PUSHORT NodeNumber) {
  NTSTATUS status;
  ...
  status = NtQuerySystemInformation(SystemNumaProximityNodeInformation,
                                    &numaProximityMap,
                                    sizeof(SYSTEM_NUMA_PROXIMITY_MAP), NULL);
...
/**
 * Provides an efficient mechanism to bring into memory potentially
 * discontiguous virtual address ranges in a process address space.
 *
 * \param ProcessHandle A handle to the process whose virtual address ranges are
 * to be prefetched.
 * \param NumberOfEntries Number of entries in the array pointed to by the
 * VirtualAddresses parameter.
 * \param VirtualAddresses A pointer to an array of MEMORY_RANGE_ENTRY
 * structures which each specify a virtual address range to be prefetched. The
 * virtual address ranges may cover any part of the process address space
 * accessible by the target process.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhPrefetchVirtualMemory(_In_ HANDLE ProcessHandle,
  ...
  status = PhpSetInformationVirtualMemory(
      ProcessHandle, VmPrefetchInformation, NumberOfEntries, VirtualAddresses,
      &prefetchInformationFlags, sizeof(prefetchInformationFlags));
 ...
 *
 * \param ProcessHandle Handle to the process whose memory pages will be
 * modified.
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhSetVirtualMemoryPagePriority(_In_ HANDLE ProcessHandle,
  ...
  status = PhpSetInformationVirtualMemory(
      ProcessHandle, VmPagePriorityInformation, 1, &virtualMemoryRange,
      &PagePriority, sizeof(PagePriority));
...
//    cfgCallTargetInfo.Offset = (ULONG_PTR)VirtualAddress -
//    (ULONG_PTR)basicInfo.AllocationBase; cfgCallTargetInfo.Flags =
...
//
//    memset(&cfgCallTargetListInfo, 0,
//    sizeof(CFG_CALL_TARGET_LIST_INFORMATION));
//    cfgCallTargetListInfo.NumberOfEntries = 1;
//    cfgCallTargetListInfo.Reserved = 0;
//    cfgCallTargetListInfo.NumberOfEntriesProcessed =
//    &numberOfEntriesProcessed; cfgCallTargetListInfo.CallTargetInfo =
//    &cfgCallTargetInfo;
//
//    status = PhpSetInformationVirtualMemory(
//        ProcessHandle,
...
/**
 * Grants suppressed call access to a specific virtual address in the target
 * process.
 *
 * \param ProcessHandle Handle to the process in which access is to be granted.
 * \param VirtualAddress Pointer to the virtual address for which suppressed
 * call access is requested.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGuardGrantSuppressedCallAccess(_In_ HANDLE ProcessHandle,
                                          _In_ PVOID VirtualAddress) {
  NTSTATUS status;
  MEMORY_RANGE_ENTRY cfgCallTargetRangeInfo;
  CFG_CALL_TARGET_INFO cfgCallTargetInfo;
  ...
  status = PhpSetInformationVirtualMemory(
      ProcessHandle, VmCfgCallTargetInformation, 1, &cfgCallTargetRangeInfo,
      &cfgCallTargetListInfo, sizeof(CFG_CALL_TARGET_LIST_INFORMATION));
...
/**
 * Disables Control Flow Guard (CFG/XFG) protection on a specified memory region
 * in a target process.
 *
 * \param ProcessHandle Handle to the target process.
 * \param VirtualAddress Pointer to the base address of the memory region to
 * modify.
 * \return NTSTATUS code indicating success or failure of the operation.
 */
NTSTATUS PhDisableXfgOnTarget(_In_ HANDLE ProcessHandle,
                              _In_ PVOID VirtualAddress) {
  NTSTATUS status;
 ...
 *
 * \param[out] SystemCompressionStoreInformation A pointer to the compression
 * store information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetSystemCompressionStoreInformation(
    _Out_ PPH_SYSTEM_STORE_COMPRESSION_INFORMATION
        SystemCompressionStoreInformation) {
  NTSTATUS status;
  ...
  status = NtQuerySystemInformation(SystemStoreInformation, &storeInfo,
                                    sizeof(SYSTEM_STORE_INFORMATION), NULL);

  if (NT_SUCCESS(status)) {
    memset(SystemCompressionStoreInformation, 0,
           sizeof(PH_SYSTEM_STORE_COMPRESSION_INFORMATION));
    SystemCompressionStoreInformation->CompressionPid =
        compressionInfo.CompressionPid;
    SystemCompressionStoreInformation->WorkingSetSize =
        compressionInfo.WorkingSetSize;
    SystemCompressionStoreInformation->TotalDataCompressed =
        compressionInfo.TotalDataCompressed;
    SystemCompressionStoreInformation->TotalCompressedSize =
        compressionInfo.TotalCompressedSize;
    SystemCompressionStoreInformation->TotalUniqueDataCompressed =
        compressionInfo.TotalUniqueDataCompressed;
  }
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhSystemCompressionStoreTrimRequest(VOID) {
  NTSTATUS status;
  SYSTEM_STORE_INFORMATION storeInfo;
...
/**
 * Requests to set or clear high memory priority for the system compression
 * store of a process.
 *
 * \param ProcessHandle Handle to the target process.
 * \param SetHighMemoryPriority TRUE to set high memory priority, FALSE to clear
 * it.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhSystemCompressionStoreHighMemoryPriorityRequest(
    _In_ HANDLE ProcessHandle, _In_ BOOLEAN SetHighMemoryPriority) {
  NTSTATUS status;
  SYSTEM_STORE_INFORMATION storeInfo;
...
/**
 * Retrieves the current size limits for the working set of the virtual memory
 * manager system cache.
 *
 * \param CacheInfo A pointer to a variable that receives the file cache
 * information.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS
PhGetSystemFileCacheSize(_Out_ PSYSTEM_FILECACHE_INFORMATION CacheInfo) {
  return NtQuerySystemInformation(SystemFileCacheInformationEx, CacheInfo,
                                  sizeof(SYSTEM_FILECACHE_INFORMATION), 0);
}

// rev from SetSystemFileCacheSize (MSDN) (dmex)
/**
 * Limits the size of the working set of the virtual memory manager system
 * cache.
 *
 * \param CacheInfo The minimum size of the file cache, in bytes. The virtual
 * memory manager attempts to keep at least this much memory resident in the
 * system file cache.
 * \param CacheInfo The maximum size of the file cache, in bytes. The virtual
 * memory manager enforces this limit only if this call or a previous call to
 * SetSystemFileCacheSize specifies FILE_CACHE_MAX_HARD_ENABLE.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhSetSystemFileCacheSize(_In_ SIZE_T MinimumFileCacheSize,
  ...
  status = NtSetSystemInformation(SystemFileCacheInformationEx, &cacheInfo,
...
/**
 * Creates an event object, sets the initial state of the event to the specified
 * value, and opens a handle to the object with the specified desired access.
 *
 * \param EventHandle A pointer to a variable that receives the event object
 * handle.
 * \param DesiredAccess The access mask that specifies the requested access to
 * the event object.
 * \param EventType The type of the event, which can be SynchronizationEvent or
 * a NotificationEvent.
 * \param InitialState The initial state of the event object.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhCreateEvent(_Out_ PHANDLE EventHandle,
                       _In_ ACCESS_MASK DesiredAccess,
  ...
  status = NtCreateEvent(&eventHandle, DesiredAccess, &objectAttributes,
 ...
 * identifies the specific operation to be performed and the type of device on
 * which to perform it.
 ...
 * \param ReturnLength A pointer to a variable that receives the size of the
 * data stored in the output buffer, in bytes.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhDeviceIoControlFile(
    _In_ HANDLE DeviceHandle, _In_ ULONG IoControlCode,
    _In_reads_bytes_opt_(InputBufferLength) PVOID InputBuffer,
    _In_ ULONG InputBufferLength,
    _Out_writes_bytes_to_opt_(OutputBufferLength, *ReturnLength)
        PVOID OutputBuffer,
  ...
  if (DEVICE_TYPE_FROM_CTL_CODE(IoControlCode) == FILE_DEVICE_FILE_SYSTEM) {
    status = NtFsControlFile(DeviceHandle, NULL, NULL, NULL, &ioStatusBlock,
                             IoControlCode, InputBuffer, InputBufferLength,
                             OutputBuffer, OutputBufferLength);
  } else {
    status = NtDeviceIoControlFile(
        DeviceHandle, NULL, NULL, NULL, &ioStatusBlock, IoControlCode,
        InputBuffer, InputBufferLength, OutputBuffer, OutputBufferLength);
  }
 ...
 *
 * \param Machine The machine architecture identifier (e.g.,
 * IMAGE_FILE_MACHINE_AMD64, IMAGE_FILE_MACHINE_I386).
 * \param IncludePathSeperator If TRUE, appends a path separator to the returned
 * path.
 * \param SystemPath A pointer to a PPH_STRINGREF that receives the selected
 * System32 path.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhWow64SelectSystem32Path(_In_ USHORT Machine,
...
CreateResult:
  ...
  PhInitializeStringRefLongHint(SystemPath,
                                WithSeperators); // RtlInitUnicodeString
  return STATUS_SUCCESS;
}
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumVirtualMemory(_In_ HANDLE ProcessHandle,
  ...
  while (TRUE) {
    status = NtQueryVirtualMemory(ProcessHandle, baseAddress,
                                  MemoryBasicInformation, &basicInfo,
    ...
    if ((ULONG_PTR)baseAddress >=
        PhSystemBasicInformation.MaximumUserModeAddress)
      break;
  }
...
/**
 * Retrieves information about a range of pages within the virtual address space
 * of a specified process in batches for improved performance.
 *
 * \param ProcessHandle A handle to a process.
 * \param BaseAddress The base address at which to begin retrieving information.
 * \param BulkQuery A boolean indicating the mode of bulk query (accuracy vs
 * reliability).
 * \param Callback A callback function which is executed for each memory region.
 * \param Context A user-defined value to pass to the callback function.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumVirtualMemoryBulk(_In_ HANDLE ProcessHandle,
  ...
  // * Unsuitable for low-memory situations and fails with insufficient system
  // resources.
  // * ...
  //
  // BulkQuery... FALSE:
  ...
  // * Suitable for low-memory situations and doesn't fail with insufficient
  // system resources.
  // * ...
  ...
  if (BulkQuery) {
    ...
    bufferLength = sizeof(NTPSS_MEMORY_BULK_INFORMATION) +
                   sizeof(MEMORY_BASIC_INFORMATION[20]);
    buffer = PhAllocate(bufferLength);
    ...
    while ((status = NtPssCaptureVaSpaceBulk_Import()(
                ProcessHandle, BaseAddress, buffer, bufferLength, NULL)) ==
           STATUS_MORE_ENTRIES) {
      PhFree(buffer);
      bufferLength *= 2;
    ...
    UCHAR stackBuffer[sizeof(NTPSS_MEMORY_BULK_INFORMATION) +
                      sizeof(MEMORY_BASIC_INFORMATION[20])];
    SIZE_T bufferLength;
    PNTPSS_MEMORY_BULK_INFORMATION buffer;
    PMEMORY_BASIC_INFORMATION information;
    ...
    while (TRUE) {
      ...
      status = NtPssCaptureVaSpaceBulk_Import()(
          ProcessHandle, buffer->NextValidAddress, buffer, bufferLength, NULL);
      ...
      if (!NT_SUCCESS(Callback(ProcessHandle, information,
                               buffer->NumberOfEntries, Context)))
        break;
...
/**
 * Retrieves information about the pages currently added to the working set of
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumVirtualMemoryPages(_In_ HANDLE ProcessHandle,
                                  _In_ PPH_ENUM_MEMORY_PAGE_CALLBACK Callback,
                                  _In_opt_ PVOID Context) {
  NTSTATUS status;
  PMEMORY_WORKING_SET_INFORMATION pageInfo;

  status = PhGetProcessWorkingSetInformation(ProcessHandle, &pageInfo);

  if (NT_SUCCESS(status)) {
    status = Callback(ProcessHandle, pageInfo->NumberOfEntries,
                      pageInfo->WorkingSetInfo, Context);

    // for (ULONG_PTR i = 0; i < pageInfo->NumberOfEntries; i++)
    //{
    //     PMEMORY_WORKING_SET_BLOCK workingSetBlock =
    //     &pageInfo->WorkingSetInfo[i]; PVOID virtualAddress =
    //     (PVOID)(workingSetBlock->VirtualPage << PAGE_SHIFT);
    // }
 ...
 * \param[in] BaseAddress The starting virtual address from which page
 * information should be queried.
 * \param[in] Size The total size, in bytes, of the address range to examine.
 * \param[in] Callback A user-supplied callback invoked once for each page in
 * the range.
 * \param[in] Context A user-defined value passed to the callback function.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumVirtualMemoryAttributes(
    _In_ HANDLE ProcessHandle, _In_ PVOID BaseAddress, _In_ SIZE_T Size,
    _In_ PPH_ENUM_MEMORY_ATTRIBUTE_CALLBACK Callback, _In_opt_ PVOID Context) {
  NTSTATUS status = STATUS_SUCCESS;
  ...
  info = PhAllocatePage(
      numberOfPages * sizeof(MEMORY_WORKING_SET_EX_INFORMATION), NULL);
  ...
  status = NtQueryVirtualMemory(
      ProcessHandle, NULL, MemoryWorkingSetExInformation, info,
      numberOfPages * sizeof(MEMORY_WORKING_SET_EX_INFORMATION), NULL);

  if (NT_SUCCESS(status)) {
    status = Callback(ProcessHandle, BaseAddress, Size, numberOfPages, info,
                      Context);
  }
 ...
 *
 * \param[out] KernelDebuggerEnabled Optional. Receives a BOOLEAN value that
 * indicates whether the kernel debugger is enabled.
 * \param[out] KernelDebuggerPresent Optional. Receives a BOOLEAN value that
 * indicates whether the kernel debugger is present.
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS
PhGetKernelDebuggerInformation(_Out_opt_ PBOOLEAN KernelDebuggerEnabled,
                               _Out_opt_ PBOOLEAN KernelDebuggerPresent) {
  NTSTATUS status;
  SYSTEM_KERNEL_DEBUGGER_INFORMATION debugInfo;

  status = NtQuerySystemInformation(SystemKernelDebuggerInformation, &debugInfo,
 ...
 */
BOOLEAN PhIsDebugPortPresent(VOID) {
  BOOLEAN isBeingDebugged;

  if (NT_SUCCESS(
          PhGetProcessIsBeingDebugged(NtCurrentProcess(), &isBeingDebugged))) {
    if (isBeingDebugged) {
      return TRUE;
    }
  }
 ...
 * debugger.
 *
 * \return TRUE if the current process is running in the context of a debugger,
 * otherwise the return value is FALSE.
 */
BOOLEAN PhIsDebuggerPresent(VOID) {
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetDeviceType(_In_opt_ HANDLE ProcessHandle, _In_ HANDLE FileHandle,
  ...
  status = PhQueryVolumeInformationFile(
      ProcessHandle, FileHandle, FileFsDeviceInformation, &debugInfo,
      sizeof(FILE_FS_DEVICE_INFORMATION), &isb);
 ...
 *
 * \param FileName A pointer to a PPH_STRING structure containing the file name
 * to check.
 * \return TRUE if the file name is an App Execution Alias target, FALSE
 * otherwise.
 */
BOOLEAN PhIsAppExecutionAliasTarget(_In_ PPH_STRING FileName) {
  ...
  if (!NT_SUCCESS(PhCreateFileWin32(
          &fileHandle, PhGetString(FileName),
  ...
  if (NT_SUCCESS(NtFsControlFile(fileHandle, NULL, NULL, NULL, &isb,
                                 FSCTL_GET_REPARSE_POINT, NULL, 0,
                                 reparseBuffer, reparseLength))) {
    if (IsReparseTagMicrosoft(reparseBuffer->ReparseTag) &&
      ...
      for (ULONG i = 0; i < reparseBuffer->AppExecLinkReparseBuffer.StringCount;
           i++) {
 ...
 *
 * \param ProcessHandle Handle to the process whose enclaves are to be
 * enumerated.
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhEnumProcessEnclaves(_In_ HANDLE ProcessHandle,
  ...
  status = PhReadVirtualMemory(ProcessHandle, LdrEnclaveList, &enclaveList,
                               sizeof(LIST_ENTRY), NULL);
  if (!NT_SUCCESS(status))
    return status;

  for (PLIST_ENTRY link = enclaveList.Flink; link != LdrEnclaveList;
       link = enclave.Links.Flink) {
    PVOID enclaveAddress;

    enclaveAddress = CONTAINING_RECORD(link, LDR_SOFTWARE_ENCLAVE, Links);

    status = PhReadVirtualMemory(ProcessHandle, link, &enclave, sizeof(enclave),
                                 NULL);
    if (!NT_SUCCESS(status))
      return status;
...
VOID PhEcContextToNativeContext(_Out_ PCONTEXT Context,
  ...
  Context->Cpsr |=
      ((EcContext->AMD64_EFlags & 0x00000800) << 4); // Direction Flag
  ...
  Context->Fpcr =
      (EcContext->AMD64_MxCsr & 0x00000080) == 0; // IM: Invalid Operation Mask
  Context->Fpcr |= ((EcContext->AMD64_MxCsr & 0x00000200) == 0)
                   << 1; // ZM: Divide-by-Zero Mask
  Context->Fpcr |= ((EcContext->AMD64_MxCsr & 0x00000400) == 0)
                   << 2; // OM: Overflow Mask
  Context->Fpcr |= ((EcContext->AMD64_MxCsr & 0x00000800) == 0)
                   << 3; // UM: Underflow Mask
  Context->Fpcr |= ((EcContext->AMD64_MxCsr & 0x00001000) == 0)
                   << 4; // PM: Precision Mask
  Context->Fpcr |= (EcContext->AMD64_MxCsr & 0x00000040)
                   << 5; // DAZ: Denormals Are Zero Mask
  Context->Fpcr |= ((EcContext->AMD64_MxCsr & 0x00000100) == 0)
                   << 7; // DM: Denormal Operation Mask
  Context->Fpcr |=
      (EcContext->AMD64_MxCsr & 0x00002000); // FZ: Flush to Zero Mask
  Context->Fpcr |=
      (EcContext->AMD64_MxCsr & 0x0000C000); // RC: Rounding Control
  Context->Fpcr <<= 8;

  Context->Fpsr = EcContext->AMD64_MxCsr & 1; // IE: Invalid Operation Flag
  Context->Fpsr |= (EcContext->AMD64_MxCsr & 2) << 6; // DE: Denormal Flag
  Context->Fpsr |=
      (EcContext->AMD64_MxCsr >> 1) &
      0x1E; // ZE | OE | UE | PE: Zero, Overflow, Underflow, Precision Flags
...
VOID PhNativeContextToEcContext(_When_(InitializeEc, _Out_)
                                    _When_(!InitializeEc, _Inout_)
                                        PARM64EC_NT_CONTEXT EcContext,
  ...
  EcContext->AMD64_MxCsr_copy |=
      (Context->Fpsr & 0x00000080) >> 6; // IM: Invalid Operation Mask
  EcContext->AMD64_MxCsr_copy |=
      (Context->Fpcr & 0x00400000) >> 8; // ZM: Divide-by-Zero Mask
  EcContext->AMD64_MxCsr_copy |=
      (Context->Fpcr & 0x01000000) >> 9; // OM: Overflow Mask
  EcContext->AMD64_MxCsr_copy |=
      (Context->Fpcr & 0x00080000) >> 7; // UM: Underflow Mask
  EcContext->AMD64_MxCsr_copy |=
      (Context->Fpcr & 0x00800000) >> 7; // PM: Precision Mask
  EcContext->AMD64_MxCsr_copy |= (Context->Fpsr & 0x0000001E)
                                 << 1; // Other Flags
  EcContext->AMD64_MxCsr_copy |= ((Context->Fpcr & 0x00000100) == 0)
                                 << 7; // DAZ: Denormals Are Zeros
  EcContext->AMD64_MxCsr_copy |= ((Context->Fpcr & 0x00008000) == 0)
                                 << 8; // DM: Denormal Operation Mask
  EcContext->AMD64_MxCsr_copy |= ((Context->Fpcr & 0x00000200) == 0)
                                 << 9; // FZ: Flush to Zero
  EcContext->AMD64_MxCsr_copy |= ((Context->Fpcr & 0x00000400) == 0)
                                 << 10; // RC: Rounding Control
  EcContext->AMD64_MxCsr_copy |= ((Context->Fpcr & 0x00000800) == 0)
                                 << 11; // RC: Rounding Control
  EcContext->AMD64_MxCsr_copy |= ((Context->Fpcr & 0x00001000) == 0)
                                 << 12; // RC: Rounding Control
  EcContext->AMD64_MxCsr_copy |= Context->Fpsr & 0x00000001;

  EcContext->AMD64_EFlags &= 0xFFFFF63E;
  EcContext->AMD64_EFlags |=
      ((Context->Cpsr & 0x200000) >> 13); // Overflow Flag
  EcContext->AMD64_EFlags |=
      ((Context->Cpsr & 0x10000000) >> 17); // Direction Flag
  EcContext->AMD64_EFlags |=
      (((Context->Cpsr >> 5) & 0x1000000) >> 20); // Carry Flag
  EcContext->AMD64_EFlags |=
      ((Context->Cpsr & 0xC0FFFFFF) >> 20); // Other Flags
...
NTSTATUS PhIsEcCode(_In_ HANDLE ProcessHandle, _In_ PVOID CodePointer,
  ...
  *IsEcCode = FALSE;
  ...
  if ((ULONG_PTR)CodePointer > 0x00007ffffffeffff ||
      (ULONG_PTR)CodePointer < 0x0000000000010000)
  ...
  if (!NT_SUCCESS(
          status = PhReadVirtualMemory(
              ...
              &ecCodeBitMap, sizeof(PVOID), NULL)))
    return status;
  ...
  if (!NT_SUCCESS(status = PhReadVirtualMemory(ProcessHandle, ecCodeBitMap,
                                               &bitmap, sizeof(ULONG64), NULL)))
    return status;
 ...
 * \return NTSTATUS Successful or errant status.
 */
NTSTATUS PhGetFileMotw(_In_ PCPH_STRINGREF FileName,
                       _Out_opt_ PPH_MOTW_ZONE_ID ZoneId,
                       _Out_opt_ PPH_STRING *ReferrerUrl,
                       _Out_opt_ PPH_STRING *HostUrl) {
  static const PH_STRINGREF zoneIdentifierStream =
      PH_STRINGREF_INIT(L":Zone.Identifier");
  ...
  if (NT_SUCCESS(status = PhCreateFile(
    ...
    if (NT_SUCCESS(status = PhGetFileText(&fileText, fileHandle, TRUE))) {
      ...
      while (remaining.Length) {
        PhSplitStringRefAtString(&remaining, &newLine, FALSE, &line,
                                 &remaining);
        ...
        if (zoneId == ULONG_MAX && PhEqualStringRef(&key, &zoneIdKey, TRUE)) {
          ...
          if (zoneId > PhMotwZoneIdRestrictedSites)
            zoneId = PhMotwZoneIdUnknown;
                   ...
                   PhEqualStringRef(&key, &referrerUrlKey, TRUE)) {
