...
static PVOID json_get_object(_In_ PVOID Object, _In_ PCSTR Key) {
...
static NTSTATUS PhJsonErrorToNtStatus(_In_ enum json_tokener_error JsonError) {
  switch (JsonError) {
...
NTSTATUS PhCreateJsonParser(_Out_ PVOID *JsonObject, _In_ PCSTR JsonString) {
...
NTSTATUS PhCreateJsonParserEx(_Out_ PVOID *JsonObject, _In_ PVOID JsonString,
  ...
  if (Unicode) {
    ...
    json_tokener_set_flags(tokenerObject,
                           JSON_TOKENER_STRICT | JSON_TOKENER_VALIDATE_UTF8);

    jsonStringUtf8 = PhConvertUtf16ToUtf8Ex(jsonStringUtf16->Buffer,
                                            jsonStringUtf16->Length);
    jsonObject = json_tokener_parse_ex(tokenerObject, jsonStringUtf8->Buffer,
...
VOID PhFreeJsonObject(_In_ PVOID Object) { json_object_put(Object); }

PPH_STRING PhGetJsonValueAsString(_In_ PVOID Object, _In_ PCSTR Key) {
  PVOID object;
  PCSTR value;
  size_t length;

  if (object = json_get_object(Object, (PCSTR)Key)) {
    if ((length = json_object_get_string_len(object)) &&
...
PPH_STRING PhGetJsonObjectString(_In_ PVOID Object) {
  PCSTR value;
  size_t length;
...
LONGLONG PhGetJsonValueAsInt64(_In_ PVOID Object, _In_ PCSTR Key) {
  return json_object_get_int64(json_get_object(Object, Key));
}

ULONGLONG PhGetJsonValueAsUInt64(_In_ PVOID Object, _In_ PCSTR Key) {
  return json_object_get_uint64(json_get_object(Object, Key));
}

ULONG PhGetJsonUInt32Object(_In_ PVOID Object) {
  return json_object_get_int(Object);
}

ULONGLONG PhGetJsonUInt64Object(_In_ PVOID Object) {
  return json_object_get_uint64(Object);
}

LONGLONG PhGetJsonInt64Object(_In_ PVOID Object) {
  return json_object_get_int64(Object);
}

PVOID PhCreateJsonObject(VOID) { return json_object_new_object(); }

PVOID PhCreateJsonStringObject(_In_ PCSTR Value) {
  return json_object_new_string(Value);
}

PVOID PhGetJsonObject(_In_ PVOID Object, _In_ PCSTR Key) {
  return json_get_object(Object, Key);
}

PH_JSON_OBJECT_TYPE PhGetJsonObjectType(_In_ PVOID Object) {
...
LONG PhGetJsonObjectLength(_In_ PVOID Object) {
  return json_object_object_length(Object);
}

BOOLEAN PhGetJsonObjectBool(_In_ PVOID Object, _In_ PCSTR Key) {
  return json_object_get_boolean(json_get_object(Object, Key)) == TRUE;
}

VOID PhAddJsonObjectValue(_In_ PVOID Object, _In_ PCSTR Key, _In_ PVOID Value) {
  json_object_object_add_ex(Object, Key, Value,
                            JSON_C_OBJECT_ADD_KEY_IS_NEW |
                                JSON_C_OBJECT_ADD_CONSTANT_KEY);
}

VOID PhAddJsonObject(_In_ PVOID Object, _In_ PCSTR Key, _In_ PCSTR Value) {
  json_object_object_add_ex(Object, Key, json_object_new_string(Value),
                            JSON_C_OBJECT_ADD_KEY_IS_NEW |
                                JSON_C_OBJECT_ADD_CONSTANT_KEY);
}

VOID PhAddJsonObject2(_In_ PVOID Object, _In_ PCSTR Key, _In_ PCSTR Value,
                      _In_ SIZE_T Length) {
  PVOID string = json_object_new_string_len(Value, (UINT32)Length);

  json_object_object_add_ex(Object, Key, string,
                            JSON_C_OBJECT_ADD_KEY_IS_NEW |
                                JSON_C_OBJECT_ADD_CONSTANT_KEY);
}

VOID PhAddJsonObjectUtf8(_In_ PVOID Object, _In_ PCSTR Key,
                         _In_ PPH_BYTES String) {
  PVOID string =
      json_object_new_string_len(String->Buffer, (UINT32)String->Length);
...
VOID PhAddJsonObjectInt64(_In_ PVOID Object, _In_ PCSTR Key,
                          _In_ LONGLONG Value) {
  json_object_object_add_ex(Object, Key, json_object_new_int64(Value),
                            JSON_C_OBJECT_ADD_KEY_IS_NEW |
                                JSON_C_OBJECT_ADD_CONSTANT_KEY);
}

VOID PhAddJsonObjectUInt64(_In_ PVOID Object, _In_ PCSTR Key,
                           _In_ ULONGLONG Value) {
  json_object_object_add_ex(Object, Key, json_object_new_uint64(Value),
                            JSON_C_OBJECT_ADD_KEY_IS_NEW |
                                JSON_C_OBJECT_ADD_CONSTANT_KEY);
}

VOID PhAddJsonObjectDouble(_In_ PVOID Object, _In_ PCSTR Key,
                           _In_ DOUBLE Value) {
  json_object_object_add_ex(Object, Key, json_object_new_double(Value),
                            JSON_C_OBJECT_ADD_KEY_IS_NEW |
                                JSON_C_OBJECT_ADD_CONSTANT_KEY);
}

PVOID PhCreateJsonArray(VOID) { return json_object_new_array(); }

VOID PhAddJsonArrayObject(_In_ PVOID Object, _In_ PVOID Value) {
  json_object_array_add(Object, Value);
}

PVOID PhGetJsonArrayString(_In_ PVOID Object, _In_ BOOLEAN Unicode) {
  PCSTR value;
  size_t length;

  if (value = json_object_to_json_string_length(
          Object, JSON_C_TO_STRING_PLAIN,
          &length)) // json_object_get_string(Object))
  {
    if (Unicode)
      return PhConvertUtf8ToUtf16Ex(value, length);
    else
...
INT64 PhGetJsonArrayLong64(_In_ PVOID Object, _In_ ULONG Index) {
  return json_object_get_int64(json_object_array_get_idx(Object, Index));
}

ULONG PhGetJsonArrayLength(_In_ PVOID Object) {
  return (ULONG)json_object_array_length(Object);
}

PVOID PhGetJsonArrayIndexObject(_In_ PVOID Object, _In_ ULONG Index) {
  return json_object_array_get_idx(Object, Index);
}

VOID PhEnumJsonArrayObject(_In_ PVOID Object,
...
PVOID PhGetJsonObjectAsArrayList(_In_ PVOID Object) {
...
NTSTATUS PhLoadJsonObjectFromFile(_Out_ PVOID *JsonObject,
...
static VOID PhJsonObjectToJsonStringIndent(_In_ PPH_BYTES_BUILDER StringBuilder,
                                           _In_ ULONG level, _In_ ULONG flags) {
...
static void PhJsonObjectToJsonStringEscape(_In_ PPH_BYTES_BUILDER StringBuilder,
  ...
  while (Length) {
    ...
    case '/': {
      ...
      if (c == '\b')
        PhAppendBytesBuilder2(StringBuilder, "\\b");
      else if (c == '\n')
        PhAppendBytesBuilder2(StringBuilder, "\\n");
      else if (c == '\r')
        PhAppendBytesBuilder2(StringBuilder, "\\r");
      else if (c == '\t')
        PhAppendBytesBuilder2(StringBuilder, "\\t");
      else if (c == '\f')
        PhAppendBytesBuilder2(StringBuilder, "\\f");
      else if (c == '"')
        PhAppendBytesBuilder2(StringBuilder, "\\\"");
      else if (c == '\\')
        PhAppendBytesBuilder2(StringBuilder, "\\\\");
    ...
    default: {
      if (c < ' ') {
        ...
        sprintf_s(buffer, sizeof(buffer), "\\u00%c%c", json_hex_chars[c >> 4],
...
static BOOLEAN
PhInternalJsonObjectObjectToString(_In_ PPH_BYTES_BUILDER StringBuilder,
                                   _In_ PVOID Object, _In_ ULONG Level,
                                   _In_ ULONG Flags) {
...
static BOOLEAN
PhInternalJsonObjectArrayToString(_In_ PPH_BYTES_BUILDER StringBuilder,
                                  _In_ PVOID Object, _In_ ULONG Level,
                                  _In_ ULONG Flags) {
...
BOOLEAN PhJsonObjectToString(_In_ PPH_BYTES_BUILDER StringBuilder,
                             _In_ PVOID Object, _In_ ULONG Level,
                             _In_ ULONG Flags) {
  switch (json_object_get_type(Object)) {
  case json_type_null: {
  ...
  case json_type_double: {
    ...
    if (NT_SUCCESS(PhFormatDoubleToUtf8(json_object_get_double(Object),
                                        FormatStandardForm, -1, formatBuffer,
      ...
      if (sprintf_s(buffer, sizeof(buffer), "%.17g",
                    json_object_get_double(Object)) > 0) {
        // Handle specific JSON quirk: if double looks like int (no dot), append
        // .0? Standard JSON-C usually handles this, but strictly speaking "1"
        // is a valid double in JSON. We stick to the string output.
  ...
  case json_type_int: {
    ...
    // else
    {
      ...
      if (NT_SUCCESS(PhIntegerToUtf8Buffer(
              json_object_get_int64(Object), 10, TRUE, formatBuffer,
              sizeof(formatBuffer), &returnLength))) {
        PH_BYTESREF stringFormat;
        ...
        if (sprintf_s(formatBuffer, sizeof(formatBuffer), "%lld",
                      json_object_get_int64(Object)) > 0) {
  ...
  case json_type_object: {
    return PhInternalJsonObjectObjectToString(StringBuilder, Object, Level,
                                              Flags);
  } break;
  case json_type_array: {
    return PhInternalJsonObjectArrayToString(StringBuilder, Object, Level,
                                             Flags);
  } break;
  case json_type_string: {
    PhAppendBytesBuilder2(StringBuilder, "\"");
    PhJsonObjectToJsonStringEscape(
        StringBuilder, json_object_get_string(Object),
        (size_t)json_object_get_string_len(Object), Flags);
    PhAppendBytesBuilder2(StringBuilder, "\"");
  } break;
  default:
    return FALSE;
...
PPH_BYTES PhJsonObjectToJsonString(_In_ PVOID Object, _In_ ULONG Flags) {
...
NTSTATUS PhSaveJsonObjectToFile(_In_ PCPH_STRINGREF FileName, _In_ PVOID Object,
  ...
  json_flags = 0;
  PhMapFlags1(&json_flags, Flags, PhJsonFormatFlagMappings,
              RTL_NUMBER_OF(PhJsonFormatFlagMappings));
  ...
  status = PhCreateFileEx(
      &fileHandle, &fileName->sr, FILE_GENERIC_WRITE | DELETE, NULL,
      &allocationSize, FILE_ATTRIBUTE_NORMAL, FILE_SHARE_NONE,
  ...
  status = NtWriteFile(fileHandle, NULL, NULL, NULL, &ioStatusBlock,
                       ...
                       (ULONG)jsonStringUtf8->Length, NULL, NULL);
  ...
  status = PhSetFileRename(fileHandle, NULL, TRUE, FileName);
...
static mxml_node_t *PhXmlLoadString(_In_ PCSTR String) {
...
static PPH_BYTES PhXmlSaveString(_In_ PVOID XmlRootObject,
...
PVOID PhLoadXmlObjectFromString(_In_ PCSTR String) {
  mxml_node_t *currentNode;
...
PVOID PhLoadXmlObjectFromString2(_In_ PCSTR String) {
  mxml_node_t *currentNode;
...
NTSTATUS PhLoadXmlObjectFromFile(_In_ PCPH_STRINGREF FileName,
...
NTSTATUS PhSaveXmlObjectToFile(_In_ PCPH_STRINGREF FileName,
  ...
  status = PhWriteFile(fileHandle, (PVOID)string->Buffer, (ULONG)string->Length,
                       NULL, NULL);
...
VOID PhFreeXmlObject(_In_ PVOID XmlRootObject) { mxmlDelete(XmlRootObject); }

PVOID PhGetXmlObject(_In_ PVOID XmlNodeObject, _In_ PCSTR Path) {
  mxml_node_t *currentNode;
...
PVOID PhCreateXmlNode(_In_opt_ PVOID ParentNode, _In_ PCSTR Name) {
  return mxmlNewElement(ParentNode, Name);
}

PVOID PhCreateXmlOpaqueNode(_In_opt_ PVOID ParentNode, _In_ PCSTR Value) {
  return mxmlNewOpaque(ParentNode, Value);
}

PVOID PhFindXmlObject(_In_ PVOID XmlNodeObject, _In_opt_ PVOID XmlTopObject,
                      _In_opt_ PCSTR Element, _In_opt_ PCSTR Attribute,
                      _In_opt_ PCSTR Value) {
  return mxmlFindElement(XmlNodeObject, XmlTopObject, Element, Attribute, Value,
                         MXML_DESCEND_ALL);
}

PVOID PhGetXmlNodeFirstChild(_In_ PVOID XmlNodeObject) {
  return mxmlGetFirstChild(XmlNodeObject);
}

PVOID PhGetXmlNodeNextChild(_In_ PVOID XmlNodeObject) {
  return mxmlGetNextSibling(XmlNodeObject);
}

PPH_STRING PhGetXmlNodeOpaqueText(_In_ PVOID XmlNodeObject) {
  PCSTR string;
...
PCSTR PhGetXmlNodeElementText(_In_ PVOID XmlNodeObject) {
  return mxmlGetElement(XmlNodeObject);
}

PCSTR PhGetXmlNodeCDATAText(_In_ PVOID XmlNodeObject) {
  return mxmlGetCDATA(XmlNodeObject);
}

PPH_STRING PhGetXmlNodeAttributeText(_In_ PVOID XmlNodeObject,
...
PCSTR PhGetXmlNodeAttributeByIndex(_In_ PVOID XmlNodeObject, _In_ SIZE_T Index,
                                   _Out_ PCSTR *AttributeName) {
  return mxmlElementGetAttrByIndex(XmlNodeObject, Index, AttributeName);
}

VOID PhSetXmlNodeAttributeText(_In_ PVOID XmlNodeObject, _In_ PCSTR Name,
                               _In_ PCSTR Value) {
  mxmlElementSetAttr(XmlNodeObject, Name, Value);
}

SIZE_T PhGetXmlNodeAttributeCount(_In_ PVOID XmlNodeObject) {
  return mxmlElementGetAttrCount(XmlNodeObject);
}

typedef BOOLEAN(NTAPI *PPH_ENUM_XML_NODE_CALLBACK)(_In_ PVOID XmlNodeObject,
                                                   _In_opt_ PVOID Context);

BOOLEAN PhEnumXmlNode(_In_ PVOID XmlNodeObject,
...
PPH_XML_INTERFACE PhGetXmlInterface(_In_ ULONG Version) {
  static PH_XML_INTERFACE PhXmlInterface = {PH_XML_INTERFACE_VERSION,
