...
VOID KphInitialize(VOID) {
  ...
  KphMessageObjectType = PhCreateObjectTypeEx(
      L"KsiMessage", PH_OBJECT_TYPE_TRY_USE_FREE_LIST, NULL, &parameters);

  parameters.FreeListSize = KPH_MESSAGE_MIN_SIZE;
  parameters.FreeListCount = 16;

  KphUserMessageObjectType = PhCreateObjectTypeEx(
      L"KsiUserMessage", PH_OBJECT_TYPE_USE_FREE_LIST, NULL, &parameters);
}

PKPH_MESSAGE KphCreateMessage(_In_ SIZE_T Size) {
  assert(KphMessageObjectType);

  return PhCreateObject(Size, KphMessageObjectType);
}

PKPH_MESSAGE KphCreateUserMessage(_In_ KPH_MESSAGE_ID MessageId) {
  PKPH_MESSAGE msg;
...
NTSTATUS KphConnect(_In_ PKPH_CONFIG_PARAMETERS Config) {
  NTSTATUS status;
  ...
  if (Config->EnableNativeLoad || Config->EnableFilterLoad) {
    ...
    if (NT_SUCCESS(status)) {
      status =
          KphCommsStart(portName, Config->Callback, Config->RingBufferLength);
    }
  ...
  status = PhOpenService(&serviceHandle, SERVICE_START | SERVICE_QUERY_STATUS,
  ...
  status =
      PhCreateService(&serviceHandle, PhGetStringRefZ(Config->ServiceName),
                      PhGetStringRefZ(Config->ServiceName), SERVICE_ALL_ACCESS,
                      SERVICE_KERNEL_DRIVER, SERVICE_DEMAND_START,
                      SERVICE_ERROR_IGNORE, PhGetStringRefZ(Config->FileName),
                      PhGetStringRefZ(Config->ObjectName), NULL);
...
NTSTATUS KphpSetParametersService(_In_ PKPH_CONFIG_PARAMETERS Config) {
#ifdef _WIN64
  NTSTATUS status;
  ...
  if (!PhFormatToBuffer(format, RTL_NUMBER_OF(format), servicesKeyNameBuffer,
                        sizeof(servicesKeyNameBuffer), &returnLength)) {
  ...
  status =
      PhCreateKey(&servicesKeyHandle, KEY_WRITE | DELETE, PH_KEY_LOCAL_MACHINE,
                  &servicesKeyName, 0, 0, &disposition);

  if (!NT_SUCCESS(status))
    return status;

  status = PhSetValueKeyZ(servicesKeyHandle, L"SupportedFeatures", REG_DWORD,
                          &Config->FsSupportedFeatures, sizeof(ULONG));
...
NTSTATUS KphSetParameters(_In_ PKPH_CONFIG_PARAMETERS Config) {
#ifdef _WIN64
  NTSTATUS status;
  ...
  if (!PhFormatToBuffer(format, RTL_NUMBER_OF(format), parametersKeyNameBuffer,
                        sizeof(parametersKeyNameBuffer), &returnLength)) {
    return STATUS_UNSUCCESSFUL;
  }
  ...
  status =
      PhCreateKey(&parametersKeyHandle, KEY_WRITE | DELETE,
                  PH_KEY_LOCAL_MACHINE, &parametersKeyName, 0, 0, &disposition);

  if (!NT_SUCCESS(status))
    return status;

  if (Config->PortName) {
    status = PhSetValueKeyStringZ(parametersKeyHandle, L"PortName",
                                  Config->PortName);
  ...
  if (Config->Altitude) {
    status = PhSetValueKeyStringZ(parametersKeyHandle, L"Altitude",
                                  Config->Altitude);
  ...
  if (Config->Flags.Flags) {
    ...
    status =
        PhSetValueKeyUlong(parametersKeyHandle, L"Flags", Config->Flags.Flags);
  ...
  if (Config->SystemProcessName) {
    status = PhSetValueKeyZ(parametersKeyHandle, L"SystemProcessName", REG_SZ,
                            Config->SystemProcessName->Buffer,
                            (ULONG)Config->SystemProcessName->Length +
                                sizeof(UNICODE_NULL));
...
VOID KphSetServiceSecurity(_In_ SC_HANDLE ServiceHandle) {
  ...
  sdAllocationLength =
      ...
      (ULONG)sizeof(ACCESS_ALLOWED_ACE) + PhLengthSid(&PhSeServiceSid) +
      (ULONG)sizeof(ACCESS_ALLOWED_ACE) + PhLengthSid(administratorsSid) +
      (ULONG)sizeof(ACCESS_ALLOWED_ACE) + PhLengthSid(&PhSeInteractiveSid);
  ...
  PhCreateAcl(dacl, sdAllocationLength - SECURITY_DESCRIPTOR_MIN_LENGTH,
              ACL_REVISION);
  PhAddAccessAllowedAce(dacl, ACL_REVISION, SERVICE_ALL_ACCESS,
                        &PhSeServiceSid);
  PhAddAccessAllowedAce(dacl, ACL_REVISION, SERVICE_ALL_ACCESS,
                        administratorsSid);
...
_Function_class_(PH_ENUM_KEY_CALLBACK) static BOOLEAN NTAPI
  ...
  if (NT_SUCCESS(PhOpenKey(&keyHandle, KEY_READ | DELETE, RootDirectory,
                           &keyName, 0))) {
    PhEnumerateKey(keyHandle, KeyBasicInformation,
                   KsiLoadUnloadServiceCleanupKeyCallback, NULL);
...
NTSTATUS KsiLoadUnloadService(_In_ PKPH_CONFIG_PARAMETERS Config,
                              _In_ BOOLEAN LoadDriver) {
#ifdef _WIN64
  static CONST PH_STRINGREF fullServicesKeyName = PH_STRINGREF_INIT(
      L"\\Registry\\Machine\\System\\CurrentControlSet\\Services\\");
  static CONST PH_STRINGREF parametersKeyName =
      PH_STRINGREF_INIT(L"Parameters");
  NTSTATUS status;
  ...
  fullServiceKeyName =
      PhConcatStringRef2(&fullServicesKeyName, Config->ServiceName);
  ...
  status = PhCreateKey(&serviceKeyHandle, KEY_WRITE | KEY_CREATE_SUB_KEY, NULL,
                       &fullServiceKeyName->sr, 0, 0, &disposition);

  if (NT_SUCCESS(status) && disposition == REG_CREATED_NEW_KEY) {
    fullServiceFileName =
        PhConcatStringRef2(&PhNtDosDevicesPrefix, Config->FileName);
    ...
    PhSetValueKeyStringZ(serviceKeyHandle, L"ImagePath",
                         &fullServiceFileName->sr);
  ...
  if (!NT_SUCCESS(status) || !LoadDriver) {
    if (NT_SUCCESS(PhOpenKey(&serviceKeyHandle, KEY_READ | DELETE, NULL,
                             &fullServiceKeyName->sr, 0))) {
      if (NT_SUCCESS(PhOpenKey(&parametersKeyHandle, DELETE, serviceKeyHandle,
                               &parametersKeyName, 0))) {
        NtDeleteKey(parametersKeyHandle);
        NtClose(parametersKeyHandle);
      }

      PhEnumerateKey(serviceKeyHandle, KeyBasicInformation,
                     KsiLoadUnloadServiceCleanupKeyCallback, NULL);
...
NTSTATUS KphServiceStop(_In_ PKPH_CONFIG_PARAMETERS Config) {
  NTSTATUS status;
  ...
  if (Config->EnableNativeLoad || Config->EnableFilterLoad) {
    ...
    status = PhOpenService(&serviceHandle, SERVICE_STOP,
                           PhGetStringRefZ(Config->ServiceName));
...
NTSTATUS KphGetInformerSettings(_Out_ PKPH_INFORMER_SETTINGS Settings) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS KphSetInformerSettings(_In_ PKPH_INFORMER_SETTINGS Settings) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS KphEscalateHandleAccess(_In_ HANDLE ProcessHandle, _In_ HANDLE Handle,
                                 _In_ ACCESS_MASK AccessMask,
                                 _Out_ PHANDLE NewHandle) {
  NTSTATUS status;
  PKPH_MESSAGE msg;

  msg = KphCreateUserMessage(KphMsgEscalateHandle);
  msg->User.EscalateHandle.ProcessHandle = ProcessHandle;
  msg->User.EscalateHandle.Handle = Handle;
  msg->User.EscalateHandle.AccessMask = AccessMask;
  msg->User.EscalateHandle.NewHandle = NewHandle;
  status = KphCommsSendMessage(msg);

  if (NT_SUCCESS(status)) {
    status = msg->User.EscalateHandle.Status;
  }
...
NTSTATUS KphReadVirtualMemory(_In_opt_ HANDLE ProcessHandle,
...
NTSTATUS KphWriteVirtualMemory(_In_opt_ HANDLE ProcessHandle,
                               _In_ PVOID BaseAddress,
  ...
  msg = KphCreateUserMessage(KphMsgWriteVirtualMemory);
  msg->User.WriteVirtualMemory.ProcessHandle = ProcessHandle;
  msg->User.WriteVirtualMemory.BaseAddress = BaseAddress;
  msg->User.WriteVirtualMemory.Buffer = Buffer;
  msg->User.WriteVirtualMemory.BufferSize = BufferSize;
  msg->User.WriteVirtualMemory.NumberOfBytesWritten = NumberOfBytesWritten;
  status = KphCommsSendMessage(msg);

  if (NT_SUCCESS(status)) {
    status = msg->User.WriteVirtualMemory.Status;
  }
...
NTSTATUS KphCaptureStackBackTraceThread(
    ...
    _In_ ULONG FramesToCapture, _Out_writes_(FramesToCapture) PVOID *BackTrace,
  ...
  msg->User.CaptureStackBackTraceThread.Timeout =
      PhTimeoutFromMilliseconds(&timeout, 300);
...
NTSTATUS
KsiEnumerateProcessHandles(_In_ HANDLE ProcessHandle,
  ...
  while (TRUE) {
    status = KphEnumerateProcessHandles(ProcessHandle, buffer, bufferSize,
                                        &bufferSize);
...
NTSTATUS KphQueryInformationObject(
  ...
  msg->User.QueryInformationObject.ObjectInformationClass =
      ObjectInformationClass;
  msg->User.QueryInformationObject.ObjectInformation = ObjectInformation;
  msg->User.QueryInformationObject.ObjectInformationLength =
...
NTSTATUS KphQueryObjectThreadName(_In_ HANDLE ProcessHandle, _In_ HANDLE Handle,
  ...
  status = KphQueryInformationObject(ProcessHandle, Handle,
                                     KphObjectThreadNameInformation, buffer,
...
NTSTATUS KphSetInformationObject(
  ...
  msg->User.SetInformationObject.ObjectInformationClass =
      ObjectInformationClass;
  msg->User.SetInformationObject.ObjectInformation = ObjectInformation;
  msg->User.SetInformationObject.ObjectInformationLength =
      ObjectInformationLength;
  status = KphCommsSendMessage(msg);
...
KPH_PROCESS_STATE KphGetProcessState(_In_ HANDLE ProcessHandle) {
  KPH_PROCESS_STATE state;

  if (!KphCommsIsConnected())
    return 0;

  if (!NT_SUCCESS(KphQueryInformationProcess(ProcessHandle,
                                             KphProcessStateInformation, &state,
                                             sizeof(state), NULL)))
    return 0;
...
KPH_PROCESS_STATE KphGetCurrentProcessState(VOID) {
  return KphGetProcessState(NtCurrentProcess());
}

KPH_LEVEL KphProcessLevel(_In_ HANDLE ProcessHandle) {
  KPH_PROCESS_STATE state;
...
KPH_LEVEL KphLevelEx(_In_ BOOLEAN Cached) {
...
KPH_LEVEL KsiLevel(VOID) { return KphLevelEx(TRUE); }
...
NTSTATUS KphSetInformationProcess(
  ...
  msg->User.SetInformationProcess.ProcessInformationClass =
      ProcessInformationClass;
  msg->User.SetInformationProcess.ProcessInformation = ProcessInformation;
  msg->User.SetInformationProcess.ProcessInformationLength =
      ProcessInformationLength;
  status = KphCommsSendMessage(msg);
...
NTSTATUS KphSetInformationThread(
  ...
  msg->User.SetInformationThread.ThreadInformationClass =
      ThreadInformationClass;
  msg->User.SetInformationThread.ThreadInformation = ThreadInformation;
  msg->User.SetInformationThread.ThreadInformationLength =
...
NTSTATUS KphSystemControl(_In_ KPH_SYSTEM_CONTROL_CLASS SystemControlClass,
...
NTSTATUS KphAlpcQueryCommunicationsNamesInfo(
  ...
  while (TRUE) {
    status = KphAlpcQueryInformation(ProcessHandle, PortHandle,
...
NTSTATUS KphQueryInformationFile(
    _In_ HANDLE ProcessHandle, _In_ HANDLE FileHandle,
    _In_ FILE_INFORMATION_CLASS FileInformationClass,
    _Out_writes_bytes_(FileInformationLength) PVOID FileInformation,
    _In_ ULONG FileInformationLength, _Out_ PIO_STATUS_BLOCK IoStatusBlock) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS KphQueryVolumeInformationFile(
    ...
    _In_ ULONG FsInformationLength, _Out_ PIO_STATUS_BLOCK IoStatusBlock) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
  ...
  msg->User.QueryVolumeInformationFile.FsInformationLength =
      FsInformationLength;
...
NTSTATUS KphDuplicateObject(_In_ HANDLE ProcessHandle, _In_ HANDLE SourceHandle,
                            _In_ ACCESS_MASK DesiredAccess,
...
NTSTATUS
KphQueryPerformanceCounter(_Out_ PLARGE_INTEGER PerformanceCounter,
    ...
    *PerformanceCounter = msg->User.QueryPerformanceCounter.PerformanceCounter;
    if (PerformanceFrequency) {
      *PerformanceFrequency =
          msg->User.QueryPerformanceCounter.PerformanceFrequency;
    }
  } else {
...
NTSTATUS KphCreateFile(_Out_ PHANDLE FileHandle, _In_ ACCESS_MASK DesiredAccess,
                       _In_ PCOBJECT_ATTRIBUTES ObjectAttributes,
...
NTSTATUS KphQueryInformationThread(
  ...
  msg->User.QueryInformationThread.ThreadInformationClass =
      ThreadInformationClass;
  msg->User.QueryInformationThread.ThreadInformation = ThreadInformation;
  msg->User.QueryInformationThread.ThreadInformationLength =
      ThreadInformationLength;
...
NTSTATUS KphQuerySection(
    ...
    _In_ ULONG SectionInformationLength, _Out_opt_ PULONG ReturnLength) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS
KphQuerySectionMappingsInfo(_In_ HANDLE SectionHandle,
                            _Out_ PKPH_SECTION_MAPPINGS_INFORMATION *Info) {
  NTSTATUS status;
  PVOID buffer;
  ULONG bufferSize = MAX_PATH;
  ...
  *Info = NULL;
  ...
  while (TRUE) {
    status = KphQuerySection(SectionHandle, KphSectionMappingsInformation,
                             buffer, bufferSize, &bufferSize);
    if (status == STATUS_BUFFER_TOO_SMALL) {
      PhFree(buffer);
      buffer = PhAllocate(bufferSize);
...
NTSTATUS KphCompareObjects(_In_ HANDLE ProcessHandle,
...
NTSTATUS KphGetMessageSettings(_Out_ PKPH_MESSAGE_SETTINGS Settings) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS KphSetMessageSettings(_In_ PKPH_MESSAGE_SETTINGS Settings) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS KphAcquireDriverUnloadProtection(_Out_opt_ PLONG PreviousCount,
  ...
  if (NT_SUCCESS(status)) {
    ...
    if (NT_SUCCESS(status)) {
      ...
      if (ClientPreviousCount)
        *ClientPreviousCount =
            msg->User.AcquireDriverUnloadProtection.ClientPreviousCount;
    }
  }
...
NTSTATUS KphReleaseDriverUnloadProtection(_Out_opt_ PLONG PreviousCount,
                                          _Out_opt_ PLONG ClientPreviousCount) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
  ...
  if (NT_SUCCESS(status)) {
    ...
    if (NT_SUCCESS(status)) {
      ...
      if (ClientPreviousCount)
        *ClientPreviousCount =
            msg->User.ReleaseDriverUnloadProtection.ClientPreviousCount;
    }
  }
...
NTSTATUS KphGetConnectedClientCount(_Out_ PULONG Count) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS KphActivateDynData(_In_ PBYTE DynData, _In_ ULONG DynDataLength,
...
NTSTATUS KphIsDynDataActive(_Out_ PBOOLEAN IsActive) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
KphRequestSessionAccessToken(_Out_ PKPH_SESSION_ACCESS_TOKEN AccessToken,
                             _In_ PLARGE_INTEGER Expiry, _In_ ULONG Privileges,
                             _In_ LONG Uses) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
  ...
  if (NT_SUCCESS(status)) {
    RtlCopyMemory(AccessToken, &msg->User.RequestSessionAccessToken.AccessToken,
...
NTSTATUS KphAssignProcessSessionToken(_In_ HANDLE ProcessHandle,
...
NTSTATUS KphAssignThreadSessionToken(_In_ HANDLE ThreadHandle,
                                     _In_ PBYTE Signature,
                                     _In_ ULONG SignatureLength) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
...
NTSTATUS KphGetInformerProcessSettings(_In_ HANDLE ProcessHandle,
...
NTSTATUS KphSetInformerProcessSettings(_In_opt_ HANDLE ProcessHandle,
...
NTSTATUS KphStripProtectedProcessMasks(_In_ HANDLE ProcessHandle,
...
NTSTATUS KphQueryVirtualMemory(
  ...
  msg->User.QueryVirtualMemory.MemoryInformationLength =
...
NTSTATUS
KsiQueryHashInformationFile(_In_ HANDLE FileHandle,
  ...
  msg->User.QueryHashInformationFile.HashingInformationLength =
...
NTSTATUS KphOpenDevice(_Out_ PHANDLE DeviceHandle,
                       _In_ ACCESS_MASK DesiredAccess,
...
NTSTATUS KphOpenDeviceDriver(_In_ HANDLE DeviceHandle,
                             _In_ ACCESS_MASK DesiredAccess,
...
NTSTATUS KphOpenDeviceBaseDevice(_In_ HANDLE DeviceHandle,
                                 _In_ ACCESS_MASK DesiredAccess,
...
NTSTATUS KphGetInformerStats(_In_opt_ HANDLE ProcessHandle,
...
NTSTATUS KphGetMessageStats(_Out_ PKPH_INFORMER_STATS Stats) {
  NTSTATUS status;
  PKPH_MESSAGE msg;
